...
The Japanese, Swedish, Italian, Chinese, and T-80-family visual deltas live in strict
`profiles/japan.ts`, `profiles/sweden.ts`, `profiles/italy.ts`, and
`profiles/china.ts` packs plus `profiles/t80.ts`, while the AMX-40 visual build
lives in strict `france.ts`; all six use narrow procedural-builder ports. Preserve their

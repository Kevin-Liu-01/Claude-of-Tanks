...
function sideFace(side:number,segments=48):THREE.BufferGeometry {
  ...
  const parts=[turned(bowl,segments),turned(cap,12).rotateX(Math.PI/12),turned(inboard,12)];
...
export function k1a1XWheelSolids(segments=48):{core:THREE.BufferGeometry;left:THREE.BufferGeometry;right:THREE.BufferGeometry} {
  return {core:turned([[-.0599,0],[-.0599,.024],[.0545,.024],[.0545,0]],24),
    left:sideFace(-1,segments),right:sideFace(1,segments)};
}

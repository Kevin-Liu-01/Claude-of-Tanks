...
try {
  ...
  // sticker (removed by the 2026-09-13 symmetry pass, restored 2026-09-15:
  // "bring back its negative space windows on the front of the turret").
  // Rays through its mouth must hit the rear bulkhead; the floor and outer
  // reveal still close the structural armor around that void, and the outer
  // chine stays the mirror of the -X cheek.
  ...
  for (const z of [1.2, 1.5, 1.75]) {
    near(shellRay(1.25, z), shellRay(-1.25, z), 0.02, `outer cheek roof symmetric at z=${z}`);
    near(shellRay(1.25, z, true), shellRay(-1.25, z, true), 0.02, `outer cheek chin symmetric at z=${z}`);
  }

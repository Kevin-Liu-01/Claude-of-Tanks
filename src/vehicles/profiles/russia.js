...
function buildT62MV1(P) {
  ...
  const pD = { rings, sz: 0.74, k1Y: 0.10, k1Pitch: 0.20, k1Out: -0.06, k1OutI: [0.03, 0.02, -0.06, -0.06, -0.06], k1N: 5, k1T0: 0.30, k1Step: 0.22, k1H: 0.20 };
  eraRuCheeks(P, pD, 'k1');
  ...
  // §B3 census MG: DShK-class pintle on the loader ring, barrel stowed
  // muzzle-down over the crown slope (elev -0.5 keeps every barrel column
  // under the dome silhouette; receiver rides the ref's own spike zone —
  // inside the ≤0.4 gate-pt pintle allowance)
  {
    const mg = FITTINGS.pintleMG({ mats: P.mats, cls: 'dshk', tone: 'dark', elev: -0.5, ammo: true });
...
function buildT90AVladimir(P) {
  ...
  loftHull(P, {
    ...
    deck: [[-4.755, 1.51], [-4.51, 1.655], [-4.29, 1.671], [-4.15, 1.50], [-3.85, 1.475], [-3.72, 1.51], [-3.05, 1.514], [-2.72, 1.66], [-2.55, 1.75], [-0.92, 1.745], [-0.86, 1.46], [0.36, 1.45], [0.77, 1.38], [1.68, 1.29]],
    belly: [[-4.755, 1.50], [-4.61, 1.19], [-4.46, 1.10], [-4.40, 0.85], [-4.10, 0.78], [-3.90, 0.77], [-3.78, 0.57], [-2.87, 0.30], [1.22, 0.30], [1.68, 0.60]],
  ...
  const rings = [[1.38, -0.02], [1.50, 0.10], [1.35, 0.24], [1.05, 0.345], [0.72, 0.405], [0.40, 0.428], [0.15, 0.433], [0.02, 0.435]];
  ...
  for (const zc of [-0.175, 0.135]) {
    P.add('turret', box(0.48, 0.49, 0.31), -0.97, 0.48, zc);
    P.add('turret', box(0.09, 0.44, 0.31), -0.735, 0.44, zc);
  }
  for (const zc of [0.447, 0.685, 0.923]) P.add('turret', box(0.52, 0.415, 0.238), -0.95, 0.4425, zc);
  // rTAIL r13: block REAR extension — ref side tops rise 2.204 by z -1.24
  // (the desirefx housing at x -0.95..-1.146, z -1.28..-0.86 tops 2.219;
  // r13b: rear face pulled to -1.29 — at -1.38 it lit the -1.345 col where
  // the ref falls to its 1.985 dome-rear line)
  // (r13c: rear face 23mm clear of the -1.293 col boundary — masks light
  // at ANY partial pixel coverage, so margins need >=2px ≈ 22mm in side)
  P.add('turret', box(0.52, 0.48, 0.20), -0.95, 0.46, -0.42);
  // rTAIL r13: block FRONT stub — ref side col +0.373 reads 2.123 (cluster
  // front edge ends z_w 0.34); stub z_w 0.335..0.41
  P.add('turret', box(0.52, 0.406, 0.075), -0.95, 0.42, 1.1225);
  // glass pane FLUSH on the block face (it sat at z_l 1.24 = z_w 0.49 and
  // alone lit the side 0.48 col at 2.096 vs the ref's bare 1.505..1.639
  // K-5-tip sliver — the round's worst side_turret column)
  P.add('turretGlass', box(0.32, 0.18, 0.03), -0.95, 0.52, 1.045);
  ...
  P.add('turret', box(0.08, 0.14, 0.10), -0.229, 0.50, -1.03);
  P.add('turretDetail', box(0.026, 0.27, 0.008), -0.229, 0.885, -1.235);
  P.add('turretDark', box(0.03, 0.04, 0.04), -0.229, 0.90, -1.235);
  ...
  P.add('turret', box(0.067, 0.167, 0.108), 0.3835, 0.6835, -1.105);
  ...
  P.add('turret', box(0.18, 0.145, 0.10), 1.11, 0.3725, -0.2175);
  ...
  // r13b: top rail raised 0.24 -> 0.345 (ref side -2.203 col tops 1.878)
  for (const y of [0.12, 0.345]) {
    P.add('turretDetail', box(0.80, 0.035, 0.10), 0, y, -1.475);
    for (const s of [-0.42, 0.02, 0.42]) P.add('turretDetail', box(0.035, y > 0.2 ? 0.0 : 0.42, 0.035), s, 0.08, -1.44);
  }
...
function buildT72B87(P) {
  ...
  P.add('turretDark', box(0.05, 0.42, 0.05), -0.75, 0.67, -1.105);
  ...
  P.add('turretTrack', box(0.06, 0.085, 0.43), 1.58, -0.0425, -0.19);   // shelf slab (floor 1.3375, clear of the 1.673 col)
  P.add('turretTrack', box(0.03, 0.085, 0.16), 1.63, -0.0425, -0.08);   // outer shelf step, z world 0.21..0.37
  P.add('turretTrack', box(0.085, 0.46, 0.44), 1.5475, 0.23, -0.19);    // standing K-1 raft, top world 1.88 (ref 1.884 @ x 1.56)
  P.add('turretTrack', box(0.044, 0.44, 0.16), 1.629, 0.24, -0.08);     // outer raft column (col 1.64, z-narrow like ref 1.673)
  P.add('turretDark', box(0.07, 0.022, 0.40), 1.5475, 0.447, -0.19);    // raft top seam (§B3)
  ...
  P.add('turret', box(0.78, 0.28, 0.73), 0, 0.25, -1.475);
  P.add('turretDark', box(0.74, 0.22, 0.028), 0, 0.25, -1.826);
  for (const sx of [-0.13, 0.13]) P.add('turretDark', box(0.022, 0.20, 0.026), sx, 0.25, -1.825); // jerrycan seams (§B3)
  ...
  for (const s of [-1, 1]) {
    P.add('turretDetail', box(0.022, 0.022, 0.34), s * 1.50, 0.28, -0.75);
    for (const dz of [-0.13, 0.13]) P.add('turretDetail', box(0.10, 0.05, 0.018), s * 1.44, 0.253, -0.75 + dz);
  }
  // ---- 2A46M (fused in the ref; mine stays a Gun node) ----
...
function buildT90SM(P) {
  ...
  loftHull(P, {
    ...
    sponsonY: [[-2.92, 0.81], [-2.84, 1.18], [-2.06, 1.18], [-1.78, 0.81], [2.52, 0.81], [2.64, 1.15], [3.02, 1.15]],
  });
  ...
  for (const s of [-1, 1]) {
    ...
    P.add('hull', box(0.46, 0.17, 0.18), s * 1.17, 1.16, 3.175);    // fender bridge base->tip (§B2; floor 1.075 — idler wrap arc 1.059, §B4)
  ...
  for (const s of [-1, 1]) {
    P.add('hull', box(0.185, 0.62, 0.30), s * 0.385, 1.07, -3.03);    // rack A x 0.29..0.48
    P.add('hull', box(0.185, 0.38, 0.24), s * 0.385, 1.19, -3.31);    // rack B x 0.29..0.48, z -3.43..-3.19 (hullLengthM body)
    P.add('hull', box(0.185, 0.08, 0.09), s * 0.385, 1.15, -3.435);   // tail sliver bar y 1.11..1.19 to -3.48
    P.add('hullDark', box(0.15, 0.30, 0.022), s * 0.385, 1.19, -3.391); // rack end-frame plate (§B3 rail tell, flush)
    P.add('hull', box(0.11, 0.50, 0.24), s * 1.15, 1.08, -3.14);      // outer rack pair rear -3.26 (ref -3.264 @ 1.16)
    P.add('hull', box(0.26, 0.44, 0.30), s * 1.30, 1.10, -2.94);
    P.add('hullDark', box(0.20, 0.022, 0.24), s * 1.30, 1.309, -2.94); // corner-bin lid seam (§B3)
  }
  // towrope coil bridging the twin racks (plan center rear -3.43, ref
  // -3.428 at |x|<=0.44; rides inside the sliver-bar side band)
  P.add('hullDark', box(0.84, 0.05, 0.05), 0, 1.13, -3.405);
  // rack tray floor: §B2 top-down fill for the pocket the twin racks +
  // towrope enclose (63 cells at (0, -3.16))
  P.add('hull', box(0.62, 0.03, 0.50), 0, 1.015, -3.17);
  ...
  ruDeck(P, { deckY: 1.44, hatchZ: 2.12, gz: -1.67, grilles: 5, gw: 1.5, periY: 1.22 });
  ...
  for (let row = 0; row < 2; row++) for (const s of [-1, 1]) {
    P.add('hullTrack', box(0.72, 0.075, 0.30), s * 0.42, 1.26 - row * 0.06, 2.05 + row * 0.27, -0.42, s * 0.35, 0);
  }
  ...
  ruSkirtBand(P, { x: 1.765, z0: -2.93, z1: 3.10, yTop: 1.30, yBot: 0.94, panels: 7, th: 0.08 });
  ...
  for (const s of [-1, 1]) {
    P.add('hullRubber', box(0.44, 0.19, 0.09), s * 1.475, 1.095, -2.975); // ref corner rear -3.02 (r12 re-decode)
    P.add('hull', box(0.05, 0.16, 0.40), s * 1.44, 1.10, -2.92);      // flap bracket onto the corner bin (§B2)
  }
  ...
  for (const s of [-1, 1]) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      P.add('hull', box(0.037, 0.37, 0.44), s * 1.8485, 1.125, -2.55 + i * 0.47);
  ...
  P.add('turret', polyTurret([
    ...
    [tw * 0.95, -0.45], [tw * 0.70, b], [-tw * 0.70, b], [-tw * 0.95, -0.45],
  ...
  for (const s of [-1, 1]) {
    ...
    P.add('turret', box(0.165, 0.48, 0.835), s * 1.6525, 0.20, 0.5925);
    P.add('turret', box(0.12, 0.40, 0.52), s * 1.80, 0.165, 0.755);
    P.add('turretDark', box(0.10, 0.022, 0.46), s * 1.80, 0.343, 0.755);
  ...
  P.add('turretTrack', box(0.04, 0.17, 0.035), -1.83, -0.085, 0.8875);
  ...
  // r10 roof band RE-DECODE (fresh side digest overrules r8): the ref 2.25
  // plateau lives at z world -0.97..-1.41 (5 cols) with 2.06-2.09 at
  // -0.43..-0.54 and 1.95 between; ref front caps the |x| 0.91..0.99 cols
  // at 2.105. Tall bin narrowed to |x| 0.70..0.86 and moved aft; the 2.10
  // box (below) owns the -0.43..-0.54 cols. Lid seam = §B3 bin tell.
  P.add('turret', box(0.16, 0.30, 0.50), -0.78, 0.70, -1.28);
  P.add('turretDark', box(0.14, 0.022, 0.46), -0.78, 0.834, -1.28);
  ...
  P.add('turretDark', box(0.28, 0.022, 0.84), 0.85, 0.748, 0.14);      // §B3: right roof-bin lid seam
  // §B3 census MG: PKT-class pintle on the bustle rack pedestal — receiver
  // top 2.11 rides UNDER the ref's own 2.25 roof plateau (z world -0.97..
  // -1.41); barrel drooped so every column stays inside the roofline
  {
    const mg = FITTINGS.pintleMG({ mats: P.mats, cls: 'nsvt', tone: 'dark', elev: -0.18, ammo: true });
  ...
  // r10b: step3 rear retreat + thin tail slat — fresh side col z -2.49
  // reads ref top 1.706 (the old -2.51 rear face held 1.925 there); the
  // slat carries the 1.55..1.71 band like the ref's rear slat (plan rear
  // -2.49 vs ref -2.43 staircase, +0.06 accepted).
  P.add('turret', box(1.82, 0.205, 0.20), 0, 0.4225, -2.39);
  P.add('turretDark', box(1.30, 0.16, 0.05), 0, 0.23, -2.555);
  for (const s of [-1, 1]) {
    P.add('turret', box(0.15, 0.36, 0.90), s * 0.985, 0.29, -1.63);
    P.add('turret', box(0.16, 0.36, 0.44), s * 1.15, 0.29, -1.20);     // mid step rear world -1.33 (ref -1.314 @ 1.152; inner edge keeps §B2 contact)
    P.add('turret', box(0.12, 0.34, 0.30), s * 1.24, 0.27, -0.86);
  }
  ...
  P.add('turretDark', box(1.80, 0.20, 0.05), 0, 0.42, -2.47);
  ...
  P.add('turretGlass', box(0.18, 0.20, 0.016), -0.50, 0.68, -1.996);  // §B3: rear sight panel lens
  P.add('turretDark', box(0.26, 0.03, 0.02), -0.50, 0.82, -2.02);     // §B3: panel hood lip (inside panel top)
  ...
  tubeGun(P, [
    [0.72, 3.20, 0.111], [3.20, 4.97, 0.097],
  ], { rings: [[1.20, 0.113], [1.90, 0.113], [2.60, 0.113], [3.20, 0.099], [3.80, 0.099], [4.45, 0.099]], muzzle: 4.97 });
    ...
    for (const s of [1, -1]) {
      ...
      for (const e of [-1, 1]) {
        P.add('turretTrack', box(0.06, H - 0.02, H - 0.02),
          x - e * bx * (L / 2 + 0.02), yc + e * Math.sin(rz) * (L / 2), z - e * bz * (L / 2 + 0.02), px5, ry, rz);
      }
      for (let i = 0; i < 3; i++) {

...
function buildT54(P) {
  ...
  P.addGunExtra(box(0.46, 0.34, 0.30), 0, 0, 0.12);                   // pig-snout collar
  tubeGun(P, [
...
function buildT80Line(P, v) {
  ...
  if (v >= 1) {
    ...
    for (const s of [-1, 1]) {
      P.add('turretTrack', box(0.30, 0.16, 0.12), s * 0.40, 0.30, 1.52, -0.20, s * 0.35, 0);
      P.add('turretTrack', box(0.30, 0.16, 0.12), s * 0.74, 0.28, 1.46, -0.20, s * 0.35, 0);
      P.add('turretTrack', box(0.30, 0.16, 0.12), s * 1.06, 0.26, 1.32, -0.20, s * 0.35, 0);
    }
  ...
  if (v === 2) {
    // T-80BV Kontakt-1: cheek arc (shared k1 kit, opt-in params) + glacis raft
    eraRuCheeks(P, { rings: ringsT, sz: 1.05, rCz: 0.0, k1Y: 0.06, k1Pitch: 0.25, k1T0: 0.24, k1Step: 0.22, k1H: 0.22, k1Out: 0.02 }, 'k1');
  ...
  if (v !== 2) {
    ...
    // r27: re-seated (0.52, 0.72) -> (0.42, 0.66) — the compressed ref's
    // front MG spikes are 2.29-2.30 at the +0.38..0.46 columns with the
    // crown falling to 2.19 by ±0.50 (the old seat printed 2.32-2.35 into
    // the 2.19 columns; a plain drop then read -0.11 under the spikes).
...
function buildT84(P) {
  ...
  loftHull(P, {
    deck: [[-4.30, 1.28], [-4.24, 1.333], [-2.60, 1.333], [-2.16, 1.321], [-0.10, 1.301], [0.35, 1.301], [0.55, 1.278], [0.75, 1.272], [0.90, 1.215], [1.45, 1.172], [1.91, 1.148], [1.99, 1.06]],
  ...
  for (let i = 0; i < 10; i++) P.add('hull', box(1.67, 0.135, 0.443), 0, 0.2925, -3.33 + (i + 0.5) * 0.455);
  ...
  for (const s of [-1, 1]) P.add('hull', box(0.42, 0.065, 0.38), s * 1.06, 1.3325, -2.86);  // SIDE humps over the track bays (ref front 1.38 only at |x| 0.86..1.27)
  P.add('hull', box(1.70, 0.03, 0.24), 0, 1.317, 0.18);       // splash rail (deckCorner 1.333@0.27)
  ...
  P.add('hull', box(0.96, 0.12, 0.42), 0, 1.18, -4.51);       // center overhang -> −4.72
  for (const s of [-1, 1]) {
    P.add('hull', box(0.38, 0.12, 0.26), s * 0.69, 1.18, -4.43);          // mid notch -> −4.56
    P.add('hull', box(0.40, 0.12, 0.23), s * 1.10, 1.18, -4.415);         // corner base -> −4.53
    P.add('hull', box(0.08, 0.12, 0.33), s * 0.92, 1.18, -4.695);         // flap finger A -> −4.86 (bins ±1.03/1.14 read the −4.53 base)
    P.add('hull', box(0.08, 0.12, 0.33), s * 1.26, 1.18, -4.695);         // flap finger B -> −4.86
    P.add('hull', box(0.14, 0.27, 0.11), s * 0.35, 0.985, -4.695);        // stern tow hooks (band-deep −4.769 col, inside the center block plan)
    P.add('hullRubber', box(0.18, 0.56, 0.06), s * 1.45, 0.92, -4.43);    // rear corner flaps (plan −4.43..−4.46 @ ±1.38..1.53)
  }
  P.add('hull', box(1.00, 0.25, 0.40), 0, 0.99, -4.50);       // rear plate center face −4.70 (|x|<=0.50: the ±0.59 plan bins read −4.50)
  for (const s of [-1, 1]) P.add('hull', box(0.24, 0.25, 0.21), s * 0.62, 0.99, -4.405);   // plate inner step -> −4.51
  for (const s of [-1, 1]) P.add('hull', box(0.14, 0.25, 0.32), s * 0.81, 0.99, -4.46);    // plate outer step -> −4.62 (ref bin ±0.81 reads −4.61)
  P.add('hull', box(1.00, 0.10, 0.33), 0, 0.815, -4.465);     // exhaust shelf face 0.765 -> −4.63
  ...
  P.add('hullDark', box(0.90, 0.16, 0.03), 0, 1.00, -4.695);  // plate louver
  ...
  for (const s of [-1, 1]) {
    ...
    P.add('hull', box(0.84, 0.07, 0.13), s * 1.28, 0.955, 2.165);          // nose lip 2.10..2.23
  ...
  for (const s of [-1, 1]) {
    P.add('hullRubber', box(0.13, 0.45, 0.06), s * 1.655, 0.525, 2.055);  // flap on the stub face (measured optimum: wide/short variants re-seated the side registration and cost 1.3-3 hull pts — §C stray-column law)
  ...
  for (const s of [-1, 1]) {
    for (let i = 0; i < 5; i++) P.add('hull', box(0.44, 0.03, 0.40), s * 1.53, 1.30, -4.28 + (i + 0.5) * 0.42);
    for (let i = 0; i < 5; i++) P.add('hull', box(0.44, 0.03, 0.40), s * 1.53, 1.28, -2.18 + (i + 0.5) * 0.42);
    for (let i = 0; i < 2; i++) P.add('hull', box(0.44, 0.026, 0.44), s * 1.53, 1.215, -0.06 + (i + 0.5) * 0.46);
  }
  ruDeck(P, { deckY: 1.31, hatchX: 0.45, hatchY: 1.245, hatchZ: 0.62, periY: 1.14, gz: -2.56, grilles: 3, gw: 1.10 });
  ...
  for (const [zr, yr] of [[1.10, 1.192], [1.31, 1.180], [1.52, 1.168], [1.70, 1.158]]) {
  ...
  // run + tow cable draped across the plateau + low tool boxes.
  {
    ...
    links.position.set(-0.62, 1.295, -3.42);   // recessed flush: top 1.345 on the 1.333 deck (station i2 reads the deck line)
    P.hullG.add(links);
    const links2 = FITTINGS.spareTrackLinks({ mats: P.mats, links: 3, width: 0.48, seed: 9 });
    links2.position.set(0.72, 1.295, -3.60);
    P.hullG.add(links2);
    ...
    const cable = FITTINGS.towCable({ mats: P.mats, eyes: false, pts: [[-0.95, 1.326, -2.72], [0.2, 1.324, -3.25], [0.95, 1.326, -3.78]], seed: 5 });
    P.hullG.add(cable);
  }
  P.add('hullDetail', box(0.52, 0.024, 0.16), -0.80, 1.333, -3.62);   // left tool tray (right side carries the second link run)
  ...
  for (const s of [-1, 1]) {
    ...
    P.add('hull', slab(                                        // z 0.87..1.31
      ...
      [xi, 1.175, 1.31], [xo, 1.175, 1.31], [xo, 1.209, 0.87], [xi, 1.209, 0.87]));
    P.add('hull', slab(                                        // z 1.35..1.79
      [xi, 0.80, 1.79], [xo, 0.80, 1.79], [xo, 0.80, 1.35], [xi, 0.80, 1.35],
      [xi, 1.150, 1.79], [xo, 1.150, 1.79], [xo, 1.176, 1.35], [xi, 1.176, 1.35]));
    P.add('hull', slab(                                        // z 1.83..1.93
      [xi, 0.80, 1.93], [xo, 0.80, 1.93], [xo, 0.80, 1.83], [xi, 0.80, 1.83],
      [xi, 1.122, 1.93], [xo, 1.122, 1.93], [xo, 1.148, 1.83], [xi, 1.148, 1.83]));
    ...
    P.add('hull', box(0.22, 0.24, 0.14), s * 1.63, 0.92, 2.00);
    ...
    P.add('hull', box(0.20, 0.62, 0.06), s * 1.62, 0.95, -4.33);
  }
  ...
  ruGlacisKit(P, { w: 3.1, y: 1.10, z: 1.55, eyes: false, hookX: 0.86, hookY: 0.72, hookZ: 1.99, hookBucket: 'hullRubber', hlY: 1.08 });
  P.add('hullRubber', box(1.90, 0.28, 0.05), 0, 0.56, 1.925);
  KIT.towCable(P, [[-1.00, 1.31, 0.40], [0, 1.305, -0.30], [1.00, 1.31, 0.40]]);
  buildRunningGear(P, {
  ...
  ruSkirtBand(P, { x: 1.72, th: 0.05, z0: -4.38, z1: -2.20, yTop: 1.32, yBot: 0.64, panels: 5, lipX: 1.737, lipY: 0.95 });
  ruSkirtBand(P, { x: 1.72, th: 0.05, z0: -2.20, z1: -0.10, yTop: 1.30, yBot: 0.64, panels: 5, lipX: 1.737, lipY: 0.95 });
  ruSkirtBand(P, { x: 1.72, th: 0.05, z0: -0.10, z1: 0.86, yTop: 1.24, yBot: 0.64, panels: 3, lipX: 1.737, lipY: 0.95 });
  for (const s of [-1, 1]) for (let i = 0; i < 10; i++) {
  ...
  // apron 0.94 spans −0.16..−1.73). local z = world + 0.95, y = world − 1.32.
  P.turretG.position.set(0, 1.32, -0.95);
  // low collar (widest band ±1.26, z −1.03..0.50, y 1.58..1.66 — front bin
  // ±1.27 wants 1.65 content 12mm past the 1.2474 boundary)
  for (let i = 0; i < 4; i++) P.add('turret', box(2.52, 0.08, 0.3825), 0, 0.30, -0.08 + (i + 0.5) * 0.3825);
  ...
  for (let i = 0; i < 2; i++) {
    P.add('turret', box(0.74, 0.32, 0.26), -0.49, 0.42, 1.30 + (i + 0.5) * 0.26);
    P.add('turret', box(0.59, 0.32, 0.26), 0.565, 0.42, 1.30 + (i + 0.5) * 0.26);
  }
  for (let i = 0; i < 2; i++) {  // ring-side cheek base sits higher (ref bottom 1.669 @ z W 0.26)
    P.add('turret', box(0.74, 0.27, 0.245), -0.49, 0.445, 0.81 + (i + 0.5) * 0.245);
    P.add('turret', box(0.59, 0.27, 0.245), 0.565, 0.445, 0.81 + (i + 0.5) * 0.245);
  }
  // cheek apex ramp 1.90@0.85 -> 2.04@−0.16 (side cols 1.94..2.03 exact)
  P.add('turret', slab(
    [-0.86, 0.56, 1.80], [0.86, 0.56, 1.80], [0.86, 0.56, 0.79], [-0.86, 0.56, 0.79],
    [-0.86, 0.585, 1.80], [0.86, 0.585, 1.80], [0.86, 0.72, 0.79], [-0.86, 0.72, 0.79]));
  // chamfer wedges fill the plan corner line (0.86,1.80)->(1.22,1.36)
  for (const s of [-1, 1]) {
    P.add('turret', slab(
      [s * 0.86, 0.26, 1.80], [s * 0.86, 0.26, 0.79], [s * 1.22, 0.26, 0.79], [s * 1.22, 0.26, 1.36],
      [s * 0.86, 0.50, 1.80], [s * 0.86, 0.50, 0.79], [s * 1.22, 0.50, 0.79], [s * 1.22, 0.50, 1.36]));
  }
  P.add('turret', box(2.48, 0.52, 0.48), 0, 0.52, 0.21);       // tall body walls 2.10 @ ±1.24 (front bins to ±1.23 read 2.06-2.11), z −0.50..−0.98
  P.add('turret', box(2.26, 0.52, 0.33), 0, 0.52, -0.195);     // shoulder ±1.13 to −1.31
  for (let i = 0; i < 4; i++) P.add('turret', box(1.60, 0.145, 0.39), 0, 0.8125, 0.55 - (i + 0.5) * 0.39);  // roof plates 2.205 @ −0.40..−1.96 (col −2.04 is the bustle's)
  P.add('turret', box(1.00, 0.14, 0.21), 0, 0.77, 0.695);      // apex step (ref 2.19@−0.29)
  P.add('turret', box(0.68, 0.175, 0.14), -0.54, 0.8075, 0.52);   // gunner/pano sight housing 2.215 (ref side 2.216 @ −0.40..−0.62; front tops 2.243 run to x −0.88 — LEFT side only)
  P.add('turretGlass', box(0.22, 0.06, 0.02), -0.35, 0.83, 0.60);
  P.add('turret', box(0.17, 0.14, 0.35), -0.945, 0.85, 0.225);    // left shoulder block 2.24 (ref front 2.243 to x −1.02)
  P.add('turret', box(0.26, 0.175, 0.12), 0.35, 0.8075, 0.51);    // commander sight 2.215
  ...
  for (const s of [-1, 1]) P.add('turret', box(0.06, 0.42, 0.36), s * 0.825, 0.53, 0.63);
  ...
  for (let i = 0; i < 4; i++) P.add('turret', box(1.60, 0.66, 0.395), 0, -0.05, 0.79 - (i + 0.5) * 0.395);  // ring carrier / apron 0.94 -> −1.74 (the −1.7583 bin boundary belongs to the bustle bottoms)
  // bustle: core ±0.86 to −3.05 (bottom staircase 1.66/1.72/1.79/1.85 per
  // the ref side line), tail chamfer ±0.80
  P.add('turret', box(1.72, 0.46, 0.30), 0, 0.57, -1.00);
  for (let i = 0; i < 2; i++) P.add('turret', box(1.72, 0.40, 0.20), 0, 0.60, -1.15 - (i + 0.5) * 0.20);
  for (let i = 0; i < 2; i++) P.add('turret', box(1.72, 0.31, 0.175), 0, 0.645, -1.55 - (i + 0.5) * 0.175);
  P.add('turret', box(0.88, 0.27, 0.20), 0, 0.665, -2.00);     // tail chamfer ±0.44 (ref plan rear −3.0 only at |x|<=0.46)
  // left bustle flank to x −0.93 (print asymmetry: ref plan bin −0.916
  // reads −2.92 while the right stops at ±0.86); bottoms follow the core
  // staircase so the −2.6..−2.9 cols keep the ref's 1.81..1.86 line
  P.add('turret', box(0.07, 0.42, 0.35), -0.895, 0.59, -1.075);
  P.add('turret', box(0.07, 0.36, 0.35), -0.895, 0.62, -1.425);
  P.add('turret', box(0.07, 0.29, 0.35), -0.895, 0.655, -1.775);
  P.add('turret', box(1.10, 0.09, 0.27), 0.10, 0.845, -1.745);  // Utes/stowage crate 2.21 @ −2.56..−2.83
  P.add('turretDark', box(1.10, 0.02, 0.29), 0.10, 0.895, -1.745);
  ...
  for (let i = 0; i < 2; i++) P.add('turretDetail', box(0.22, 0.34, 0.23), 0.98, 0.59, -0.85 - (i + 0.5) * 0.23);
  P.add('turretDetail', box(0.105, 0.32, 0.07), 1.1475, 0.50, -0.885);
  P.add('turretDetail', box(0.19, 0.36, 0.06), -0.965, 0.52, -0.88);
  ...
  P.add('turret', cylY(0.235, 0.265, 0.098, 14), 0.42, 0.869, -0.35);   // drum wall 2.14..2.238
  P.add('turretDark', cylY(0.20, 0.20, 0.014, 14), 0.42, 0.905, -0.35); // recessed hatch disc (top 2.232, inside grace)
  for (let k = 0; k < 7; k++) {
    const a = (k / 7) * Math.PI * 2 + 0.35;
    P.add('turretDark', box(0.055, 0.032, 0.045), 0.42 + Math.cos(a) * 0.195, 0.897, -0.35 + Math.sin(a) * 0.195, 0, -a, 0);
  }
  {
    ...
    kord.position.set(-0.35, 0.735, -0.10);
    P.turretG.add(kord);
  }
  for (const s of [-1, 1]) {
    ...
    smoke.position.set(s * 0.60, 0.52, 1.55);  // tube tips stay behind the 0.87 cheek plan face and under the 1.94 mantlet-band line
    P.turretG.add(smoke);
  }
  {
    ...
    rack.position.set(0.93, 0.42, -0.96);  // outer face 1.08: the print's stowage plan steps to −1.87 at x 1.10..1.20 — the first 1.17 seat printed +0.176 into the 1.15 plan column
    P.turretG.add(rack);
    // roof-plate seam lines (5 mm — side-invisible, top-view edge density
    // toward the ref's 1363 edge-px class)
    for (const zr of [0.16, -0.23, -0.62]) P.add('turretDark', box(1.52, 0.006, 0.02), 0, 0.888, zr);
  }
  // ---- KBA-3 (2A46M class): axis 1.835 (band 1.94..1.73), tube r 0.100
  // (plan bin law: edge inside the ±0.1015 column boundary — the warped
  // ref's own plan tube lights only bins −0.04/0.08), evac as a BOX (side
  // 1.97 without plan width), muzzle +4.86 = rear −4.86 + 9.72.
  P.gunG.position.set(0, 0.515, 1.55);
  ruSaddle(P, { rollR: 0.13, rollW: 0.44, tubeR: 0.064, rootR: 0.080, rootL: 0.50 });
  P.addGunExtra(box(0.38, 0.36, 0.42), -0.05, -0.075, 0.06);   // mantlet 1.58..1.94 @ 0.45..0.87 (slot-asymmetric like the print)
  tubeGun(P, [
  ...
  P.addGunExtra(box(0.40, 0.235, 0.71), 0, 0.018, 2.15);       // evac box: band 1.735..1.97 @ world 2.40..3.11, plan halfW 0.20 (ref plan bins ±0.15/0.18 read 3.10)
  P.decal('turret', 'number', P.spec.visual.number || '', 0.26, [1.215, 0.50, 0.20], Math.PI / 2);
  P.decal('turret', 'number', P.spec.visual.number || '', 0.26, [-1.215, 0.50, 0.20], -Math.PI / 2);
  P.topY = 1.30;
}
...
function buildT90MProryv(P) {
  ...
  loftHull(P, {
    ...
    deck: [[-2.90, 1.37], [-2.47, 1.39], [-1.77, 1.35], [-1.10, 1.37], [0.10, 1.385], [1.22, 1.38], [2.18, 1.28], [2.70, 1.23], [3.00, 1.17], [3.06, 1.05]],
  ...
  for (const s of [-1, 1]) {
    P.add('hull', box(0.72, 0.28, 0.26), s * 1.35, 1.02, 3.14);
    P.add('hull', box(0.72, 0.05, 0.12), s * 1.35, 1.145, 3.21);
    ...
    P.add('hullRubber', box(0.39, 0.33, 0.045), s * 1.6475, 0.875, 3.265);
  }
  P.add('hull', box(0.18, 0.36, 0.06), 0, 0.92, 3.37);
  ...
  for (const s of [-1, 1]) P.add('hull', box(0.695, 0.08, 0.50), s * 0.4925, 1.21, -3.16);
  ...
  for (const s of [-1, 1]) {
    P.add('hullDetail', cylX(0.14, 0.66, 12), s * 0.48, 1.62, -3.18);
    P.add('hull', box(0.09, 0.53, 0.44), s * 0.9425, 1.485, -3.14);
  }
  for (const s of [-1, 1]) P.add('hullDetail', box(0.44, 0.14, 0.30), s * 0.38, 1.77, -3.19);
  ...
  P.add('hullDetail', box(0.24, 0.12, 0.28), 0, 1.55, -2.96);
  for (const s of [-1, 1]) P.add('hullWood', cylX(0.09, 0.72, 10), s * 0.50, 1.60, -3.41);
  for (const s of [-0.86, 0.86]) P.add('hullDark', cylX(0.097, 0.04, 10), s, 1.60, -3.41);
  for (const s of [-1, 1]) P.add('hullDark', box(0.52, 0.34, 0.045), s * 0.42, 1.52, -3.4025);
  P.add('hull', box(0.24, 0.20, 0.18), 0, 1.25, -3.02);
  P.add('hullDark', box(1.45, 0.32, 0.05), 0, 1.05, -2.925);   // hull rear grille
  ...
  for (const s of [-1, 1]) P.add('hullRubber', box(0.30, 0.24, 0.04), s * 1.55, 1.20, -2.94);
  ...
  for (const s of [-1, 1]) for (let i = 1; i < 9; i++) {
    P.add('hull', box(0.20, 0.05, 0.50), s * 1.70, 1.37, -2.60 + i * 0.545);
  }
  ...
  ruSkirtBand(P, { x: 1.802, th: 0.08, z0: -2.61, z1: 2.55, yTop: 1.36, yBot: 0.78, panels: 6, lipX: 1.80, lipY: 0.80 });
  for (const s of [-1, 1]) {
    P.add('hull', box(0.08, 0.50, 0.25), s * 1.802, 1.03, 2.675);
    P.add('hull', box(0.08, 0.38, 0.22), s * 1.802, 0.98, 2.91);
    P.add('hull', box(0.08, 0.33, 0.24), s * 1.802, 0.955, 3.14);
  }
  ...
  for (const s of [-1, 1]) P.add('hullDark', box(0.10, 0.015, 4.30), s * 1.74, 0.79, 0.55);
  ...
  for (const s of [-1, 1]) {
    for (let k = 0; k < 6; k++) {
      P.add('hullTrack', box(0.045, 0.86, 0.80), s * 1.8675, 0.91, 2.05 - k * 0.86);
      P.add('hullDark', box(0.04, 0.74, 0.045), s * 1.865, 0.91, 1.62 - k * 0.86);
    }
    P.add('hullTrack', box(0.045, 0.86, 0.36), s * 1.8675, 0.91, 2.62);
    P.add('hullDark', box(0.04, 0.74, 0.045), s * 1.865, 0.91, 2.48);
  }
  ...
  P.add('turret', polyTurret([
    [-0.45, 1.50], [0.45, 1.50], [0.90, 1.44], [1.14, 1.24], [1.32, 1.06],
    [1.50, 0.90], [1.62, 0.77], [1.72, 0.57], [1.55, -0.13],
    [1.35, -0.58], [1.15, -1.03], [1.00, -1.38], [0.90, -1.62],
    [-0.90, -1.62], [-1.00, -1.38], [-1.15, -1.03], [-1.35, -0.58],
    [-1.55, -0.13], [-1.72, 0.57], [-1.62, 0.77], [-1.50, 0.90],
    [-1.32, 1.06], [-1.14, 1.24], [-0.90, 1.44], [-0.45, 1.50],
  ], hm, 1.00, 0.93));
  ...
  for (const s2 of [-1, 1]) {
    P.add('turret', box(0.62, 0.24, 0.31), s2 * 0.51, 0.7175, -1.085);
    P.add('turret', box(0.62, 0.24, 0.77), s2 * 0.51, 0.7175, -0.545);
  }
  ...
  P.add('turret', box(2.02, 0.20, 1.04), 0, 0.70, -0.70);
  P.add('turret', box(0.14, 0.20, 1.04), 1.08, 0.70, -0.70);
  P.add('turret', box(0.14, 0.17, 1.04), -1.08, 0.525, -0.70);
  for (const s of [-1, 1]) P.add('turret', box(0.16, 0.23, 0.80), s * 1.23, 0.555, -0.70);
  ...
  for (const s of [-1, 1]) P.add('turret', box(0.14, 0.16, 0.60), s * 1.38, 0.50, -0.13);
  P.add('turret', box(1.56, 0.20, 0.54), 0, 0.70, 0.11);
  P.add('turret', box(1.30, 0.12, 0.53), 0, 0.57, 0.705);  // r30c: 2.03 step runs to world 1.10 (ref 2.00@0.99-1.06)
  P.add('turret', box(1.10, 0.10, 0.25), 0, 0.45, 1.095);
  P.add('turret', box(0.92, 0.09, 0.43), 0, 0.365, 1.435);
  P.add('turret', box(1.60, 0.10, 0.24), 0, 0.54, -1.34);
  ...
  P.add('turret', box(1.90, 0.50, 0.42), 0, 0.36, -1.66);
  P.add('turret', box(1.90, 0.50, 0.41), 0, 0.36, -2.075);
  P.add('turret', box(1.82, 0.25, 0.23), 0.04, 0.345, -2.395);
  P.add('turretDark', box(1.78, 0.22, 0.03), 0.04, 0.36, -2.50);
  P.add('turret', box(1.70, 0.09, 0.045), 0.04, -0.045, -2.5625);
  ...
  {
    const kord = FITTINGS.pintleMG({
      mats: P.mats, cls: 'nsvt', scale: 0.54, tone: 'dark', ammo: true, elev: 0.04,
    });
  ...
  for (const s of [-1, 1]) {
    P.add('turret', box(0.16, 0.18, 0.42), s * 1.56, 0.36, 0.81);
    P.add('turretDark', box(0.14, 0.02, 0.38), s * 1.56, 0.46, 0.81);
    const bank = FITTINGS.smokeBank({
      mats: P.mats, count: 5, r: 0.033, len: 0.30, pitch: -0.25, splay: s * 0.55,
      arc: 0.40, spacing: 0.08, seed: 9 + s,
    });
    bank.position.set(s * (s < 0 ? 1.445 : 1.52), 0.26, 0.90);
    P.turretG.add(bank);
  }
  ...
  eraRuCheeks(P, {
    rings: [[twm, 0], [twm * 0.96, hm * 0.6], [twm * 0.9, hm]], sz: 0.80,
    weldFlat: true, rCz: 0.10, rDist: -0.14, rRows: 1,
  }, 'relikt');
  ...
  tubeGun(P, [
    [0.55, 0.85, 0.118], [0.85, 1.40, 0.118], [1.40, 1.92, 0.118],
    [1.92, 2.16, 0.100], [2.16, 2.62, 0.100], [2.62, 3.08, 0.100],
    [3.08, 3.54, 0.100], [3.54, 4.00, 0.100], [4.00, 4.46, 0.100],
    [4.46, 4.92, 0.100],
  ], { rings: [[2.60, 0.106], [3.40, 0.106], [4.20, 0.106]], muzzle: 4.92 });
  P.add('gun', box(0.02, 0.25, 0.24), 0, 0, 2.04);      // evac crest fin (r30c: 0.25 band — under the 12% body-column threshold)
...
function buildT72B3M(P) {
  ...
  for (const [zc, top, cTop, s1Top, s2Top] of [
    [-2.7775, 1.792, 1.7675, 1.67, 1.57],
    [-2.3765, 1.7805, 1.7675, 1.70, 1.60],
    [-1.9755, 1.7875, 1.7675, 1.73, 1.64],
  ...
  P.add('hullCloth', box(2.75, 0.02, 0.42), -0.075, 1.7155, -2.01);
  ...
  for (const [zc, py] of [[-2.88, 1.7865], [-2.52, 1.775], [-2.02, 1.782], [-1.68, 1.7865], [-1.22, 1.776]]) {
    ...
    // on the sagged left-outboard shoulder in the front render; r25: the
    // -2.88 drop caps 1.64 — its 1.70 top read row ~197 at image col
    // 100-101, +8 over the ref's 205 fender line at the window edge.)
    P.add('hullDark', box(0.008, zc === -2.88 ? 0.20 : 0.26, 0.05), -1.4415, zc === -2.88 ? 1.54 : 1.57, zc);
  }
  ...
  P.add('hull', box(0.18, 0.095, 0.30), -1.36, 1.7145, -1.5745);
  ...
  for (const [px3, pw3, pt3] of [[-1.02, 0.48, 1.735], [-0.33, 0.55, 1.70], [0.36, 0.50, 1.63], [1.02, 0.46, 1.575]]) {
    P.add('hullDetail', box(pw3, pt3 - 1.495, 0.005), px3, (1.495 + pt3) / 2, -2.9675, 0.30, 0, 0);
    P.add('hullDark', box(0.016, pt3 - 1.475, 0.004), px3 + pw3 / 2 + 0.055, (1.475 + pt3) / 2, -2.9665, 0.30, 0, 0);
  }
  ...
  for (const s of [-1, 1]) {
    ...
    // behind the prong's own ~1.935 front corner (the hullLengthM/dAlong
    // anchor column is untouched); x-reach inside the 1.51 corner.
    P.add('hull', box(0.10, 0.14, 0.008), s * 1.468, 1.032, 1.885, -0.35, s * -0.75, 0);
  ...
  // row; mine read as clean bars). Dot tops 1.4325 = the certified deck
  // row-band ceiling the lips already hold.
  [-3.50, -3.645, -3.83, -3.955].forEach((iz, li) => {
    for (let di = 0; di < 7; di++) {
      const dx3 = 0.145 + di * 0.138 + [0.006, -0.008, 0.004, -0.005][((di + li) % 4)];
      P.add('hullDark', box(0.014, 0.006, 0.012), dx3, 1.4295, iz - 0.026);
    }
  });
  ...
  P.add('hull', box(0.72, 0.34, 0.015), -0.91, 1.5855, -3.000);
  P.add('hull', box(0.75, 0.085, 0.078), -0.925, 1.4575, -3.046);
  // (r24: a first cut hung two full-width wall panels at z -3.07..-3.08 to
  // cover the stepped lines — their 1.7495 top edges re-created the exact
  // x499-505 aft-step silhouette the round is ordered to SHRINK (Δ20 at
  // the fall cols, measured). Reverted; the lines die at their sources
  // instead: the spine cTop raise + sag-strip deletes below.)
  // (r24 item 6c: center/right rears -3.065 -> -3.035 — they were 2 of the
  // remaining fall cols; fronts stay -2.99, tops 1.62/1.60 side-covered by
  // the left piece's max-over-x as before.)
  P.add('hull', box(1.10, 0.205, 0.045), 0, 1.5175, -3.0125);
  P.add('hull', box(0.75, 0.185, 0.045), 0.925, 1.5075, -3.0125);
  ...
  // (r24 item 6b: sag plates slide 0.025 forward — their rear corners were
  // 2 of the 7 aft-step fall cols; the left corner's 1.793 side-row is
  // carried by the band top 1.792 via max-over-z either way.)
  // r25 item 3 (second cut): the left plate's 0.03 outboard overhang past
  // the pile edge (-1.30 vs pile -1.27) far-projected the view-front
  // row-182 dash at image cols 122-125 (+19 over the ref's 201 line —
  // measured on the r25 pairs). Trimmed to the pile edge; cols 122-125
  // fall back to the front lip's own 201 print. Side rows unchanged (the
  // band top 1.792 carries them via max-over-z, r24 note above).
  P.add('hull', box(0.72, 0.02, 0.075), -0.91, 1.766, -2.945, -0.5, 0, 0);
  P.add('hull', box(1.10, 0.02, 0.075), 0, 1.673, -2.945, -0.5, 0, 0);
  P.add('hull', box(0.75, 0.02, 0.075), 0.925, 1.573, -2.945, -0.5, 0, 0);
  // r25 item 2b DELIVERED HERE (band-pile rear +8-10 luma): the visible
  // dead-rear wall rows 264-296 is NOT the z -2.9675 bag lobes (they sit
  // 2.5 mm BEHIND every pile piece's own -2.965 rear plane — fully
  // occluded; re-bucketing them measured 74.4 -> 74.0 = nothing). The
  // actual wall = the AFT-STEP rear faces (z -3.0075/-3.035) + the spine
  // rear (z -2.965) peeking above the center step. SKIN PLATES: thin
  // hullDetail faces 2 mm proud of each parent, TOP-EDGE-FLUSH pitched
  // up 0.30 rad (the whole plate leans toward the rear camera, so no row
  // of it hides behind its parent — the bottom-flush variant buries
  // itself after 7 mm). Hemi up-catch is the only lever on rear normals
  // (sun NdotL < 0 until ~29 deg; flat-vertical detail ceilings at ~76 vs
  // the ref's 84.3 flat wall). Split with 12-15 mm gaps = the ref's own
  // vertical bin seams (parent camo reads through as the dark parting).
  // Masks: tops 4-8 mm under each parent's certified print; bottom edges
  // swing aft only at y 1.44-1.52 (inside the stern mass, below every
  // aft-step fall-col row); plan covered by the pile/sag plates/deck.
  {
    // (r25 tone calibration, three measured points on the aft-most faces:
    // detail skins 88.4 @ 0.30 rad / 87.6 @ 0.20; cloth skins 73.4 @ 0.20
    // — the flat-top tone bank (69.6/63.8) does not transfer to exposed
    // rear-vertical faces, where the buckets straddle the ref's 84.3 by
    // +3.3/-10.9 and tilt is a ~0.1 luma/deg trim. NEITHER bucket lands
    // the ordered 82-84 window alone. Delivery: VERTICAL 71/29
    // detail/cloth slat interleave (bin-strap grammar) — every image row
    // crosses the full pattern, so 300-col row means blend to
    // 0.71*87.6 + 0.29*73.4 ~ 83.5 with near-zero row-std, the ref
    // wall's own flat signature.)
    const rearSkin = (x0, x1, y0, y1, zPlane) => {
      const h = y1 - y0, zc = zPlane - (h / 2) * Math.sin(0.20);
      const yc = (y0 + y1) / 2, PITCH = 0.20, DET = 0.142;
      for (let xa = x0; xa < x1 - 0.002; xa += PITCH) {
        const xd1 = Math.min(xa + DET, x1);
        P.add('hullDetail', box(xd1 - xa, h, 0.004), (xa + xd1) / 2, yc, zc, 0.20, 0, 0);
        const xc1 = Math.min(xa + PITCH, x1);
        if (xc1 - xd1 > 0.002) P.add('hullCloth', box(xc1 - xd1, h, 0.004), (xd1 + xc1) / 2, yc, zc, 0.20, 0, 0);
      }
    };
    rearSkin(-1.255, -0.87, 1.45, 1.7515, -3.0095);   // left step face
    rearSkin(-0.855, -0.565, 1.45, 1.7515, -3.0095);
    rearSkin(-0.53, -0.03, 1.44, 1.612, -3.037);      // center step face
    rearSkin(-0.015, 0.53, 1.44, 1.612, -3.037);
    rearSkin(-0.45, -0.03, 1.617, 1.7595, -2.967);    // spine rear peek
    rearSkin(-0.015, 0.54, 1.617, 1.7595, -2.967);
    rearSkin(0.575, 0.92, 1.44, 1.592, -3.037);       // right step face
    rearSkin(0.935, 1.27, 1.44, 1.592, -3.037);
  }
  // r24 item 2 (critic r12 TURRET-REAR GRAMMAR, decoded on the pairs): the
  // dead-rear "horizontal slat-crate wall" rows 264-296 is NOT the rack —
  // it is the BAND-PILE REAR + AFT-STEP zone (z -2.96..-3.09, the
  // aft-most surfaces): the stacked lines were the three hullDark sag
  // strips (rows 283/297 class) and the center-spine cTop staircase step
  // edges (row 268-270). The strips are DELETED (the sag plates keep the
  // r15 roll-down read); the spine raise is in the band loop above.
  ...
  const pD = { rings, sz: 0.733, rT0: 0.46, rStep: 0.27, rDists: [-0.27, -0.10, -0.06], rD: 0.14, rDeep: 0.10, rY: 0.0115, rY0: 0.0115, rH: 0.19, rTilt: -0.12, rRows: 1, rStrip: false, rCz: -0.20,
    ...
    rXPairs: [[0.62, -0.16, 0.242, 0.36, 0.1325, 0.12, 0.112], [0.90, -0.15, 0.246, 0.38, 0.1345, 0.12, 0.116],
      [1.16, -0.10, 0.3375, 0.31, 0.17825, 0.12, 0.22], [1.44, -0.10, 0.3375, 0.31, 0.17825, 0.12, 0.22],
      [1.72, -0.17, 0.31, 0.31, 0.203, 0, 0.20], [2.00, -0.26, 0.26, 0.30, 0.225, 0, 0.15],
  ...
  P.add('turret', box(0.032, 0.16, 0.30), -1.254, 0.48, -0.46);
  ...
  //    exactly like the ref's; the under-rod air breaks open to sky.
  {
    ...
    rodMesh(ghostFlat, 0.022, 0.018, 0.28, -0.955, 0.771, -0.19);
    ...
    rodMesh(ghostFlat, 0.016, 0.30, 0.022, 0.276, 0.57, 0.35);
  ...
  P.add('turret', box(0.56, 0.316, 0.012), -0.005, 0.185, -1.907);
  P.add('turret', box(0.33, 0.31, 0.010), -0.42, 0.180, -1.8425, 0, -0.30, 0);
  P.add('turret', box(0.33, 0.31, 0.010), 0.41, 0.180, -1.8425, 0, 0.30, 0);
  P.add('turretDark', box(0.014, 0.30, 0.075), -0.302, 0.18, -1.869);
  P.add('turretDark', box(0.014, 0.30, 0.075), 0.292, 0.18, -1.869);
  ...
  P.add('turret', box(0.004, 0.26, 0.34), 0.622, 0.15, -1.545);
  P.add('turretDark', box(0.006, 0.010, 0.40), 0.6235, 0.225, -1.545);
  P.add('turretDark', box(0.006, 0.010, 0.40), 0.6235, 0.085, -1.545);
  P.add('turretDark', box(0.006, 0.30, 0.014), 0.6235, 0.155, -1.44);
  P.add('turretDark', box(0.20, 0.008, 0.012), 0.30, 0.19, -1.8555, 0, 0.30, 0);
  ...
  P.add('turretTrack', box(0.062, 0.28, 0.10), -0.4625, 0.555, -1.57);
  P.add('turretTrack', box(0.112, 0.012, 0.112), -0.465, 0.693, -1.57);

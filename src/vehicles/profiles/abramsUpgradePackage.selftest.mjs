...
for (const quality of ['high', 'low']) {
  ...
  assert.ok(Math.abs(turret.position.y-(1.67 + .09 + .012))<1e-8, 'complete M1A3 turret reseated to leave a 12 mm ring seam');
  ...
  for(let i=0;i<bearing.count;i++) {
    if(Math.abs(bearing.getY(i)+.115)<1e-5) bearingBottom=Math.min(bearingBottom,bearing.getY(i)+turret.position.y);
    if(Math.abs(bearing.getY(i)+.097)<1e-5) bearingTop=Math.max(bearingTop,bearing.getY(i)+turret.position.y);
  }
  assert.ok(Math.abs(bearingTop-bearingBottom-.018)<1e-6, 'actual bearing is 90% shorter than its previous 180 mm height');
  ...
  assert.ok(visibleSeam>.011 && visibleSeam<.013, 'roughly 12 mm of exposed ring remains');
  ...
  for (const side of [-1,1]) for (const y of [-.108,-.106,-.104]) {

...
const CR1_HULL = {
  ...
  deck: [[4.16, 1.19], [2.90, 1.60], [2.30, 1.64], [-1.30, 1.66],
    [-2.20, 1.728], [-2.56, 1.732], [-3.10, 1.727], [-3.51, 1.734], [-4.03, 1.73],
    [-4.09, 1.71]],
  ...
  // Main skirt run ends z 1.45 — the ref's own front-panel course drops to
  // a 1.62 top there (side_hull cols z 1.5..2.45 read 1.624; the old full-
  // height 1.74 run was +0.13 on eight columns). The lower front panels
  // are authored in the build fn (real CR1 panel course, co-planar hem).
  skirt: { x: 1.655, top: 1.74, bot: 0.615, z0: -3.30, z1: 0.90 }, skirtPanels: 8,
...
function challenger1Build(P) {
  ...
  for (const s of [-1, 1]) {
    ...
    W(0.95, 1.65, 4.165, 2.95, (z) => (z > 3.45 ? 0.99 + 0.01 * (4.165 - z) / 0.715 : 1.00));
    W(1.65, 1.70, 3.30, 2.95, () => 1.285);
    W(1.65, 1.745, 3.60, 3.28, (z) => rk(z) - 0.11);
    // tip flap z 4.02 (the old 4.05 center let the flap's last 3 cm paint
    // the 4.112 column to 0.65 where the ref bottom steps up to 0.974)
    P.add('hullRubber', box(0.5, 0.35, 0.05), s * 1.30, 0.82, 4.02, -0.08, 0, 0);
  }
  ...
  for (const s of [-1, 1]) {
    ...
    // joint, not a slope quantization; side_hull cols 0.99..2.45 read 1.624)
    for (const zc of [1.1775, 1.7275, 2.2775]) {
      P.add('hull', box(0.05, 1.01, 0.525), s * 1.63, 1.12, zc);
    }
    P.add('hullDark', box(0.05, 0.90, 0.016), s * 1.633, 1.11, 1.4525);
    P.add('hullDark', box(0.05, 0.90, 0.016), s * 1.633, 1.11, 2.0025);
  }
  towCableUK(P);
  // Engine deck louvres.
  ...
  P.add('hull', box(1.6, 0.17, 0.20), 0, 1.745, -2.97);
  ...
  for (const s of [-1, 1]) {
    ...
    // and ±1.2 columns both ways)
    P.add('hull', box(0.175, 0.61, 1.03), s * 1.0425, 1.425, -3.535);
    segBoxZ(P, 'hull', 0.20, 0.05, s > 0 ? 1.50 : 1.35, s * 1.31, 1.705, s > 0 ? -2.95 : -2.875);
    // outer tail-guard stubs (ref plan is ASYMMETRIC: left rear -3.51,
    // right rear -3.705 at the ±1.5 columns). They SEAL against the skirt
    // inner face (x to 1.61) and vent inboard at 1.41..1.50 — a mid-channel
    // stub enclosed the strip/skirt slot top-down (§B2, 20 cells).
    P.add('hull', box(0.11, 0.05, s > 0 ? 0.40 : 0.20), s * 1.555, 1.705, s > 0 ? -3.50 : -3.40);
  }
  ...
  for (const s of [-1, 1]) P.add('hullDark', box(0.62, 0.19, 0.09), s * 0.61, 1.525, -4.145);
  ...
  P.add('turret', slab(
    [-1.02, 0.05, 2.90], [1.02, 0.05, 2.90], [1.16, 0.05, 0.75], [-1.16, 0.05, 0.75],
  ...
  // own nose line (1.573@2.81 rising to 1.66@1.70 world — one surface, not
  // a dropped ledge).
  P.add('turret', slab(
    [-0.52, -0.05, 3.02], [0.52, -0.05, 3.02], [0.80, 0.04, 1.9], [-0.80, 0.04, 1.9],
    [-0.30, 0.62, 2.02], [0.30, 0.62, 2.02], [0.44, 0.72, 1.55], [-0.44, 0.72, 1.55]));
  // Mantlet-recess underside mass (the ref's 1.42..1.62 band at world
  // z 1.66..2.06 — its trunnion/collar line sweeps low ahead of the deep
  // mass; also closes the slot under the raised chin).
  P.add('turret', box(1.2, 0.24, 0.40), 0, -0.08, 2.06);
  // Crown plateau (the casting's own 2.498 roof) — narrowed to the ref's
  // ±0.70 flat with 2.28 cheek-top shelves outboard (front_whole cols
  // 0.78..0.92 read 2.27-2.31 where the old ±0.93 flat painted 2.50);
  // plan footprint unchanged (shelves carry the ±0.93 plane).
  P.add('turret', box(1.40, 0.30, 1.01), 0, 0.728, 0.315);
  for (const s of [-1, 1]) P.add('turret', box(0.23, 0.22, 1.01), s * 0.815, 0.55, 0.315);
  // Rear roof falling to the bustle (underside raised 0.05 -> 0.13: the
  // dead slab belly painted the turret-mask bottoms 1.67 across five
  // columns where the ref's shell reads 1.75-1.79).
  P.add('turret', slab(
    [-1.05, 0.13, -0.10], [1.05, 0.13, -0.10], [0.95, 0.13, -1.50], [-0.95, 0.13, -1.50],
    [-0.74, 0.828, -0.15], [0.74, 0.828, -0.15], [0.62, 0.60, -1.48], [-0.62, 0.60, -1.48]));
  // §B1 chamfered joint: the crown plateau (0.878) met the rear roof
  // (0.828) over a bare 5 cm ledge — a bevel now grades the two planes
  // (top corners follow the narrowed ±0.70-0.74 crown so the front-view
  // cheek columns keep the ref's 2.28 shelf line).
  P.add('turret', slab(
    [-0.90, 0.70, -0.10], [0.90, 0.70, -0.10], [0.90, 0.70, -0.19], [-0.90, 0.70, -0.19],
    [-0.74, 0.828, -0.10], [0.74, 0.828, -0.10], [0.70, 0.878, -0.19], [-0.70, 0.878, -0.19]));
  ...
  P.add('turret', box(1.06, 0.30, 0.44), 0, 0.47, -1.61);
  P.add('turret', box(1.06, 0.20, 0.18), 0, 0.34, -1.82);
  P.add('turretDark', box(0.98, 0.02, 0.38), 0, 0.63, -1.61);
  for (const s of [-1, 1]) {
    P.add('turret', box(0.30, 0.36, 0.55), s * 0.755, 0.42, -1.44);
    // plan step shoulder: the ref's -1.92 mid-step at |x| 0.54..0.62
    P.add('turret', box(0.09, 0.30, 0.20), s * 0.575, 0.42, -1.62);
  }
  // right bin-end bracket (ref plan col x 0.99 rear -1.46)
  P.add('turret', box(0.16, 0.30, 0.26), 0.98, 0.42, -1.13);
  ...
  binNose(-1.31, -1.10, 1.96, 2.46);
  P.add('turretDark', box(0.19, 0.02, 2.65), -1.205, 0.633, 0.585);
  segBoxZ(P, 'turret', 0.17, 0.44, 3.12, -1.395, 0.415, 0.11);
  P.add('turretDark', box(0.15, 0.02, 3.02), -1.395, 0.633, 0.11);
  segBoxZ(P, 'turret', 0.32, 0.44, 3.25, 1.265, 0.415, 0.425);
  binNose(1.16, 1.37, 2.05, 2.50);
  P.add('turretDark', box(0.30, 0.02, 3.15), 1.265, 0.633, 0.425);
  ...
  P.add('turretDark', box(0.19, 0.02, 1.51), -1.545, 0.549, 0.045);
  segBoxZ(P, 'turret', 0.21, 0.42, 2.00, 1.545, 0.33, 1.20);
  P.add('turretDark', box(0.19, 0.02, 1.90), 1.545, 0.549, 1.20);
  // tier end posts (the ref's 2.28 front-view tops at the ±1.5-1.65 cols)
  for (const [px, pz] of [[-1.545, -0.71], [-1.545, 0.78], [1.545, 0.25], [1.545, 2.15]]) {
    P.add('turret', box(0.21, 0.30, 0.10), px, 0.51, pz);
  }
  ...
  // across z -2.16..-1.32 — the ref's own REAL course lines (kept), but
  // narrowed to the ref's plan: the hump/mid boxes are |x|<=0.90-0.92
  // (the old 2.32 width overran the x 0.99/1.12 plan columns to -1.92
  // where the ref ends -1.43/-1.46), the tail |x|<=0.53, and the left
  // wall pulls back to the ref's -1.61/-1.72 rear line.
  P.add('turret', box(1.06, 0.33, 0.26), 0, 0.38, -1.83);
  P.add('turret', box(1.84, 0.38, 0.30), -0.01, 0.41, -1.60);
  P.add('turret', box(0.26, 0.38, 0.37), -1.29, 0.41, -1.335);
  P.add('turret', box(1.84, 0.57, 0.16), -0.01, 0.505, -1.60);
  P.add('turret', box(1.84, 0.40, 0.32), -0.01, 0.42, -1.275);
  P.add('turretDark', box(1.70, 0.02, 0.22), 0, 0.80, -1.58);
  P.add('turret', box(0.13, 0.46, 0.26), -1.485, 0.455, -1.28);
  P.add('turretCloth', box(1.7, 0.14, 0.5), -0.2, 0.51, -1.28);
  ...
  P.add('turret', box(0.30, 0.48, 0.46), -0.415, 1.085, 0.65);
  P.add('turretGlass', box(0.21, 0.05, 0.03), -0.40, 1.24, 0.91);
  P.add('turret', box(0.33, 0.49, 0.50), -0.725, 0.92, 1.25);
  // NBC pack on the left rear roof (the ref's 2.57 step at the -0.30 col)
  P.add('turret', box(0.40, 0.10, 0.36), -0.45, 0.90, -0.10);
  ...
  // basket's 2.42 silhouette band (§B3 mandatory MG — FITTINGS census).
  {
    ...
    mg.position.set(0.35, 0.46, -1.35);
    mg.rotation.y = 2.9;
    P.turretG.add(mg);
  }
  ...
  P.addGunExtra(box(0.29, 0.21, 0.48), -0.015, 0, 4.44);
  // Thermal-sleeve junction ring (the ref's 2.06-2.08 gun-top band at the
  // 2.43..2.67 columns).
  P.addGunExtra(box(0.30, 0.30, 0.24), 0, 0.06, 2.13);
...
const CENTURION_HULL = {
  ...
  // high 1.2-line shelf).
  deck: [[3.48, 1.462], [2.80, 1.483], [2.56, 1.512], [2.44, 1.693], [2.30, 1.694],
  ...
  sprocket: { z: -2.95, y: 0.99, r: 0.36 }, idler: { z: 3.30, y: 0.96, r: 0.345 },
...
function centurionBuild(P, mk) {
  ...
  for (const s of [-1, 1]) {
    for (let k = 0; k < 3; k++) {
      P.add('hull', box(0.09, 0.045, 0.42), s * 1.622, 1.435, 2.60 + k * 0.435);
    ...
    // FALLING toward the mask tip (1.40@3.68 -> 1.24@3.80 -> the thin
    // 1.19..1.13 sliver at 3.87; ref reads 1.364@3.70, 1.242@3.80,
    // 1.188..1.133@3.85). Two raked courses (NO-STAIRCASES), outboard of
    // the wrap (x 1.585+ clears the 1.5325 shoe face — containment). The
    // TIP course is the two-threshold key: its 0.06-0.17 hull-row band
    // stays under the 0.21 registration cut (dAlong holds 1.237) while its
    // whole-row union with the gun keeps the 3.81..3.93 window in the
    // hullLengthM body span (7.52 — dropping it read 7.40/-2.1%).
    const hx0 = s === 1 ? 1.585 : -1.665, hx1 = s === 1 ? 1.665 : -1.585;
    P.add('hull', slab(
      [hx0, 1.06, 3.80], [hx1, 1.06, 3.80], [hx1, 1.28, 3.68], [hx0, 1.28, 3.68],
      [hx0, 1.24, 3.80], [hx1, 1.24, 3.80], [hx1, 1.40, 3.68], [hx0, 1.40, 3.68]));
    P.add('hull', slab(
      [hx0, 1.13, 3.87], [hx1, 1.13, 3.87], [hx1, 1.06, 3.80], [hx0, 1.06, 3.80],
      [hx0, 1.19, 3.87], [hx1, 1.19, 3.87], [hx1, 1.24, 3.80], [hx0, 1.24, 3.80]));
  ...
  P.add('hull', slab(
    ...
    [-1.575, 1.618, -3.51], [1.575, 1.618, -3.51], [1.575, 1.664, -3.40], [-1.575, 1.664, -3.40]));
  P.add('hull', slab(
    [-1.575, 1.20, -3.585], [1.575, 1.20, -3.585], [1.575, 1.20, -3.51], [-1.575, 1.20, -3.51],
    [-1.575, 1.575, -3.585], [1.575, 1.575, -3.585], [1.575, 1.618, -3.51], [-1.575, 1.618, -3.51]));
  P.add('hull', slab(
    [-1.575, 1.25, -3.675], [1.575, 1.25, -3.675], [1.575, 1.21, -3.585], [-1.575, 1.21, -3.585],
    [-1.575, 1.372, -3.675], [1.575, 1.372, -3.675], [1.575, 1.575, -3.585], [-1.575, 1.575, -3.585]));
  ...
  if (mk === 5) P.add('hullDetail', box(0.40, 0.05, 0.18), 0.72, 1.532, 2.60); // periscope hump (ref 1.564 col)
  for (const s of [-1, 1]) {
    headlight(P, s * 1.05, 1.40, 3.06, -0.2);
    P.add('hullDetail', box(0.2, 0.02, 0.16), s * 1.05, 1.445, 3.00, -0.25, 0, 0);
    P.add('hullDetail', box(1.05, 0.045, 0.08), s * 0.54, 1.685, 2.32, 0, s * -0.3, 0);
    P.add('hullDetail', box(0.11, 0.1, 0.15), s * 0.82, 0.95, 3.40);
    P.add('hullDetail', torus(0.065, 0.017, 10), s * 0.82, 0.95, 3.50, Math.PI / 2, 0, 0);
  }
  KIT.towCable(P, [[-1.0, 1.70, 2.35], [0, 1.672, 1.5], [1.0, 1.70, 2.35]]);
  P.add('hullDetail', box(0.1, 0.05, 0.14), -1.0, 1.685, 2.35);
  P.add('hullDetail', box(0.1, 0.05, 0.14), 1.0, 1.685, 2.35);
  ...
  // r5: flaps held inside x 1.663 — at 1.625+-0.06 they lit the ±1.69
  // front columns to 1.58 where the ref keeps the bare outer strip (1.30).
  for (const s of [-1, 1]) {
    P.add('hullRubber', box(0.10, 0.32, 0.03), s * 1.613, 1.42, -3.12, 0.05, 0, 0);
  }
  ...
  P.add('hullDark', box(1.70, 0.11, 0.03), 0, 1.312, -3.68);
  ...
  P.add('turret', slab(                       // nose plate -> forward cheeks
    ...
    [-0.66, -0.23, 1.46], [0.66, -0.23, 1.46], [1.02, -0.29, 1.04], [-1.02, -0.29, 1.04],
  ...
  P.add('turret', box(0.92, 0.11, 0.435), -0.49, 0.912, -0.7025);
  // ---- bustle: mark-specific roofline lofts (tables in local coords) ----
  if (mk === 5) {
    loftBand(P, 'turret', 0.95, 0.04, [
      // r5: crest raised to the mask-true 2.61-2.62 line (probe read the
      // r2 crest 0.02-0.04 shy over build -0.90..-1.20).
      [-0.92, 0.83], [-1.01, 0.71], [-1.07, 0.71], [-1.10, 0.79], [-1.53, 0.835],
      [-1.56, 0.72], [-1.645, 0.64], [-1.667, 0.607], [-2.02, 0.60],
    ...
    // r5: walls z-SPLIT — the ref keeps them at the 2.48 roof DIP through
    // the dip zone (side read) but raises them to the 2.60 crest through
    // local -1.16..-1.57, where they also own the FRONT-view 2.58-2.61
    // tops at |x| 1.05..1.25 (first cut lowered the whole wall and cost
    // front_whole 9 pts).
    P.add('turret', box(0.30, 0.70, 0.24), -1.10, 0.35, -1.04);
    P.add('turret', box(0.26, 0.70, 0.24), 1.08, 0.35, -1.04);
    P.add('turret', box(0.30, 0.82, 0.41), -1.10, 0.41, -1.365);
    P.add('turret', box(0.26, 0.82, 0.41), 1.08, 0.41, -1.365);
    for (const s of [-1, 1]) {
    ...
    loftBand(P, 'turret', 0.95, 0.04, [
      [-0.92, 0.755], [-1.01, 0.708], [-1.31, 0.708], [-1.40, 0.728], [-1.477, 0.747],
      [-1.647, 0.637], [-1.75, 0.545],
    ], (z) => lineAt([[-0.92, -0.28], [-1.02, -0.248], [-1.09, 0.011], [-1.42, 0.011],
      [-1.647, 0.042], [-1.75, 0.07]], z), -0.92, -1.75);
    P.add('turret', box(0.10, 0.68, 0.37), -0.99, 0.34, -1.085);
    P.add('turret', box(0.10, 0.55, 0.37), 0.965, 0.275, -1.085);
  ...
  P.add('turret', cylY(0.215, 0.235, 0.13, 16), -0.45, 0.85, -0.30);
  cupola(P, 'turret', -0.45, 0.87, -0.30, 0.195, 0.145, 6);
  P.add('turretDark', torus(0.17, 0.016, 16), -0.45, 0.97, -0.30);
  P.add('turret', box(0.06, 0.11, 0.40), -0.45, 1.085, -0.22);
  ...
  P.add('turret', box(0.24, 0.10, 0.30), 0.44, 0.74, 0.29);
  P.add('turretGlass', box(0.16, 0.05, 0.03), 0.44, 0.765, 0.45);
  periscope(P, 'turretDetail', 0.30, mk === 5 ? 0.63 : 0.565, 0.67);
  ...
  // censuses zero). Stowed-low aft pose keeps the receiver under the ref's
  // 2.75 crown plateau and the barrel crossing the bustle lids (MG PHYSICS
  // dark crown-riding polarity). §H4 variety: M2 fifty on the Mk.5/2,
  // MAG on the Mk.3.
  {
    const mg = FITTINGS.pintleMG({ mats: P.mats, cls: mk === 5 ? 'm2' : 'mag', tone: 'two-tone', elev: 0, scale: 0.8, seed: mk === 5 ? 5 : 9 });
  ...
  // Smoke discharger banks on the shelf front faces. r5: the Mk.5/2's tall
  // 2x6 bank reads as a PROUD BIN riding the shelf lip (ref side bump 2.51
  // over build 1.03..1.15, ref FRONT tops 2.58 only to |x| 1.30) — the bin
  // overlaps the shelf top by 20 mm (floater law; a raised free tube row
  // floated off the shelf and cost the pose gate). Mk.3 keeps the low
  // triple; cluster tubes stay at the r2 seat.
  for (const s of [-1, 1]) {
    P.add('turretDetail', box(0.05, 0.12, 0.10), s * 1.28, 0.28, 0.80, 0, s * 0.4, 0);
    smokeCluster(P, s * 1.22, 0.38, 0.86, mk === 5 ? 6 : 3, s * 0.95, 0.7);
    if (mk === 5) P.add('turretDetail', box(0.14, 0.30, 0.24), s * (s > 0 ? 1.13 : 1.17), 0.55, 0.76, 0, s * 0.2, 0);
  }
  ...
  P.add('turretCloth', box(0.42, 0.24, 0.34), 0.23, 0.16, 1.63, -0.25, 0, 0);
  P.add('turretCloth', box(0.30, 0.16, 0.22), 0.24, 0.09, 1.78, -0.1, 0, 0);
  const gunLen = 5.15;
  if (mk === 5) {
    ...
    // r5: the print's L7 extractor drum is TOP-BIASED (band 2.097..1.758 —
    // drum axis rides 22 mm over the bore line): an axis-centered buildGun
    // drum can't match both edges, so the drum is authored offset.
    P.add('gun', cylZ(0.171, 0.36, 12), 0, 0.022, 2.42);
    P.add('gun', cylZ(0.171, 0.17, 12, 0.145), 0, 0.022, 2.155);
    P.add('gun', cylZ(0.145, 0.17, 12, 0.171), 0, 0.022, 2.685);

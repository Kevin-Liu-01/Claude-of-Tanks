...
function verifyRearGuards(root) {
  for(const side of [-1,1]){
    const hit=firstHit(root,[side*1.4409,1.3,-2.30],[0,1,0]);
  ...
  for(const quality of ['high','low']){
    ...
    try{
      ...
      for(const[z,sourceY,tolerance]of[[0,.570706,.002],[2.3,.566126,.002],[3.3,1.213680,.005]]){
        const hit=firstHit(root,[0,0,z],[0,1,0]);
      ...
      for(const[y,sourceZ]of[[.7,-2.22809],[1.2,-2.26826],[1.9,-2.32455]]){
        ...
        assert(hit&&Math.abs(hit.point.z-sourceZ)<.004,'source rear door plane, whole visible scene');checks++;
      }
      assert.equal(firstHit(root,[0,.6,-2.8],[0,1,0],1.4),undefined,'central space behind the recessed ramp stays open');checks++;
      for(const side of[-1,1]){
        const hit=firstHit(root,[side*1.4,0,-2.8],[0,1,0]);
      ...
      for(const name of['gun','gunDark']){
        ...
        assert(bounds.max.z<3.673,'barrel rings must not extend beyond the source muzzle');checks++;
      }
      for(const[x,y]of[[.004,.006],[-.006,.004]]){
        const hit=firstHit(root,[x,2.6+y,3.9],[0,0,-1]);

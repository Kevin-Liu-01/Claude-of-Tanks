...
function buildType90(P) {
  ...
  m2.position.set(0.82, 0.49, 0.02);                                           // reseated on the corrected commander cupola; fitting retains its authored scale
  P.turretG.add(m2);
  ...
  for (const s of [-1, 1]) {
    ...
    smoke.position.set(s * 1.04, 0.30, -0.42);                                 // 2x3 dischargers follow the corrected half-height bustle flank
    P.turretG.add(smoke);
  ...
  for (const s of [-1, 1]) {
    ...
    whipA.position.set(s * 1.11, 0.22, -1.72);                                 // antenna collars remain seated after the turret-height correction
  ...
  // Owner correction (2026-08-12): the live garage silhouette showed the
  // turret body at roughly twice the proper Type 90 section.  Compress every
  // repository-authored turret surface and roof fitting in local Y while
  // leaving X/Z plan, the circular gun geometry, and the articulated gun rig
  // untouched.  Direct child fittings above were explicitly re-seated at the
  // corresponding half-height roots, so yaw cannot reveal a floating bank,
  // MG, or antenna collar.
  P.scaleBuckets(
    ['turret', 'turretDark', 'turretDetail', 'turretGlass', 'turretCloth'],
    1, 0.50, 1,
  );
  // Rebuild the functional roof cadence at its real scale on the new
  // 0.47-local roof.  These are added after the body-section compression so
  // the cupolas, sight heads, periscopes and MG cradle do not become
  // vertically squashed decorations.
  P.add('turret', cylY(0.225, 0.235, 0.065, 16), 0.56, 0.505, -0.17);
  P.add('turretDark', torus(0.215, 0.011, 16), 0.56, 0.535, -0.17);
  P.add('turret', cylY(0.19, 0.205, 0.035, 16), 0.56, 0.555, -0.17);
  P.add('turret', cylY(0.20, 0.205, 0.055, 16), -0.52, 0.505, -0.11);
  P.add('turretDark', torus(0.19, 0.010, 16), -0.52, 0.535, -0.11);
  for (let k = 0; k < 5; k++) {
    const a = (125 + k * 27) * Math.PI / 180;
    P.add('turretGlass', box(0.060, 0.070, 0.045),
      0.56 + Math.cos(a) * 0.205, 0.555, -0.17 + Math.sin(a) * 0.205, 0, -a, 0);
  }
  P.add('turret', frustum(0.18, 0.34, -0.18, 0.145, 0.30, -0.15, 0.49, 0.70), 0.10, 0, 0.18);
  P.add('turretDark', box(0.25, 0.11, 0.025), 0.10, 0.60, 0.515);
  P.add('turretGlass', box(0.17, 0.065, 0.015), 0.10, 0.60, 0.530);
  P.add('turret', box(0.36, 0.14, 0.44), 0.30, 0.545, 0.90);
  P.add('turretDark', box(0.27, 0.075, 0.025), 0.30, 0.555, 1.135);
  P.add('turretGlass', box(0.19, 0.045, 0.015), 0.30, 0.555, 1.150);
  P.add('turretDetail', box(0.40, 0.045, 0.26), 0.70, 0.485, 0.02);            // broad MG swing-mount seat
  for (const [x, z, ry] of [[-0.30, 0.16, 0.25], [-0.56, 0.22, -0.12], [0.35, 0.34, 0.18]]) {
    periscope(P, 'turretDetail', x, 0.505, z, ry);
  }
  P.topY = 0.72;
}
...
function buildAMX30(P, b2) {
  ...
  const roadWheelR = 0.38;
  const roadWheelY = 0.42;
  buildRunningGear(P, {
    ...
    trackW: 0.55, trackTh: 0.07, topY: 1.02, botY: b2 ? 0.02 : 0.052,
  ...
  const lowerCrownScale = b2 ? 0.88 : 0.89;
  const upperCrownScale = b2 ? 0.56 : 0.57;
  P.add('turret', KIT.polyTurret(PLAN, 0.50, 1.10, lowerCrownScale), 0, 0, 0);   // lower cast band (flared skirt -> clipped shoulder)
  P.add('turret', KIT.polyTurret(PLAN.map(([x, z]) => [x * 0.86, z * 0.965]), 0.36, 1.0, upperCrownScale), 0, 0.50, 0); // upper cast dome band
  ...
  const stationZ = -0.26;
  const stationHeadZ = stationZ;
  const stationHeadDepth = 0.80;
  P.add('turret', cylY(0.30, 0.32, 0.12, 14), 0.52 * stationSign, 0.85, commanderZ);           // cupola ring
  P.add('turret', cylY(0.24, 0.28, 0.07, 14), 0.52 * stationSign, 0.895, commanderZ);          // cupola dome band
  P.add('turret', cylY(0.10, 0.20, 0.04, 14), 0.52 * stationSign, 0.91, commanderZ);           // dome crown
  // TOP-7 commander/sight station: broad overlap into the cupola ring, a
  // tapered armored head, and a compact optical crown.  These authored parts
  // supply the real vehicle's height without distorting the cast shell.
  P.add('turret', frustum(0.42, 0.10, -0.86, 0.36, 0.04, -0.80, 0.68, 0.94),
    0.72 * stationSign, 0, 0);                                                 // broad armored cupola foundation
  for (const x of (b2 ? [0.41, 1.03] : [0.45, 0.99])) {
    P.add('turret', box(0.13, 0.44, stationHeadDepth), x * stationSign, 1.15, stationHeadZ); // open TOP-7 side frames
  }
  P.add('turret', box(b2 ? 0.78 : 0.68, 0.10, stationHeadDepth), 0.72 * stationSign, 1.36, stationHeadZ); // framed optical bridge
  P.add('turretDark', box(0.46, 0.24, 0.025), 0.72 * stationSign, 1.16, stationHeadZ + stationHeadDepth * 0.515);
  const stationMastTop = (b2 ? 1.68 : 1.43) / turretYScale;
  const stationCrownY = (b2 ? 1.70 : 1.50) / turretYScale;
  P.add('turret', box(0.11, stationMastTop - 1.16, 0.11), 0.68 * stationSign,
    (1.16 + stationMastTop) / 2, stationHeadZ);
  P.add('turret', cylY(0.105, 0.125, 0.10, 12), 0.68 * stationSign, stationCrownY, stationHeadZ);
  P.add('turretGlass', box(0.10, 0.07, 0.025), 0.68 * stationSign, stationCrownY, stationHeadZ + 0.155);
  for (let k = 0; k < 8; k++) {
  ...
  {
    const mg = FITTINGS.pintleMG({ mats: P.mats, cls: 'mag', tone: 'dark', elev: -0.04, seed: 8, scale: b2 ? 0.95 : 0.85, ammo: true });
    ...
    mg.position.set(0.84 * stationSign, 1.30, stationZ + 0.08);
    P.turretG.add(mg);
  }
  ...
  P.gunG.position.set(0, b2 ? 0.425 : 0.366, 0.75);
  ...
  const gunRadius = b2 ? 0.122 : 0.120;

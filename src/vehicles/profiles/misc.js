...
function buildLeclerc(P) {
  ...
  for (const s of [-1, 1]) {
    ...
    {
      // FRANCE ROUND re-attribution: the B2 scan NOW EXCLUDES /shadow/i
      // meshes (tool drift — the baseline bytes read the same 4 open cells
      // and no shadow-named cover moves them), so the slot floor is a REAL
      // hullDark panel: wall-to-wall x 1.624..1.700 (1 mm off the pad
      // plane; --exact counts inside-solid voxels only — clip stays at the
  ...
  P.add('turret', slab(                                                        // bustle tail LEFT of the center notch (photo round: inner face at
    ...
    [-1.20, 0.615, -1.72], [-0.12, 0.615, -1.72], [-0.12, 0.578, -2.16], [-1.06, 0.578, -2.23])); // x -0.12 — the old -0.02 wall put z_w -2.26 in the ref col -0.042 window (-2.09)
  ...
  P.add('turret', box(0.54, 0.11, 0.62), 0.72, 0.665, 0.47);                   // housing base (top 2.32w, z_w 0.06..0.68 — the ref's own band)
  ...
  for (const s of [-1, 1]) {
    P.add('turret', slab(
      ...
      [s * 0.98, LH, -0.02], [s * 1.36, 0.615, -0.06], [s * 1.36, 0.615, -1.30], [s * 0.98, LH, -1.30]));
  }
  ...
  for (const s of [-1, 1]) {
    ...
    P.add('turret', slab(
      ...
      [s * 1.40, s < 0 ? 0.62 : 0.53, 0.30], [s * 1.44, s < 0 ? 0.62 : 0.53, 0.30], [s * 1.44, s < 0 ? 0.62 : 0.53, -0.45], [s * 1.40, s < 0 ? 0.62 : 0.53, -0.45]));
  ...
  P.add('turret', box(0.40, 0.049, 0.40), 0.55, 0.8615, 1.00);                 // head block (top 0.886)
  P.add('turret', box(0.41, 0.040, 0.41), 0.55, 0.900, 1.00);                  // lid CAP, top 0.920 = 2.52w — THE heightM p95 anchor (loop-1 measure: at 2.492 the p95 fell to 2.48, dims 100 -> 92.7; the 2.52 x4-col anchor is the ratified baseline trade: +0.02 x11 front / +0.028 x4 side cols vs the ref's 2.501/2.492 band, dims-sovereign)
  ...
  P.add('turretDark', box(0.014, 0.135, 0.014), 0.55, 0.8195, 0.793);          // rear cable conduit (flush on the head rear face — z_w 0.686 stays inside the col-0.736 window the head already owns)
...
function buildType90(P) {
  ...
  for (const s of [-1, 1]) {
    ...
    if (s > 0) {
      ...
      P.add('hullDetail', box(0.028, 0.22, 0.04), 1.700, 0.98, 3.40, 0.2, 0, 0); // folded mirror arm
      P.add('hullDark', box(0.042, 0.11, 0.16), 1.6925, 1.06, 3.33);           // mirror head (folded flat)
    }
  ...
  for (const s of [-1, 1]) {
    for (const [zc, d] of [[2.7675, 0.385], [3.04, 0.16]]) {                   // FRONT armored panels z 2.575..3.12 (r5 station re-read: ref width
      P.add('hull', box(0.06, 0.582, d), s * 1.648, 0.899, zc);                // steps to 3.39-3.42 only over i12-13 [z 2.54+] — panels starting 2.52
    }                                                                          // still poked i11 [5.2 wPct]; outer faces 1.678: the ref's outer
  ...
  buildRunningGear(P, {
    ...
    style: 'rubber', wheelR: 0.37, wheelW: 0.21, wheelY: 0.47, xc: 1.290,
    wheelZs,
    ...
    trackW: 0.59, topY: 0.92, botY: 0.055, contactZF: 2.18, contactZR: -2.26, paintedEnds: true, coveredTop: true, arms: true,
  });
  ...
  wheelRecessAt(P, wheelZs, 1.290, 0.47, 0.37, 0.21);
  tightenHullShadowProxy(P, { xc: 1.29, trackW: 0.36, y0: 0.15, y1: 0.60, z0: -2.35, z1: 2.25, hullZ0: -3.10, hullZ1: 3.10 });
  ...
  const PLAN_LO = [
    ...
    [1.19, 0.88], [1.19, -1.127], [1.18, -2.11], [-1.18, -2.11],
    [-1.26, -1.127], [-1.26, 1.13], [-1.20, 1.17], [-1.08, 1.68], [-0.93, 1.82], [-0.80, 1.90],
  ];
  const PLAN_HI = [
    [-0.55, 1.98], [0.55, 1.98], [0.85, 1.88], [1.00, 1.76], [1.11, 1.30],
    [1.21, 0.90], [1.21, -1.127], [1.18, -2.11], [-1.18, -2.11],
    [-1.30, -1.127], [-1.30, 1.19], [-1.20, 1.19], [-1.08, 1.70], [-0.93, 1.84], [-0.80, 1.92],
  ];
  P.add('turret', KIT.polyTurret(PLAN_LO, 0.34, 1.0, 1.0), 0, 0.03, 0);        // lower band: walls to 1.77 (the prow/mantlet height). r6: base RAISED 3 cm — the ref turret underside reads 1.423 fore -> 1.446 aft across z_w -0.45..-1.3 where the 1.40 wall base printed +0.02-0.03 on ~14 side cols (ariete r8 rising-underside class); the turret AABB bottom stays the gun roll at 1.402 so the turretRows camera never re-frames (t80u law)
  P.add('turret', KIT.polyTurret(PLAN_HI, 0.08, 1.0, 1.0), 0, 0.37, 0);        // wall extension band to 1.85 (ref front-view 1.852 at -1.33 / 1.852 at +1.22; the plan R wall caps at 1.21)
  P.add('turret', box(0.995, 0.29, 1.795), 0.3825, 0.515, 0.5775);             // roof core R piece x -0.115..0.88 (top 2.06, z_w 1.275..-0.52 — r5: the 1.34 front lit the ref's 1.806-want col at 2.06)
  P.add('turret', box(0.665, 0.29, 1.795), -0.5475, 0.515, 0.5775);            // roof core L piece x -0.88..-0.215
  P.add('turret', box(0.10, 0.12, 1.795), -0.165, 0.43, 0.5775);               // roof CHANNEL floor 1.89 x -0.215..-0.115 (r6 worldtrace: the ref front tops read 1.892 in a narrow valley at x -0.13..-0.21 between its center pintle mount and the sight ridge — the flat 2.06 core printed +0.17 on 3 front cols; plan footprint unchanged, contiguity held by the floor)
  for (const s of [-1, 1]) {
    P.add('turret', slab(                                                      // roof edge: steep 2.06@0.88 -> 1.87@0.98 -> 1.82 wall tops
      [s * 0.88, 0.42, 1.42], [s * (s > 0 ? 1.21 : 1.30), 0.42, 1.02], [s * (s > 0 ? 1.21 : 1.30), 0.42, -0.32], [s * 0.88, 0.42, -0.32],
      [s * 0.88, RY, 1.42], [s * 0.98, 0.46, 1.02], [s * 0.98, 0.46, -0.32], [s * 0.88, RY, -0.32]));
  }
  P.add('turret', frustum(0.88, 1.475, 0.28, 0.86, 1.42, 0.30, 0.37, 0.64));   // front roof wedge closing the core to the prow line (ends z_w 1.275 with the core)
  P.add('turret', box(0.995, 0.23, 0.24), 0.3825, 0.485, -0.20);               // hatch-zone plate R (top 2.00, z_w -0.28..-0.52)
  P.add('turret', box(0.665, 0.23, 0.24), -0.5475, 0.485, -0.20);              // hatch-zone plate L
  P.add('turret', box(0.10, 0.115, 0.24), -0.165, 0.4275, -0.20);              // hatch-plate CHANNEL floor 1.885 (r6b: the ref's x -0.13..-0.21 front valley reads 1.89 — the 2.00 plate re-topped the channel cols the core cut opened)
  P.add('turret', box(2.36, 0.115, 1.61), 0, 0.4275, -1.305);                  // bustle roof band (top 1.885, z_w -0.70..-2.31)
  // hatches: commander RIGHT / loader LEFT — RAISED ring + domed lid (r5:
  // the fresh ref rows read 2.05-2.08 over z_w -0.46..-0.65 where the flat
  // 1.99 rims sat -0.09; ariete turret-fix crown class)
  P.add('turret', cylY(0.225, 0.225, 0.09, 14), 0.56, 0.575, -0.17);           // (r5b: ring aft to -0.17 — the ref hatch band runs to z_w -0.65 where the -0.11 ring's circle ended -0.535, leaving the -0.586 col 0.07 low)
  P.add('turretDark', torus(0.22, 0.014, 14), 0.56, 0.628, -0.17);
  P.add('turret', cylY(0.195, 0.21, 0.035, 14), 0.56, 0.645, -0.17);           // domed lid (crown 2.0625)
  P.add('turretDark', box(0.36, 0.012, 0.03), 0.56, 0.669, -0.17);
  P.add('turret', cylY(0.20, 0.20, 0.062, 14), -0.52, 0.516, -0.11);
  P.add('turretDark', cylY(0.205, 0.205, 0.012, 14), -0.52, 0.555, -0.11);
  periscope(P, 'turretDetail', 0.56, 0.545, 0.16);
  ...
  // box fwd + commander sight tower aft to z_w -0.17. r5 re-meter: tower/
  // lid/step RIGHT edges pulled 0.32->0.24-0.28 (ref front 2.18-line starts
  // x 0.256), housing 0.50->0.445 + front seated z_w 1.29 (robust in the
  // 2.202-want col; the 1.31 face was a coin-flip pixel), lid 2.25.
  P.add('turret', box(0.345, 0.13, 0.69), 0.2725, 0.725, 1.135);               // gunner sight housing (top 2.19, z_w 0.59..1.28)
  P.add('turretDark', box(0.30, 0.09, 0.03), 0.27, 0.72, 1.495);
  P.add('turretGlass', box(0.22, 0.06, 0.02), 0.27, 0.72, 1.51);
  sightBox(P, 'turret', 0.10, 0.74, 0.39, 0.36, 0.16, 0.80);                   // commander sight tower body (z_w -0.21..0.59 — r6: rear pulled off the [-0.392,-0.267] trace col; the -0.27 face half-phase-lerped the ref's -0.33 col [want 2.028] to 2.19)
  P.add('turret', box(0.36, 0.04, 0.84), 0.10, 0.81, 0.39);                    // tower lid (top 2.25; rear -0.23 with the body)
  P.add('turret', box(0.215, 0.045, 0.42), 0.1175, 0.9075, 0.30);              // rear lid step: heightM p95 anchor (top 2.33, ~4 body cols; ref reads 2.23-2.26 here — the +0.07 buys dims grace, priced vs every relocation. r6: z_w -0.11..0.31 — the old -0.31..0.11 seat put the 2.33 crown in station i6 [slab ends -0.145] for topPct 3.63 vs the ref 2.257 lid line; in i7 it rides a slab that already carries the same trimmed-tier error, i6 drops to ~0.05. r6b: x 0.01..0.225 — the -0.06/0.24 edges lit the -0.096 and 0.251 front cols [wants 2.19/2.18] at +0.14; the p95 anchor is z-driven, x-trim free)
  P.add('turretDark', box(0.18, 0.045, 0.20), -0.02, 0.83, 0.82);              // pano head (top 2.2525 — the r4 "2.315 ref spike" read is stale post-amendment: the fresh ridge want is 2.23-2.26)
  P.add('turretGlass', box(0.14, 0.05, 0.016), -0.02, 0.825, 0.93);
  ...
  m2.position.set(0.05, 0.52, 0.48);                                           // envelope z_w -0.16..0.84; receiver/barrel band ~2.20-2.22 (r5: the fresh ref ridge line is 2.202 across z_w 0.75-1.24 — the 0.56 foot read +0.05 on four cols)
  P.turretG.add(m2);
  ...
  for (const s of [-1, 1]) {
    ...
    P.add('turret', box(xOut - xIn, 0.10, 0.609), s * xc2, 0.39, -0.7915);     // shelf body (top 1.84, z_w -0.687..-1.296)
    P.add('turretDark', box(xOut - xIn - 0.02, 0.02, 0.57), s * xc2, 0.325, -0.79); // under-lip shadow
    P.add('turretDetail', box(xOut - xIn - 0.04, 0.02, 0.045), s * xc2, 0.45, -0.55); // lid ribs
    P.add('turretDetail', box(xOut - xIn - 0.04, 0.02, 0.045), s * xc2, 0.45, -1.01);
    const smoke = FITTINGS.smokeBank({ mats: P.mats, count: 3, r: 0.040, len: 0.20, splay: s * 0.75, pitch: -0.45, seed: 3 + s });
    smoke.position.set(s * 1.04, 0.28, -0.42);                                 // tips <=1.22 (r5: the plan ±1.294 col is the SHELF SLIVER's — 1.29 smoke tips ahead of the sliver's z-window would junk its front; the ref's own dischargers print nothing there)
    P.turretG.add(smoke);
    liftEye(P, 'turretDetail', s * 0.80, 0.58, 0.75, s * 0.4);                 // (r5: the RY+0.01 seat topped 2.13 in the +-0.78-0.87 front cols where the ref reads 2.00-2.04)
  }
  ...
  {
    const zf = -1.83, zr = -2.18;                                              // local (world -2.03/-2.38; r5: floor rail to the ref's own plan rear -2.453; r5b: the TOP rail/posts at -2.41 z_w poked the -2.413 col boundary and printed 1.91 into the ref's 1.46-want deck col [err 0.167] — upper basket pulled to -2.40 faces)
    P.add('turretDetail', box(2.39, 0.04, 0.04), 0, 0.28, -2.235);             // floor rail rear RAISED to 1.66-1.70 (r6: the ref basket floor RISES aft — its -2.40 col reads [1.66..1.79] when its thin mast rolls unlit; the 1.475 rail paid 0.28 there on unlit rolls. underside lip to z_w -2.455; r5b width ±1.195: the plan ±1.20 col wants the full -2.455 rear — 2px clear of the 1.201 front-col boundary)
    P.add('turretDetail', box(2.39, 0.04, 0.04), 0, 0.075, zf);                // floor rail front
    P.add('turretDetail', box(2.39, 0.04, 0.04), 0, 0.49, zr);                 // top rail 1.90
    for (let i = 0; i < 7; i++) {
      const x = -1.11 + i * 0.37;
      P.add('turretDetail', box(0.03, 0.40, 0.03), x, 0.29, -2.13);            // rear posts (r6: z_w -2.33 — off the [-2.34,-2.46] flicker col)
    }
    P.add('turretDark', box(2.35, 0.34, 0.014), 0, 0.30, -2.12);               // mesh back (r6: z_w -2.32, same pull)
    P.add('turretCloth', box(2.06, 0.40, 0.36), 0, 0.29, -2.02);               // strapped cargo fill (top 1.89)
    P.add('turretCloth', box(1.00, 0.085, 0.26), -0.10, 0.535, -1.70);         // tarp roll (top 1.98 = the ref -1.90 col)
    P.add('turretDark', box(0.02, 0.40, 0.36), -0.62, 0.29, -2.02);            // cinch straps
    P.add('turretDark', box(0.02, 0.40, 0.36), 0.55, 0.29, -2.02);
    // NARROW center top frame (r5: ref side 2.263-2.293 band z_w -2.05..
    // -2.30 — the r4 -2.01 front edge bled the -1.987 col and smeared the
    // ref's 1.989-want col +0.15; front pulled to -2.10, raised 2.3325 as
    // the heightM p95 partner)
    for (const sx of [-1, 1]) {
      P.add('turretDetail', box(0.035, 0.035, 0.30), sx * 0.08, 0.915, -1.95); // (r6: bar z_w -2.00..-2.30 — the ref 2.31 side band starts at its -2.037 col where the old -2.10 front edge left the col reading the 2.17 lerp, err 0.09)
      P.add('turretDetail', box(0.03, 0.46, 0.03), sx * 0.08, 0.68, -1.92);
      P.add('turretDetail', box(0.03, 0.46, 0.03), sx * 0.08, 0.68, -2.08);
    }
  }
  ...
  for (const s of [-1, 1]) {
    const whipA = FITTINGS.antennaWhip({ mats: P.mats, h: s > 0 ? 0.68 : 0.48, r: s > 0 ? 0.018 : 0.012, rake: -s * 0.02, seed: 2, rotation: [s > 0 ? -0.78 : -0.60, 0, 0] });        // (r6b: rake 0.05 -> 0.02 — the fat R tip at x 1.144±0.018 leaked into the 1.178 front col [want 1.934] for 0.246 and left the ref's 1.1 mast col reading the shaft mid)
    whipA.position.set(s * 1.11, 0.44, -1.72);                                 // r6 ASYMMETRIC MASTS (fresh front rows: ref mast cols L 2.232 / R 2.395; the old symmetric 0.60 whips paid 0.107 L): L h 0.48 tip (2.236, z_w -2.19); R h 0.68 rot -0.78 TRUE tip (2.386, z_w -2.398 — shaft rides a 0.06 pot, tip = pot + h, measured on the r2 dump where the h 0.74 tip read 2.449 @ -2.47 and spilled 0.52+0.28 err into the -2.52/-2.64 deck cols) — one rig satisfies BOTH the front R col want 2.395 AND the ref's side -2.40 mast-cliff col (want 2.39). L tip stations in i3 under the 2.3325 frame
    P.turretG.add(whipA);
  }
  ...
  P.addGunExtra(box(0.32, 0.29, 0.26), 0, 0.003, 1.585);                       // mantlet block: x +-0.16, y 1.42..1.71 w, face z_w 2.065 (print station)
  P.addGunExtra(box(0.30, 0.27, 0.025), 0, 0.003, 1.735);                      // the FLAT FACE plate (proud 2 cm, the Type 90 tell)
  P.addGunExtraDark(cylZ(0.088, 0.03, 12), 0, 0.003, 1.76);                    // tube exit collar on the face
  for (const bs of [-1, 1]) {
    P.addGunExtraDark(box(0.014, 0.24, 0.018), bs * 0.148, 0.003, 1.752);      // bolted edge strips on the face
    P.addGunExtra(box(0.05, 0.24, 0.30), bs * 0.185, 0.003, 1.30);             // trunnion cheek taper closing block -> prow (§B2 connected)
  }
  P.addGunExtraDark(box(0.30, 0.014, 0.24), 0, -0.142, 1.55);                  // block underside shadow line
  ...
  P.addGunExtra(cylZ(0.135, 0.48, 14), 0, 0, 1.72);                            // REAR DRUM (ref tops 1.697-1.715, z_w 1.83-2.31; band 0.27 under the ~0.29 12% cut — the r 0.15 exact match would CROSS it, landmine)
  P.addGunExtraDark(cylZ(0.137, 0.05, 14), 0, 0, 1.51);                        // drum seam rings
  P.addGunExtraDark(cylZ(0.137, 0.05, 14), 0, 0, 1.93);
  P.addGunExtra(box(0.37, 0.05, 0.24), 0.035, 0, 5.35);                        // off-axis MRS housing PLATE x -0.15..0.22 (z_w 5.58-5.82: covers the ref's 0.198 plan tube col [reach 0.22, front want 5.751]. r6: the r5 cylZ r0.10 collar printed side band [1.47..1.65] where the ref muzzle band is [1.514..1.617] — a flat y±0.025 plate keeps the plan reach at zero side-band cost; the first -0.17 left edge lit the -0.183 plan col [want front 3.427] at err 1.20 — 15 mm margin now; §B3: the MRS mirror housing rides flat on the muzzle)
...
function buildType74(P) {
  ...
  wheelRecessAt(P, wheelZs, XC, 0.475, 0.42, 0.26);
  ...
  P.add('turret', lathe([
    [1.04, 0.0], [1.13, 0.10], [1.09, 0.30], [0.98, 0.50], [0.82, 0.62],
    [0.64, 0.70], [0.42, 0.78], [0.02, 0.85],
  ], P.q ? 30 : 16, 1.45), 0, 0.0, -0.45);                                     // cast dome: crown 2.28 world (photo class: real dome 2.0-2.13 + cupola;
                                                                               // print 2.40-2.47 — documented ~0.15 cap), shoulders fall to 2.10/2.16
                                                                               // (the ref's own 2.03-2.13 front-view shoulder band)
  ...
  cupola(P, 'turret', 0.40, 0.74, -0.46, 0.20, 0.16, 6);                       // lid ~2.34
  const m2 = FITTINGS.pintleMG({ mats: P.mats, cls: 'm2', tone: 'two-tone', seed: 7 });
  m2.position.set(0.44, 0.74, -0.48);                                          // receiver+sight top ~2.535: the heightM-2.48 p95 anchor (scale-1.0 frame)
  P.turretG.add(m2);
  P.add('turret', cylY(0.19, 0.20, 0.05, 14), -0.44, 0.715, -0.30);            // loader ring on the left shoulder
  P.add('turret', cylY(0.165, 0.165, 0.028, 14), -0.44, 0.765, -0.30);         // oval lid ~2.20 world
  P.add('turretDark', box(0.31, 0.013, 0.03), -0.44, 0.782, -0.30);            // lid seam
  periscope(P, 'turretDetail', 0.24, 0.80, 0.28);                              // gunner periscope fwd-right
  periscope(P, 'turretDetail', -0.44, 0.795, -0.02, 0.3);                      // loader periscope
  ...
  for (const s of [-1, 1]) {
    ...
    // §I fittings: whip antenna on the bustle flank (ref spikes x ±0.94,
    // body-rel -0.93 tips 2.59 — ours tip ~2.56, 1-col) + 3-tube smoke
    // discharger bank on the dome rear quarter (real 2x3 JGSDF fit)
    const whipF = FITTINGS.antennaWhip({ mats: P.mats, h: 0.52, rake: s * 0.05, seed: 3 + s });
  ...
  // (§B3.1 mantlet mass), bare rifled tube, fat fume extractor at the ref's
  // own +3.43..+3.72 station, muzzle step + §B3.1 bore at +5.96 (overall
  // 9.42-9.47 from the -3.44 sprocket-shoe extreme)
  ...
  buildGun(P, { len: 4.31, r: 0.062, sleeve: false, evac: 0.445, evacR: 1.75, collar: false, baseR: 0.15 });
  P.add('gun', cylZ(0.068, 0.09, 10), 0, 0, 4.25);                             // muzzle reference step
  muzzleBore(P, 0.062, 4.295);                                                 // §B3.1 muzzle bore inside the step face (shadow-named)
  P.topY = 1.15;
}
...
function buildAMX30(P, b2) {
  ...
  P.add('hull', box(1.96, 0.70, 4.90), 0, 0.71, 0);                            // tub x +-0.98, belly 0.36, z -2.45..2.45
  P.add('hull', box(1.88, 0.70, 0.50), 0, 0.71, -2.70);                        // tail tub segment x +-0.94 (§B4: the +-0.98 corner kissed the dilated sprocket wraps — 44 rear voxels + 6 shoe)
  P.add('hull', frustum(1.51, 1.94, -3.28, 1.49, 1.90, -3.26, 1.00, 1.36));    // sponson band over the tracks (full 3.02 width, y 1.00..1.36)
  P.add('hull', box(2.30, 0.05, 2.30), 0, 1.355, 0.45);                       // fore deck 1.38 (z -0.70..1.60)
  P.add('hull', box(2.30, 0.05, 2.55), 0, 1.315, -1.98);                      // engine deck 1.34 (slopes to the tail below)
  ...
  for (const s of [-1, 1]) {
    P.add('hull', slab(                                                        // casting blend: glacis edge -> sponson line (above the wraps)
      [s * 1.51, 1.05, 1.60], [s * 1.10, 1.05, 2.35], [s * 1.10, 1.05, 2.62], [s * 1.51, 1.05, 1.90],
      [s * 1.51, 1.372, 1.58], [s * 1.10, 1.372, 1.60], [s * 1.10, 1.10, 2.60], [s * 1.51, 1.10, 1.88]));
  }
  ...
  for (const s of [-1, 1]) {
    ...
    P.add('hullDark', cylZ(0.148, 0.06, 12), s * 1.24, 1.50, -2.94);           // end ring
    P.add('hullDark', cylZ(0.075, 0.22, 10), s * 1.24, 1.52, -3.06);           // tail pipe
    P.add('hullDetail', box(0.30, 0.02, 0.72), s * 1.24, 1.415, -2.44);        // mount straps
  ...
  fenders(P, 1.06, 1.50, 1.375, -2.90, 2.10, 0.028);                           // fender planes over the tracks
  ...
  P.add('turret', KIT.polyTurret(PLAN, 0.50, 1.10, 0.94), 0, 0, 0);            // lower cast band (flared skirt -> walls)
  P.add('turret', KIT.polyTurret(PLAN.map(([x, z]) => [x * 0.86, z * 0.965]), 0.36, 1.0, 0.62), 0, 0.50, 0); // upper cast dome band (strong taper — the rounded cast crown)
  P.add('turret', box(0.92, 0.05, 1.55), 0, 0.845, -0.45);                     // center roof plate (top 2.275w)
  P.add('turret', slab(                                                        // bustle spine: the long taper read from above
    [-0.42, 0.80, -0.90], [0.42, 0.80, -0.90], [0.30, 0.72, -1.98], [-0.30, 0.72, -1.98],
    [-0.42, 0.87, -0.90], [0.42, 0.87, -0.90], [0.30, 0.76, -1.98], [-0.30, 0.76, -1.98]));
  // mantlet zone: cast collar + canvas boot; the F1 exits LOW-center
  ...
  P.add('turret', cylY(0.155, 0.165, 0.045, 12), -0.48, 0.885, -0.60);         // loader hatch ring (LEFT)
  P.add('turretDark', torus(0.135, 0.010, 12), -0.48, 0.935, -0.60);
  periscope(P, 'turretDetail', -0.30, 0.87, -0.10, 0.2);
  periscope(P, 'turretDetail', 0.30, 0.87, 0.30);
  ...
  P.add('turret', box(0.24, 0.10, 0.30), 0.42, 0.865, 0.42);
  ...
  if (b2) {
    for (const s of [-1, 1]) {
      ...
      smoke.position.set(s * 0.98, 0.42, -0.90);
      P.turretG.add(smoke);
    }
  }
  ...
  P.gunG.position.set(0, 0.41, 0.75);                                          // axis 1.79w, trunnion z_w 1.05
  ...
  P.addGunExtra(box(0.16, 0.22, 0.55), -0.30, 0.06, 0.42);                     // coax housing slot
  P.addGunExtra(cylZ(0.030, 1.55, 10), -0.30, 0.075, 1.45);                    // 20 mm barrel
  P.addGunExtraDark(cylZ(0.036, 0.10, 10), -0.30, 0.075, 2.22);                // 20 mm muzzle ring
  P.addGunExtraDark(cylZ(0.017, 0.02, 8), -0.30, 0.075, 2.245);                // 20 mm bore dot (§B3.1 pinhole class)
  // PH-8-B IR searchlight LEFT of the mantlet (moves with the gun — the
  // real mount is mantlet-linked). Box + lens + guard bars.
  P.addGunExtra(box(0.50, 0.50, 0.28), -0.72, 0.14, 0.62);
  P.addGunExtraDark(box(0.44, 0.44, 0.03), -0.72, 0.14, 0.775);                // lens frame
  P.addGunExtraDark(box(0.015, 0.46, 0.02), -0.84, 0.14, 0.79);                // guard bars
  P.addGunExtraDark(box(0.015, 0.46, 0.02), -0.60, 0.14, 0.79);
  if (b2) {
    // LLLTV camera box on the mantlet RIGHT (the B2 tell)
    P.addGunExtra(box(0.30, 0.34, 0.30), 0.52, 0.16, 0.55);
    P.addGunExtraDark(box(0.24, 0.26, 0.03), 0.52, 0.16, 0.715);               // camera window
    P.addGunExtraDark(cylZ(0.055, 0.05, 10), 0.52, 0.24, 0.735);               // lens hood
  }
  buildGun(P, { len: 5.15, r: 0.062, sleeve: false, evac: null, collar: false, baseR: 0.135 });
  muzzleBore(P, 0.062, 5.13);                                                  // §B3.1 muzzle bore (shadow-named)
  P.topY = 1.15;
}

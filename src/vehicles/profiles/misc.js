...
function buildAriete(P) {
  ...
  P.add('hull', frustum(1.66, 3.78, 1.32, 1.48, 1.36, 1.26, 0.98, 1.45));      // long upper glacis
  P.add('hull', frustum(1.50, 3.22, 3.78, 1.66, 3.78, 3.78, 0.44, 0.98));      // raked lower bow
  P.add('hull', box(3.06, 0.46, 0.10), 0, 1.20, -3.73);                        // rear plate
  ...
  for (const s of [-1, 1]) {
    skirtZ.forEach((z, k) => {
      P.add('hull', box(k === 0 ? 0.085 : 0.045, k === 0 ? 0.54 : 0.48, 0.90), s * 1.79, 0.87, z);
      P.add('hullDark', box(0.05, 0.44, 0.018), s * 1.795, 0.87, z - 0.45);
      P.add('hullDark', cylZ(0.022, 0.018, 8), s * 1.815, 1.06, z, 0, s * Math.PI / 2, 0);
    });
    P.add('hullDark', box(0.02, 0.06, 6.30), s * 1.786, 0.615, 0, 0, 0, 0);    // shadow lip, no sunlit top face
  }
  ...
  buildRunningGear(P, {
    ...
    wheelZs,
    sprocket: { z: -3.28, y: 0.47, r: 0.29 }, idler: { z: 3.26, y: 0.46, r: 0.28 },
  ...
  // roof: TURMS gunner sight box (right-front, split doors), commander pano
  sightBox(P, 'turret', 0.62, TH + 0.15, 0.40, 0.44, 0.30, 0.52);
  P.add('turretDetail', box(0.48, 0.035, 0.56), 0.62, TH + 0.315, 0.40);       // sight lid
  P.add('turret', cylY(0.10, 0.11, 0.18, 12), 0.42, TH + 0.09, -0.32);         // pano pedestal
  P.add('turretDark', cylY(0.14, 0.14, 0.20, 12), 0.42, TH + 0.28, -0.32);     // pano head
  P.add('turretGlass', box(0.14, 0.07, 0.018), 0.42, TH + 0.30, -0.18);
  ...
  jerryCan(P, 'turretCloth', -0.85, 0.86, -1.95, 0.2);
  stowage(P, 'turretCloth', rng, [[0.55, 0.86, -1.95, 0.8, 0.24, 0.45]]);
  whip(P, 1.00, TH, -1.50, 0.80, 0.12, -0.62);                                 // stowed rake aft
  whip(P, -1.00, TH, -1.50, 0.80, -0.12, -0.62);
  for (const s of [-1, 1]) {
  ...
  // len tuned to the oracle overhang (its L/44 reads ~0.25 m shorter than
  // the published muzzle station — r2 mask); fatter sleeved tube Ø, the
  // evacuator pulled toward the root where the oracle's tube mass sits.
  buildGun(P, { len: 4.90, r: 0.098, sleeve: true, evac: 0.44, evacR: 1.80, collar: true, baseR: 0.17 });
  P.topY = TH + 0.42;
}
...
function buildLeclerc(P) {
  ...
  const { rng } = P;
  // hull: compact tub, vertical band, single-plane glacis
  P.add('hull', box(2.35, 0.58, 6.55), 0, 0.79, 0);
  P.add('hull', frustum(1.64, 3.36, -3.40, 1.60, 3.34, -3.38, 1.02, 1.60));
  P.add('hull', box(3.16, 0.05, 4.55), 0, 1.575, -1.05);                       // fore/mid deck
  P.add('hull', box(2.95, 0.16, 2.15), 0, 1.66, -2.32);                        // raised engine run
  P.add('hull', frustum(1.66, 3.42, 1.58, 1.66, 1.60, 1.58, 0.90, 1.60));      // glacis
  P.add('hull', frustum(1.66, 3.10, 3.20, 1.66, 3.42, 3.20, 0.48, 0.90));      // lower nose
  P.add('hullDetail', box(2.88, 0.05, 0.08), 0, 1.34, 2.28, -0.32, 0, 0);      // splash ridge
  fenders(P, 1.24, 1.79, 1.18, -3.38, 3.30, 0.03);
  ...
  for (const s of [-1, 1]) {
    P.add('hull', box(0.09, 0.55, 2.15), s * 1.795, 0.94, 2.18);
    for (let k = 0; k < 3; k++) P.add('hullDark', box(0.096, 0.48, 0.016), s * 1.80, 0.94, 2.85 - k * 0.72);
    P.add('hull', box(0.035, 0.48, 4.20), s * 1.795, 0.92, -1.05);
    for (let k = 0; k < 5; k++) P.add('hullDark', box(0.04, 0.42, 0.016), s * 1.80, 0.92, 0.72 - k * 0.86);
    P.add('hullDark', box(0.02, 0.06, 6.35), s * 1.786, 0.655, 0.05);
    mudflap(P, s * 1.44, 0.50, -3.46);
    mudflap(P, s * 1.44, 0.56, 3.30);
  }
  // rear plate: grilles + taillights + REAR STOWAGE RACK overhang
  P.add('hull', box(3.00, 0.48, 0.10), 0, 1.30, -3.42);
  for (const s of [-1, 1]) {
    P.add('hullDark', box(0.64, 0.34, 0.04), s * 0.85, 1.10, -3.475);
    for (let k = 0; k < 4; k++) P.add('hullDetail', box(0.62, 0.04, 0.05), s * 0.85, 0.97 + k * 0.095, -3.49);
    P.add('hullDark', box(0.14, 0.08, 0.05), s * 1.32, 1.36, -3.47);
  }
  P.add('hullDetail', box(2.60, 0.035, 0.035), 0, 1.70, -3.62);                // rack rails
  P.add('hullDetail', box(2.60, 0.035, 0.035), 0, 1.30, -3.80);
  for (let k = 0; k < 6; k++) P.add('hullDetail', box(0.03, 0.44, 0.22), -1.25 + k * 0.5, 1.50, -3.68);
  stowage(P, 'hullCloth', rng, [[-0.7, 1.50, -3.66, 1.0, 0.34, 0.38], [0.75, 1.48, -3.66, 0.9, 0.30, 0.36]]);
  // engine deck: fan field + louvres + caps
  P.add('hullDark', box(1.90, 0.018, 1.35), 0, 1.75, -2.30);
  for (let k = 0; k < 7; k++) P.add('hullDetail', box(1.80, 0.024, 0.055), 0, 1.76, -1.75 - k * 0.17);
  ...
  buildRunningGear(P, {
    ...
    wheelZs,
    sprocket: { z: -2.96, y: 0.50, r: 0.30 }, idler: { z: 2.92, y: 0.48, r: 0.28 },
  ...
  const LH = 0.80;                                                             // roof 2.40
  P.add('turret', frustum(1.30, -0.12, -2.14, 1.24, -0.15, -2.10, 0.0, LH));   // rear/autoloader box
  ...
  // forward side ARMOR CHEEK BOXES + midships stowage baskets (width 3.02)
  for (const s of [-1, 1]) {
    P.add('turret', box(0.20, 0.58, 1.25), s * 1.41, 0.33, 0.18, 0, s * 0.06, 0);
    P.add('turretDark', box(0.21, 0.10, 1.15), s * 1.415, 0.66, 0.16, 0, s * 0.06, 0);
    P.add('turretDetail', box(0.045, 0.045, 1.30), s * 1.44, 0.58, -1.15);     // basket rails
    P.add('turretDetail', box(0.045, 0.045, 1.30), s * 1.44, 0.14, -1.15);
    for (let k = 0; k < 5; k++) P.add('turretDetail', box(0.03, 0.44, 0.03), s * 1.44, 0.36, -0.62 - k * 0.26);
    stowage(P, 'turretCloth', rng, [[s * 1.36, 0.37, -1.15, 0.15, 0.32, 1.05]]);
  }
  // roof: HL-70 armored sight head (right of gun, front) + panel lines
  sightBox(P, 'turret', 0.55, LH + 0.21, 0.72, 0.40, 0.42, 0.42);
  P.add('turret', box(0.46, 0.06, 0.48), 0.55, LH + 0.44, 0.72);
  ...
  P.add('turret', cylY(0.22, 0.22, 0.045, 14), -0.56, LH + 0.02, -0.55);
  P.add('turretDark', box(0.38, 0.012, 0.03), -0.56, LH + 0.07, -0.55);
  P.add('turret', cylY(0.20, 0.20, 0.04, 14), 0.58, LH + 0.02, -0.90);
  periscope(P, 'turretDetail', -0.35, LH + 0.05, -0.28, 0.3);
  periscope(P, 'turretDetail', 0.42, LH + 0.05, -0.58);
  // HL-15 panoramic: THIN pedestal + small head, left-rear
  P.add('turretDetail', cylY(0.055, 0.06, 0.34, 10), -0.55, LH + 0.17, -1.10);
  P.add('turretDark', box(0.20, 0.24, 0.22), -0.55, LH + 0.46, -1.10);
  P.add('turretGlass', box(0.13, 0.10, 0.018), -0.55, LH + 0.48, -0.98);
  ...
  P.decal('turret', 'number', '33', 0.30, [-1.30, 0.35, -1.0], -Math.PI / 2);
  // CN120-26 L/52 seated LOW (axis 1.93) in a wide shallow mantlet plate
  P.gunG.position.set(0, 0.33, 0.50);
  trunnionRoll(P, 0.21, 0.68);
  P.addGunExtra(box(0.92, 0.72, 0.34), 0, 0.08, 0.32);                         // wide tall mantlet plate
  P.addGunExtra(cylZ(0.15, 0.34, 12, 0.175), 0, 0, 0.56);                      // stepped collar
  P.addGunExtraDark(cylZ(0.028, 0.10, 8), 0.34, 0.10, 0.44);                   // coax port
  P.add('turret', box(0.68, 0.14, 0.30), 0, 0.62, 0.98, -0.40, 0, 0);          // brow
  buildGun(P, { len: 6.17, r: 0.076, sleeve: true, evac: 0.52, evacR: 1.85, collar: true, baseR: 0.16 });
  P.topY = LH + 0.55;
}
...
function buildT80U(P) {
  ...
  // pancake hull
  P.add('hull', box(2.40, 0.55, 6.60), 0, 0.70, -0.08);
  P.add('hull', frustum(1.70, 3.00, -3.34, 1.48, 2.94, -3.30, 1.06, 1.38));    // tapered deck band
  fenders(P, 1.30, 1.79, 1.065, -3.38, 3.22, 0.032);
  P.add('hull', frustum(1.62, 3.42, 1.92, 1.66, 1.88, 1.92, 0.82, 1.38));      // 68° glacis
  P.add('hull', frustum(1.62, 3.08, 3.14, 1.62, 3.42, 3.14, 0.42, 0.82));      // blunt lower nose
  // K-5 glacis wedge raft: 3 courses across the plate
  for (let row = 0; row < 3; row++) for (let c = 0; c < 5; c++) {
    const y = 0.92 + row * 0.145;
    const z = 1.90 + (1.38 - y) * 2.72 + 0.09;
    P.add(c % 2 ? 'hullDetail' : 'hull', box(0.56, 0.15, 0.17), -1.12 + c * 0.56, y, z, -68 * D2R, 0, 0);
  }
  // driver strip + V splash board
  P.add('hull', box(0.50, 0.05, 0.42), 0, 1.30, 2.10, -1.19, 0, 0);
  periscope(P, 'hullDetail', -0.14, 1.375, 1.78);
  periscope(P, 'hullDetail', 0.14, 1.375, 1.78);
  for (const s of [-1, 1]) P.add('hullDetail', box(0.80, 0.05, 0.08), s * 0.37, 1.12, 2.55, -1.19, s * 0.5, 0);
  // skirts: rubber sheet + 3 armored front panels, dark seams, inset lip
  for (const s of [-1, 1]) {
    P.add('hull', box(0.07, 0.42, 2.00), s * 1.795, 0.88, 2.20);
    P.add('hull', box(0.035, 0.40, 4.35), s * 1.795, 0.87, -0.98);
    for (let k = 0; k < 6; k++) P.add('hullDark', box(0.045, 0.34, 0.018), s * 1.80, 0.86, 2.60 - k * 1.05);
    P.add('hullDark', box(0.02, 0.06, 6.30), s * 1.786, 0.64, -0.05);
    mudflap(P, s * 1.46, 0.50, 3.26, 0.60, 0.34);
    mudflap(P, s * 1.42, 0.46, -3.40, 0.56, 0.30);
  }
  // TURBINE EXHAUST BOX jutting off the rear plate (T-80 tell) + drums + log
  P.add('hull', box(1.90, 0.55, 0.26), 0, 0.88, -3.50);
  P.add('hullDark', box(1.55, 0.34, 0.05), 0, 0.88, -3.62);
  for (let k = 0; k < 4; k++) P.add('hullDetail', box(1.50, 0.042, 0.05), 0, 0.74 + k * 0.10, -3.645);
  P.add('hullDetail', box(1.70, 0.05, 0.12), 0, 1.18, -3.56);
  for (const s of [-1, 1]) {
    P.add('hullDetail', cylY(0.14, 0.14, 1.00, 12), s * 1.05, 1.02, -3.28, 0, 0, s * 0.10);
    P.add('hullDark', cylY(0.145, 0.145, 0.03, 12), s * 1.05, 1.30, -3.32, 0, 0, s * 0.10);
    P.add('hullDark', box(0.05, 0.38, 0.03), s * 1.05, 1.00, -3.42);
  }
  P.add('hullWood', cylX(0.105, 2.05, 10), 0, 0.62, -3.50);                    // log LOW on the plate
  for (const s of [-0.55, 0.55]) P.add('hullDark', cylX(0.112, 0.04, 10), s * 1.6, 0.62, -3.50);
  ...
  headlight(P, -1.42, 1.12, 3.02, -0.2, 0.05);
  headlight(P, -1.10, 1.12, 3.05, -0.2, 0.05);
  P.add('hullDetail', torus(0.085, 0.016, 10), -0.55, 0.55, 3.18, Math.PI / 2, 0, 0);
  P.add('hullDetail', torus(0.085, 0.016, 10), 0.55, 0.55, 3.18, Math.PI / 2, 0, 0);
  liftEye(P, 'hullDetail', -1.15, 1.40, 1.5);
  liftEye(P, 'hullDetail', 1.15, 1.40, 1.5);
  towCable(P, [[-1.25, 1.04, 2.85], [-0.35, 0.98, 3.05], [0.50, 1.02, 2.95]]);
  spareTrackStrip(P, 'hull', 1.28, 1.16, 2.36, 2, -1.15, 0);
  P.decal('hull', 'soot', null, 1.0, [0.0, 0.9, -3.62], Math.PI);
  // 6 small dished wheels + 5 skirt-hidden rollers, rear sprocket
  const wheelZs = [2.42, 1.45, 0.48, -0.49, -1.46, -2.43];
  buildRunningGear(P, {
    style: 'dished', wheelR: 0.335, wheelW: 0.21, wheelY: 0.42, xc: 1.47, dishR: 0.80,
    wheelZs,
    sprocket: { z: -3.04, y: 0.50, r: 0.28 }, idler: { z: 3.00, y: 0.47, r: 0.26 },
  ...
  // the gun + flat side-shoulder boxes (front-view span ~2.9 m)
  for (const s of [-1, 1]) {
    P.add('turret', box(0.98, 0.36, 0.38), s * 0.52, 0.24, 0.86, -0.20, s * 0.52, 0);  // lower clam
    P.add('turretDark', box(1.00, 0.035, 0.34), s * 0.52, 0.435, 0.85, -0.20, s * 0.52, 0); // gap seam
    P.add('turret', box(0.88, 0.32, 0.34), s * 0.47, 0.60, 0.73, -0.50, s * 0.52, 0);  // upper clam
    P.add('turretDetail', box(0.06, 0.32, 0.34), s * 0.94, 0.26, 0.50, -0.20, s * 0.52, 0); // end plate
    P.add('turret', box(0.52, 0.48, 0.96), s * 1.18, 0.32, 0.08, 0, s * 0.14, 0);      // side shoulder box
    P.add('turretDark', box(0.52, 0.05, 0.94), s * 1.19, 0.58, 0.07, 0, s * 0.14, 0);
    P.add('turretDetail', box(0.36, 0.20, 0.62), s * 1.22, 0.66, -0.10, 0, s * 0.10, 0); // shoulder stowage
  }
  // commander cupola RIGHT + Utyos NSVT on the AA ring; gunner hatch left
  cupola(P, 'turret', 0.52, 0.66, -0.35, 0.22, 0.12, 5);
  P.add('turretDetail', torus(0.30, 0.02, 14), 0.52, 0.90, -0.35);
  P.add('turretDark', box(0.09, 0.11, 0.48), 0.60, 0.98, -0.16);               // receiver
  P.add('turretDark', cylZ(0.024, 0.66, 8), 0.60, 1.00, 0.30, -0.05, 0, 0);    // barrel
  P.add('turretDark', cylZ(0.037, 0.12, 8), 0.60, 1.015, 0.62, -0.05, 0, 0);   // muzzle booster
  P.add('turretDetail', box(0.10, 0.14, 0.18), 0.47, 0.94, -0.24);             // ammo box
  P.add('turretDark', box(0.03, 0.14, 0.10), 0.60, 0.90, -0.30);               // grips
  ...
  // 1G46 sight doghouse left of gun (tall armored tower) + Luna IR right
  sightBox(P, 'turret', -0.40, 0.86, 0.26, 0.40, 0.34, 0.40);
  P.add('turret', box(0.44, 0.05, 0.44), -0.40, 1.06, 0.26);                   // doghouse lid
  P.add('turretDetail', box(0.26, 0.26, 0.24), 0.55, 0.42, 0.96);
  P.add('turretGlass', box(0.18, 0.18, 0.02), 0.55, 0.42, 1.09);
  // 902 smoke tubes clustered LEFT side, riding the upper cheek line
  for (let k = 0; k < 5; k++) P.add('turretDark', cylZ(0.042, 0.28, 8), -1.02 - k * 0.09, 0.58 + (k % 2) * 0.03, 0.48 - k * 0.11, -0.48, -(0.85 + k * 0.12), 0);
  for (let k = 0; k < 4; k++) P.add('turretDark', cylZ(0.042, 0.28, 8), -1.10 - k * 0.09, 0.40, 0.24 - k * 0.11, -0.38, -(1.0 + k * 0.14), 0);
  // bustle: transverse OPVT snorkel + stowage band + basket + rails
  P.add('turretDark', cylX(0.075, 1.55, 10), 0, 0.72, -1.08);
  P.add('turretDark', cylX(0.055, 0.30, 8), 0.88, 0.72, -1.08);
  P.add('turret', box(2.60, 0.58, 0.66), 0, 0.30, -1.42);                      // stowage box row wrapping the rear
  P.add('turretDark', box(2.45, 0.05, 0.58), 0, 0.60, -1.42);
  P.add('turretCloth', box(1.30, 0.24, 0.50), 0.10, 0.70, -1.42);              // strapped kit on top
  ...
  buildGun(P, { len: 5.50, r: 0.068, sleeve: true, evac: 0.47, evacR: 1.95, collar: false, baseR: 0.15 });
  P.topY = 1.15;
}

// ---------------------------------------------------------------------------
// Type 90 — docs/references/tanks/type90.md
// Parametric base kept (type90 hull/turret styles in kit.js) + bespoke
// surface pass: taller slab turret (roof 2.33 real), forward-right commander
// sight cluster, center M2, rear rack overhang, long raked whips, re-seated
// L/44 (overhang 2.26), evacR 1.9 gun rebuild, lower visible wheel band.
// Oracle is width-under-normalized (~20% tall) — real dims kept, cap logged.
// ---------------------------------------------------------------------------
function type90Extras(P, p) {
  const { box, cylY, cylZ, buildGun, liftEye } = KIT;
  const TH = p.turretHeight;
  // gun rebuild with the fat readable evacuator (kit path lacks evacR)
  P.clear('gun', 'gunDark');
  buildGun(P, { len: p.gunLength, r: p.gunRadius, sleeve: true, evac: 0.49, evacR: 1.9, collar: true, baseR: 0.16 });
  // commander's stabilized periscope sight: tall box forward-RIGHT of center
  sightBox(P, 'turret', 0.52, TH + 0.19, 0.42, 0.38, 0.38, 0.44);
  P.add('turret', box(0.44, 0.05, 0.50), 0.52, TH + 0.40, 0.42);
  // center pintle M2 between the hatches
  P.add('turretDark', cylY(0.026, 0.032, 0.18, 8), 0.0, TH + 0.09, -0.55);
  P.add('turretDark', box(0.10, 0.07, 0.16), 0.0, TH + 0.21, -0.55);
  P.add('turretDark', box(0.095, 0.11, 0.46), 0.0, TH + 0.30, -0.46);
  P.add('turretDark', cylZ(0.025, 0.58, 8), 0.0, TH + 0.315, -0.05, -0.06, 0, 0);
  P.add('turretDark', cylZ(0.037, 0.10, 8), 0.0, TH + 0.33, 0.26, -0.06, 0, 0);
  P.add('turretDetail', box(0.10, 0.12, 0.17), -0.13, TH + 0.26, -0.52);       // ammo box
  // rear stowage rack overhanging the deck + cargo
  basket(P, 1.05, p.turretRear - 0.18, p.turretRear - 0.62, 0.10, 0.52, 0.65);
  // long near-vertical whip antennas off the bustle flanks (oracle: straight
  // rods at ~±0.8 reaching well above the roof)
  whip(P, 0.80, TH, p.turretRear * 0.72, 1.55, 0.05, -0.06);
  whip(P, -0.80, TH, p.turretRear * 0.72, 1.55, -0.05, -0.06);
  liftEye(P, 'turretDetail', -0.95, TH + 0.02, 0.55, -0.4);
  liftEye(P, 'turretDetail', 0.95, TH + 0.02, 0.55, 0.4);
  // hull: front fender bins + mud flaps + tow eyes
  bin(P, -1.42, 1.52, 2.45, 0.34, 0.18, 0.85);
  bin(P, 1.42, 1.52, 2.45, 0.34, 0.18, 0.85);
  for (const s of [-1, 1]) {
    mudflap(P, s * 1.43, 0.52, 3.68, 0.54, 0.34);
    mudflap(P, s * 1.40, 0.48, -3.66, 0.52, 0.30);
  }
  wheelRecessAt(P, [2.71, 1.63, 0.55, -0.55, -1.63, -2.71], 1.47, 0.45, p.wheelR, 0.20);
}
...
function buildType74(P) {
  ...
  for (const s of [-1, 1]) {
    ...
    // tall whip antennas on the REAR hull corners (oracle: near-vertical
    // rods rising past the turret roof)
    P.add('hullDetail', box(0.05, 0.06, 0.05), s * 1.36, 1.35, -3.05);
    P.add('hullDetail', box(0.022, 1.50, 0.022), s * 1.42, 2.10, -3.05, 0, 0, s * 0.06);
  }
  ...
  P.add('turretDark', cylY(0.026, 0.032, 0.20, 8), 0.58, 0.90, -0.50);
  P.add('turretDark', box(0.10, 0.07, 0.15), 0.58, 1.03, -0.50);
  P.add('turretDark', box(0.10, 0.12, 0.48), 0.58, 1.12, -0.38, 0, 0.12, 0);
  P.add('turretDark', cylZ(0.026, 0.60, 8), 0.62, 1.135, 0.04, -0.06, 0.12, 0);
  P.add('turretDark', cylZ(0.038, 0.11, 8), 0.66, 1.15, 0.34, -0.06, 0.12, 0);
  P.add('turretDetail', box(0.10, 0.12, 0.16), 0.44, 1.07, -0.52);             // ammo can
...
export const MISC_PROFILES = {
  ...
  type90: {
    ...
    trackW: 0.56, wheels: 6, wheelR: 0.37, wheelY: 0.45, wheelSpan: 5.42, frontSprocket: false,
    skirts: true, skirtPanels: 6, skirtHeight: 0.36, skirtY: 0.84, skirtLength: 6.30,
    // gunZ stays 0: the kit path seats its mantlet extras against the turret
    // face in gun-frame coords — a forward gun origin detaches them (r1
    // floater). The L/44 muzzle station comes from gunLength instead: the
    // visible tube past the mantlet is 5.91-0.95 ≈ 5.0 m ≈ L/44 exposed run.
    turret: 'type90', turretWidth: 2.66, turretDepth: 3.62, turretHeight: 0.90, turretFront: 1.13, turretRear: -2.16,
    turretPivotY: 1.42, turretPivotZ: -0.18, gunY: 0.40, pano: false, gunLength: 6.09, gunRadius: 0.079,
    mantletWidth: 0.50, mantletHeight: 0.38, antennas: false, smokeCount: 3,
    build: (P, p) => { buildProfile(P, p); type90Extras(P, p); },
  },

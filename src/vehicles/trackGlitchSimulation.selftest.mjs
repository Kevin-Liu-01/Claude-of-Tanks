...
const IDS = ['t90m', 'm1a2', 'leo2a6', 'tiger1', 'mbt70', 'cv90_mkiv', 'kf51_x', 't72b3m_x', 'ztz100_x', 'type100'];
...
for (const id of IDS) {
  ...
  try {
      ...
      for (const side of ['L', 'R']) {
        ...
        for (const mesh of tires) {
          ...
          for (let k = 0; k < mesh.count; k++) {
            ...
            if (!rec.wheelZs.some((sz) => Math.abs(sz - _p.z) < 0.02)) continue; // stowed spares are not road wheels
    ...
    results.push({ id, cutMm: +(maxCut * 1000).toFixed(1), daylightMm: +(maxDay * 1000).toFixed(1), travelMm: [Math.round(minOff * 1000), Math.round(maxOff * 1000)] });

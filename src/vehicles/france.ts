...
// §B3.1 MUZZLE BORE (modern3.ts mirror — see its header note): the open
// outer wall carries the tube to the face and one flat dark cap closes it;
// the factory's fallback assembly is the visible dark mouth. Owner
// 2026-09-22: "the point of adding holes instead of carving them into the
// barrel is that we save on triangles" — the former inward funnel + finish
// torus + recessed disc (24·seg) is gone; 3·seg now. boreR documents the
// true caliber at the call site.
function muzzleBore(
  P: FranceBuilderPort,
  faceZ: number,
  R: number,
  ...
  P.add('gunDark', new CircleGeometry(R - 0.001, seg), 0, 0, faceZ - 0.002);
}

...
and deterministic participant/camouflage planning; `soloBattleRuntime.ts` is
the lazy, typed import boundary for legacy solo authority in `state.js`, which
owns battle setup and the fixed battle step; `ai.js` owns bot decisions and is
...
through `soloBattleRuntime.ts` on Battle or capture intent. Multiplayer work

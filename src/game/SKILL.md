...
and deterministic participant/camouflage planning; `state.js` owns legacy solo
battle setup and the fixed battle step; `ai.js` owns bot decisions and is
...
in `state.js`. Multiplayer work must move visual creation out of authority
rather than importing more UI into `state.js`.
...
calls; deepen that seam incrementally through `rosterState.ts`. Entity IDs
historically equal spec IDs and must not remain so in multiplayer.

...
export const FRONTLINE_LIMITS = Object.freeze({
  ...
  // owner 2026-09-15 (evening): no distant smoke. The front keeps its artillery anchors
  // ("columns" by name only), flashes, flak, aircraft and AA guns.
...
export function createFrontlineAtmosphere(options: FrontlineAtmosphereOptions): FrontlineAtmosphere {
  ...
  /** The front's artillery anchors: a fan of positions beyond the playable half-map (no smoke since 2026-09-15). */
  function layoutFront(): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // the former plume layout drew height, width and a plume seed here; keep the draws so a
      // seed still replays the same front (receipt: byte-identical event logs)
      rng(); rng(); rng();
      columnAnchors.push(new THREE.Vector3(x, groundY(x, z) - 6, z));
    }
  }

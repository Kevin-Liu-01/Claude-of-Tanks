...
await withFixture(async f => {
  ...
  assert.match(String(terrain._terrainBuildSteps), /createSplatMaterialSteps\([\s\S]*?sourcePreparation,\s*seaOpenings,\s*cfg\?\.sky \?\? null,\s*heightField\.getOutlandWaterAt \?\? null,?\s*\)/);

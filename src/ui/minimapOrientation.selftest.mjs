...
assert.equal(minimapAngleForDirection(1, 0), 0,
  'a world-right view cone points right on the fixed map');
...
assert.match(hudSource,
  /pushLiveBlip\([\s\S]{0,220}normalizeMinimapAngle\(state\.yaw\)/,
...
assert.match(worldActivationSource,
  /minimapAssetVersion \|\| 'north-up-v4'/,

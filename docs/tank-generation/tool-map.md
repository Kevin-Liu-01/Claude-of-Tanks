...
| Wheels and belts | [nationWheelSets.ts](../../src/vehicles/nationWheelSets.ts) (donor table, per-nation rules, exceptions, period list) and [nationWheelConstructions.ts](../../src/vehicles/nationWheelConstructions.ts) (donor faces drawn at any hull's size), [wheelPatterns.ts](../../src/vehicles/wheelPatterns.ts), [trackPatterns.ts](../../src/vehicles/trackPatterns.ts), [suspensionPatterns.ts](../../src/vehicles/suspensionPatterns.ts); one suspension-owned moving assembly, no static duplicate course. Review with `tools/wheel-inventory.mjs` and `tools/wheel-review.mjs --all --gate`. |

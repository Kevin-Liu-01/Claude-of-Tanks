...
| --- | --- | --- |
| FSP-01 | Measure and reduce excessive geometry/construction cost; eliminate the reported tank-switch stalls. | LANDED 2026-09-25 — profile + reductions ([garage switch profile](../performance/garage-switch-profile-20260925.md)): the stall was the first frame blocking on the new hero's shader links (0.25–1.1 s tasks whenever a switch created program variants), then the 50–120 ms synchronous core builder step, then LRU rebuilds. R1 prepares the links before reveal (cold worst long task p95 903 → 123 ms, first painted frame p95 1217 → 754 ms); R2 builds the two adjacent cards in the quiet window with a desktop LRU of six (adjacent walk 3 → 10 warm of 12, warm reveal 6 ms, no long tasks; cold worst long task p95 89 ms). Receipt `tools/garage-switch-probe.selftest.mjs` gates warm switches (p95 long task ≤ 120 ms, painted ≤ 250 ms). Open: the core step and cross-nation cold switches |
...
| FSP-03 | Add and verify return rollers across the full fleet wherever the actual vehicle has them; preserve genuinely rollerless suspensions. | LANDED 2026-09-25 (owner: yes) — 38 hulls gained verified rollers (12 source-measured T-72/T-90 X stations restored, 26 fitted under measured lane ceilings), 6 kept rollerless (T-62 pair, Type 59, STB-1/Type 74) plus the JPz E-100 concept by chassis derivation, 1 deferred (Challenger 1: belt-line re-loft needed); the 2026-09-23 "T-72/T-90 rollerless" reading is reversed on the repo's own source studies and FAS/GlobalSecurity — see [the audit](return-roller-audit-20260925.md). Round 96 (2026-09-25): landed together with FSP-05 on one tree over the FSP-06 base — the per-id regeneration re-run there (anchors and marking seats byte-identical, anatomy and views re-rendered), geometry gate 11/11 for the registered T-72/T-90 X ids (fleet 78/168, worst t90ms_x 92.2), sealed ledger held for all 38 ids, wheel review 192 / 0 flagged, the digest receipts the lane had not run (`equipmentDamage` t90m rest + calibrated armor) re-pinned once with dated notes |
| FSP-04 | Thicken the new tracks to the established original-fleet visual standard, with correct moving-shoe and end-wheel clearances. | CLOSED 2026-09-25 — owner: the tracks are good |
| FSP-05 | Complete lower hull/chassis side plates and connect hull sides, shoulders, fenders and skirts without accidental holes or floating panels. | LANDED 2026-09-25 (owner: yes) — 16 hulls closed ([audit](chassis-closure-audit-20260925.md): 23 → 7 unsealed ids, the 7 turret-owned; fleet openings 6,299 → 1,912 px); floating-panel candidates recorded there for the next round. Round 96 (2026-09-25): landed together with FSP-03 on one tree over the FSP-06 base — the closed lofts, wedges and slabs re-rendered with the Merkava pale kit and the K2 reels (anatomy moved for merkava1b / 3c / 3d / 4b, m3a3_bradley, ua_m2a3_bradley, marder1a3 and t14; anchors and marking seats byte-identical), sealed ledger held for all 22 ids, the digest receipts the lane's name filter had missed (`sourceXFleet` merkava3d / t14 / k2, `tankFactoryStaging` merkava1b, `sourceXSecondWave` amx30) re-pinned once with dated notes; the closed bows enclosed small voids the committed fills did not claim (`tank-watertight-check`: m3a3_bradley 2.11 L, t14 0.83 L, m46_patton 0.48 L, m2a2_bradley 0.14 L, merkava2d 0.06 L new against the base tree, amx30 / amx30b2 / merkava3c grown by 0.03–0.36 L), closed the same day by a fills-to-zero regeneration of those eight hulls (`gen-interior-fills --rounds=8 --min-fine=1`, records merged by id): all eight read WATERTIGHT 0 L / 0 gaps; the interior-fill receipts, the 8-id sealed ledger gate, 19 family receipts and the whole-model digest receipts (`sourceXFleet`, `sourceXSecondWave`, `sourceXOtherAuxArmor`, `tankFactoryStaging`, `equipmentDamage`, `camoPolicy`) pass without a re-pin — fills sit outside the authored fingerprints — and `gen-combat-anatomy --check` holds, fills being no anatomy input |

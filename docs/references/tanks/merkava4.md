...
## 2026-08-13 FIRST-PARTY WIND BREAKER RESTORATION — RE-FROZEN

The playable is wholly repository-authored procedural geometry. The former
arlassar comparison print is foreshortened, yawed and component-fused; it is
no longer a runtime model, a selectable candidate or a UI attribution. No
imported mesh, vertex/index payload, material, texture, rig, animation or
source-derived geometry module contributes to this build. Public IDF and
Wikimedia photographs plus published 7.60 / 9.04 / 3.72 / 2.66 m dimensions
govern the source-semantic rebuild.

The primary Mk.4M turret now has a short, low clipped wedge instead of the
long rectangular roof box. The crown narrows and falls into a compact solid
rear shoulder before the open chain basket; cheek planes and the tight gun
carrier remain continuous with the cast core. The basket is shortened,
lowered and opened with visible side/end returns. Three proper antenna whips
replace buried pegs. A broad commander/cupola seat, supported ballistic
shield, standardized seated M2, second roof gun, stabilized panoramic head,
six-tube smoke bank, unequal periscopes/access covers and rear control boxes
form one coherent turret-owned combat package. All equipment overlaps a roof,
collar, cradle or cheek foundation.

The rear track blind spot was traced to the forward face of the hull-owned
stowage rack, not the body loft. That rack alone is split and lifted above the
full rear idler/shoe crest; its aft service face retains the low silhouette.
The strict exact audit now reports band **0/0**, instanced shoes **0/0** and
full moving sweep **0/0**, with no semantic exemption.

Freeze **`df0a5768`** reproduces twice at 44 meshes / 79,959 vertices.
Parent audit is **0 stranded / 0 abutting / 0 dangling**. Winding is **0
reversed / 0 mixed / 0 deficit pixels**; runtime articulation passes 10/10
and the authored muzzle bore passes at 107.2 contrast. The standard census
records `mg1+0d`; its only failed numeric column is the known unusable
comparison-oracle row, which remains honestly zero rather than becoming a
fabricated pass. Published-dimension fidelity remains 91.8.

The final procedural packet at `/private/tmp/merkava4-final-r5` contains 15
yaw0 and 15 yaw90 frames including the standardized elevated-left profile:
**30 PNGs / 30 distinct hashes**. Every frame was inspected. Gun, mantlet,
complete turret, roof stations, both guns, sight, smoke, antennae and basket
execute a genuine quarter-turn over a fixed coherent hull, rear service field
and six-wheel native course. No fused duplicate, empty-air decoration, open
sheet, track collision or yaw-dependent wound is visible.

...
| leclerc | e6523de8 | OWNER ROUND 2026-08-17 (§5.342 three-item: lower glacis re-lofted continuous to the beak, rear sprocket r0.24@y0.92 nothing-deleted, §B2 fills; flagship bar held; xlr/amx56 shared-flow 1f64e4c/71e39be; oracle .bak-only so §5.254+§B2 evidence per §5.265; prior row pre-§5.229-stale) |
| amx30 | 33b5048 | REBUILT 2026-08-17 by owner order (§5.309 "stripped amx40 base": skirtless, cast-round 5-ring turret, TOP/lights/20mm-coax/PH-8-B; row 83.8 = owner-decreed divergence vs the ahab print; prior 89a4cdf0 graduate state retired by owner authority) |
| amx30b2 | 88801828 | REBUILT 2026-08-17 by owner order (§5.309: the B2 branch — COTAC head, LLLTV, fender bins, 3-tube smoke; row 82.6 owner-decreed divergence; prior 34076800 retired by owner authority) |
...
| t90sm | 17b8330 | OWNER ROUND 2026-08-17 (§5.343 flush-panels + flowing cage: three marked verticals pitched 33.6-36.9° from 0-9.7°, inboard wall BURIED flush, shingle course 42-46°, T4S brim + carrier boxes retired; cage = one shared-vertex monotone polyline/side, chamfer 40.5°, numerals unclipped; whole-AABB byte-identical = dims hold; .bak-only oracle so §5.254+§B5 evidence per §5.265; prior 7efc69c9 pre-§5.229-stale) |
...
## §5.310 — STB-1 actual-source ground-up rebuild (OWNER session entry) (2026-08-17)
- Replaced the earlier limited dome-articulation pass with a true comparison against the owner-authoritative `/Users/kevinliu/Downloads/stb_1.glb` (SHA-256 `81ab9724a572410cb0eebeda0d716c65bf9c7ad2bddb1ef77a9f715d2184b7a6`). The final evidence packet contains **14 actual-source/procedural pairs + 14 yaw-0 + 14 yaw-90 frames**. Hero-front-left, hero-rear-right, left, close-roof and both quarter-turn heroes were inspected at the final bytes.
- Rebuilt the hull from nested boxes into a closed seven-station, two-stage loft: narrow lower tub inside the shoe corridor, full-width shoulder flare above the course, falling upper/lower glacis and tucked nose. Added backed radiator meshes, source-specific horizontal muffler canisters, three-lamp armored banks and closed stern fittings. The single suspension-driven smart course now uses the source's exposed five-road-wheel stance without deleting fenders or armor.
- Reformed the turret as the source's tall organic casting rather than a flat pancake, with buried ring skirt, four-cell cheek grilles, flank vent arrays, unequal cast service plates, roof bosses, commander/loader stations, crew MG, six-pane searchlight, smoke banks, raised basket/stowage and whips. All fittings have visible armor shoes, backs or rail returns and remain turret-owned at yaw 90.
- Final structural audits: exact track clip **front 0 / rear 0 / shoe 0 / strict sweep 0**, duplicate-course **1/1 PASS**, turret parent **0 stranded / 0 abutting / 0 dangling**, focused asset check **1 tank / 9 files PASS** with muzzle bore verified. Selective icon/silhouette/armor/hit-zone/module assets and `tank-assets.json` were regenerated; full `npm test` and the public production build pass. The legacy geometry gate is explicitly not claimed: this unusually scaled source GLB still defeats the existing source-normalization path, so its absolute gate measurements are not valid evidence for this rebuild.

## §5.311 — FULL LANE NOTATION (owner request: "notate what each lanes are fully because i might lose wifi rn") + NETWORK-DROP EVENT (2026-08-17)
**The wifi drop materialized**: four agents died mid-flight (three stream-watchdog stalls + one ENOTFOUND API kill). Everything below is the complete, durable state — all WIP is ON DISK in the worktree + agent scratchpad snapshots per the snapshot law. NOTHING IS LOST.

### LANDED THIS ARC (committed + pushed)
- **Lane A §5.305** — swedish triple revert (owner §5.299/§5.301): strv81 `911d5770` (row 0), strv122 `1ca18498` (86.6), strv103 `4c8f1330` (57; §3 graduate row REVERTED to it, 4ac3c8c8 retired by owner authority).
- **§E batch §5.306** — 9 print repairs DONE (pt91 0→24.6 registration de-poison; leo2a6m detail-shell excision 90.9/100/100 EXACT-HOLD move-invariance; pl01 66.9; bmp3 39.8; oplot stations 68.5; ua_t64bv 42.9-whole; strv81/strv103/spz_puma prints), 2 SKIPPED w/ receipts (kursk dims-sovereignty; leo2a4m below-ceiling), 3 STOPPED w/ receipts (bmpt absent-geometry; ua_t80bv y_map-crater needs new plan; m3a3 skinned-bounds hypothesis DISPROVEN). Batches 57-65 in repair_oracles.py.
- **Lane B §5.307** — K2B `13afe560` (Korea, tier 9): byte-identical resurrection of the pre-§5.248 pl01 (K2-donor variant), 7-file wiring complete, garage-verified (IX K2B in the KR tab).
- **Lane C §5.308** — type10 REVERTED `a1cecea6` (row 68.7, old floater gone) + type10b PINNED `77870ef0` byte-identical (buildType10BBase verbatim pin; 14/14 pixel hold).

### DELIVERED ON DISK, PENDING RE-CERT (uncommitted in worktree)
- **Lane D (ariete slopes, owner §5.299)**: ariete_c1 `49c15299` (49/64151) + ariete_c2 `b0b3c184` (51/68699) in src/vehicles/profiles/italy.js (47+/23−). Sloped cheeks ~40° + true glacis planes; c1 rows hold-or-improve ×2 (39.6 held, hull 47.87, dims byte-identical); guards carro `9fa68918` + donor `43e126e8` held. Packets + §5.254 pairs at shots/ariete-slopes/. **Re-cert sitting DIED early (harness inspection stage) — RESPAWN with the §5.309 brief** (changed views: front/FQ/glacis/hero; slope verify; c2 package rides the rake; KEEPs spot-check).

### DIED MID-WORK — WIP ON DISK, RESUME VIA SendMessage (agents resume from transcript)
- **Lane E (a...006, owner §5.299+§5.303)**: THREE items in profiles/leopard.js (+germany.js): (1) leo2a4m = KEEP germany-wave hull+gun, turret from the pre-b66d6d03 wrapper (94a83234 baseline); (2) leo2a6m turret FINISH (critic polish list: bow-corner cage flares, rear-wall X-cables + tail lamps, nose-flank cage run, front density; 59452b7a baseline, hold-or-improve 90.9 + dims 100); (3) kf51b FLEET INTEGRATION (tracks/hull/turret to fleet grammar; owner b3d15714 absorbed; kf51 `ffb1144c` untouchable; FALSE-0 no-gate). Last state: kf51b mid-integration ("fleet camo, skirted course, readable wheels" — checking rear/roof). HAZARD: a mid-edit frustum ReferenceError was seen in buildLeo2A4M — profiles/leopard.js may NOT parse until they finish; npm test on the live tree fails until then (their &&-gate protects the landing).
- **Lane F (a...635, owner §5.302+§5.303+§5.306-order)**: FOUR items in afvFamily.js ×2 files — (1) marder1a3 old-hull (2fc642fb^) + new-turret splice (694568 baseline); (2) bmp3 §B2 bow-gap fix + twin bow PKT MGs + equipment/decorations (310b7f2e baseline; bmp3_rok inherit-verify); (3) **bmpt id REMOVAL** (keep bmpt_terminator2; de-wire spec/builder/tier/labels/markings/garage/maps; ledger row drops ORCHESTRATOR-side at landing); (4) m3a3 revert-except-equipment hybrid (old base + wave TOW/ISU/CIV/armor; b0eb98a1 baseline). Last state: "everything worktree-modified, nothing staged... one last eyeball of the AFTER frames, then the report" = **ESSENTIALLY COMPLETE, report unsent**. RESUME to emit the final report + receipts, then land.
- **Lane G (a...e99, owner §5.304)**: (1) t62mv1 +10% width (build + spec widthM ×1.10; owner-decreed print divergence documented; profiles/russia.js:506 + userdrops5.js spec); (2) type59 procedural redesign on the widened base (MODEL_SOURCE.type59 flip per t44 mechanics; userdrops7.js type59 region — FENCED vs lane H's amx rows; ztz85_iii `13f0c8d7` must NOT move). Last state: t62 widened, type59 redesigned, at "render the redesigned type59 and inspect". RESUME to finish inspection + gates ×2 + guards + report.
- **Lane H (a...79a, owner §5.309-order)**: amx30 + amx30b2 rebuilt on stripped amx40 base (no side skirts §B9, less side-turret armor, cast-round turret re-loft, TOP cupola/lights/20mm-coax/IR/B2 tells; amx40 + leclerc untouchable; userdrops7 amx rows only). Spawned minutes before the drop — status unknown; RESUME or respawn with the §5.309 brief.

### RESUME PROTOCOL (when network returns)
Per-lane: SendMessage the agent id (resumes from transcript); if "No transcript found", respawn with the notated brief above + the packet receipts on disk. FIRST ACTION each resume: marker-grep + git log -1 + re-verify own WIP md5 vs scratchpad snapshot (the network drop is exactly the external-interference window the snapshot law exists for). Landing law unchanged: §5.287 (&&-gated npm, checklists), stage-ledger-rows.py, bmpt row-drop orchestrator-side, changed-view re-certs for D/E, garage spot-reads for F/G/H.

### POST-SET QUEUE (unchanged)
Icon regen (k2b full set + every changed id, §5.246 clean-HEAD recipe) → §5.248 hook-closure entry w/ owner-refined scoreboard → orchestrator queue as listed in §5.309.
...
## §5.313 — t95 yaw90 orphan ring FIXED (owner task / chip task_3d06d29a); ariete slopes RE-CERTIFIED (2026-08-17)
- **t95 fix**: root cause = the FLEET fallback bore (muzzleBoreShadowFallback rim-torus/annulus/disc, tankFactory ~5026-5111) seats on the muzzle anchor, which stays inside rig_turret unless P.fixedMount — t95's tube is hull-bucketed so the trio orphaned 5m off-hull at yaw90. Fix = `P.fixedMount = true` in buildT95 (strv103 precedent; the flag's ONLY two factory consumers audited — it re-attaches muzzle+turretTop to hullG with world seats preserved). PROOF: rig probe (tools/tmp-t95-rig-probe.mjs) = rig_turret EMPTY of the trio; **hash UNCHANGED 88bf1858** (attach preserves world seats; geometry buffers untouched) — ratification + row bind unchanged; gate ×2 = 80.8/dims 100/floaters 100 EXACT HOLD; casemate guards ×4 byte-identical. FLEET NOTE: sturmtiger/jpz/isu casemates share the rig-contract pattern — their fallback bores also ride the rig (invisible in their sittings; a fleet fixedMount sweep for hull-tube casemates queued as a polish item).
- **ariete re-cert: BOTH PASS** — c1 9.1 re-binds @ `49c15299`, c2 9.2 @ `b0b3c184` (raycast receipts: glacis plane slopes 0.0638/0.1013 match briefed stations to 4 decimals; cheeks 41.1°/37.2° from vertical vs the old ~8°; c2 modules ride the rake ~0.05 proud; KEEPs held; re-pair correctly absent). Lane D LANDS with this commit (owner §5.299 item complete). tmp-hashgeo.html carries my one-line __TANK_ROOT__ export (probe instrumentation — kept, harmless).
## §5.314 — Type 90 owner-source turret correction (OWNER session entry) (2026-08-17)
- Rebound the comparison lane to the owner-supplied `type_90_kyu-maru_japan.glb` and corrected the authored Type 90's oversized turret envelope as one turret-owned assembly. The shell, gun saddle, bustle, roof stations, smoke banks, sight heads and shielded M2 now share the same low welded silhouette and remain attached through yaw; the established gun run is compensated inside the scaled assembly rather than shortened.
- Added broad backed cheek courses, source-readable roof service plates, weld lines, four-tube cheek smoke banks and a seated shielded roof weapon. The hull, skirts and suspension-driven running gear were deliberately left unchanged. Type 90A was checked as the inherited-family guard and remains coherent at the new base proportions.
- Final evidence contains **14 actual-source/procedural pairs + 14 yaw-0 + 14 yaw-90** frames for Type 90 and the same **42-frame** procedural determinism/ownership packet for Type 90A. Structural gates: duplicate course **2/2 PASS**, turret parent **0 stranded / 0 abutting / 0 dangling** for both, and exact strict track clip **front 0 / rear 0 / shoe 0 / sweep 0** for both. Selective Type 90/Type 90A assets were regenerated and the focused asset gate passes **2 tanks / 18 files** with muzzle bores verified.

## §5.315 — LANE G LANDED (t62 widen + type59 redesign); owner type90 correction gate-costed (2026-08-17)
- **t62mv1 `aa479400→ac414eaa`** (45/65597): all lateral stations ×1.10 (gauge 1.27→1.397, trackW 0.572, dome rings ×1.10 w/ sz 1.0273 so plan length byte-held), spec widthM 3.30→**3.63**. AABB receipt [3.30,4.1718,9.905]→[3.63,4.1718,9.905] — width exactly ×1.10, height/length byte-identical. Its oracle is OFFLINE (registration retired at 1a96cb49) — the harness hard-fails, so the frozen honest row 91.1 was RESTORED VERBATIM per the false-0 law after two error-0 runs; print divergence −9.1% by decree documented.
- **type59 `ea1ff837→a62bf43c`** (53/78371): §5.45 print-frame build retired (pointer comment); NEW buildType59 in profiles/china.js = buildT62Obr1975Chassis verbatim + Chinese dressing (WZ-120 mushroom dome, DShK census MG, Type 59-II searchlight RIGHT, 100mm w/ evacuator+bore, PLA palette). Row: the live before was min-0-dims-0 (the §5.259 silhouette-leak class — type59's spec inherited t62mv1's silhouette rows via the userdrops7 donor spread; stored 58.7 was stale); after ×2: min 0 | hull 32.5/whole 18.8/dims **100**/floaters 100 — owner-decreed-divergence-capped (safeScale inflates the 3.27 print +11% vs the widened build; ztz85_iii precedent). **Orchestrator item: t54 carries the same silhouette-leak** (no override in its patch). One real §B4 fix (46-voxel bow-pocket sheet, 4mm shim). Guards ×6 byte-exact.
- **OWNER type90 correction (a35ac3a7) gate-costed**: `43179448→d4a9410` (+4224 verts), row 83.9→**20.3** (dims 45.9, turret 20.3) ×1 — the owner re-proportioned the turret their way (the §5.281 08-12-flag ANSWERED: they want the taller read; this reverses the print-parity rebuild by their own hand). Staged honest; owner-decreed divergence class (t62-widen law: never chase the print back). **ASK-OWNER flag (non-blocking): type90's turret now reads far over published/print dims — confirm this garage read is the intent** (the §5.34 incident was the same visual class in the other direction).

## §5.316 — LANE F LANDED (four items); upior-REG task confirmed-fixed; kit.js TDZ race hardened (2026-08-17)
- **marder1a3 `694568→59cb105c`** (70/82601): old hull restored completely + §5.269 cast turret preserved byte-verbatim, ring re-seated at the old station (collar buried 0.030 under the roof plate); row 85.1→81.9 = the ordered old-hull-vs-print cost. **bmp3 `310b7f2e→8d9d7aa3`** (65/69974): the §B2 bow corridor CLOSED in pixels + instrument (worst view 89px vs the multi-thousand-px gap; certified donor class 1045px); twin PKT ball mounts at ±0.88; 902V/waterjets/grille/bins/snorkel/shackles/cable enriched; row 36.6→**40.2** hold-or-improve; bmp3_rok does NOT inherit (independent buildBMP2 path, byte-held 7456de28). **bmpt REMOVED** (owner §5.303): 9-surface de-wiring grep-verified zero dangling (roster 114, constructor throws Unknown-id; bmpt_terminator2 KEPT byte-held 1c7d8fbc; packet retirement note; gate JSON stays history) + **ledger row DROPPED orchestrator-side** (117→116 rows). **m3a3_bradley `b0eb98a1→2c5ce78c`** (64/83507): pre-wave base + wave equipment re-seated (TOW-at-depth LEFT bracket w/ handedness fix, CIV plinth, ISU, bustle; lid bug fixed in carry-over); min 0 lawful cap (print doesn't assemble), dims/floaters 100. Guards ×8 byte-identical; npm exit 0 live; recovery verdict: the dead agent's work was complete and honest — all claims reproduced independently.
- **Owner task 1 (upior turretYaw) CONFIRMED ALREADY FIXED at §5.286**: all three maps carry the removal, parser REG has no turretYaw, and a fresh loader box probe at the current registration reads z **[-2.561,+2.561] — numerically identical to the critic's certified-correct native BEFORE state** (gun forward inside the span over the shackled bow; the chip's "z-max grows" acceptance extrapolated from the mirror — the short gun never overhangs the bow tip). §5.286's ×2 re-gate stands.
- **Owner task 2 (kit.js TDZ flake) FIXED**: the catch-side queueMicrotask now try-guards its tick (the §5.309-flagged authoritativeMatch race — a microtask firing while KIT sat in TDZ mid-import-cycle); fitting behavior unchanged (census §B3 wiring intact); **npm test ×3 consecutive exit 0, zero TDZ crashes**.

## §5.317 — OWNER ORDER (verbatim, 2026-08-17): t95 complete redesign + NEW strv103a; lane status answered
- Owner: "are all lanes complete? also we need to completely redesign and update the t95 and add a new strv 103A" (+2 GLB drops). ANSWER GIVEN: 6/8 landed; E (leopards+kf51b) and H (amx pair) still live.
- Drops onboarded LOCAL-ONLY (gitignored): t95_world_of_tanks.glb 14c576b5… (extraction-suspect, owner-sanctioned reference) + strv_103b.glb e0b09973… (provenance TBD by lane census). ATTRIBUTION sections written.
- **LANE I spawned**: t95 COMPLETE REDESIGN vs the new print (oracle-swap class: census → register maps → honest re-baseline vs the new print → ground-up redesign at leclerc bar; the doomturtle print retires to history; t95 currently 88bf1858/80.8/ratified-9.2 + the §5.313 fixedMount fix must carry).
- **LANE J spawned**: NEW id `strv103a` (Sweden) vs strv_103b.glb (visual+measured reference; the A-model distinctions vs the reverted strv103 B: no flotation-screen rim, earlier engine deck, A-era fittings — family distinctness strv103a ≠ strv103 at a glance); full wiring checklist per §5.287 (tier/labels/markings/roster); sweden.js free post-§5.305.
## §5.318 — LANE H LANDED: amx30 pair rebuilt on the stripped amx40 base (owner §5.309) (2026-08-17)
- **amx30 `→33b5048`** (58/70051, row 90→83.8) + **amx30b2 `→88801828`** (60/72085, row 90.1→82.6): amx40 grammar stripped per order (skirtless §B9 five-wheel train, flank cassettes deleted, 20-station shell Chaikin-smoothed to a 5-ring cast-round loft w/ domed cap + AMX bustle taper), TOP cupola + remote 7.62, twin guarded bow lights, 20mm M693 coax in a bulged rotor cheek, PH-8-B gun-linked; B2 tells (COTAC, LLLTV, bins, smoke, 2nd whip). Specs trued to published 6.59/9.48/3.10/2.29 + measured silhouettes. Rows ×2 = owner-decreed divergence vs the ahab print (§5.304 class; prints stayed registered, gates ran honest; the 90-class print-shaped rows retire with the old builds). Guards amx40/leclerc/leclerc_xlr/amx56 exact; one real §B4 fixed (glacis wrap-lane split); npm green. **§3 graduate rows re-bound by owner authority** (89a4cdf0/34076800 retired). Garage spot sitting spawned.

## §5.319 — OWNER ORDER (verbatim, 2026-08-17): "finish the left side of oplots turret" (+screenshot)
- Screenshot shows the left turret flank bare (flat camo + one sensor box) vs the §5.288 right-wing Duplet terraces. Scoped lane spawned: left-side print-parity finish (Duplet courses around the gunner-sight zone per the PRINT's left — asymmetry law, not a blind mirror; p95 discipline per the §5.288 tilted-brick + roof-datum receipts; ua_ guards untouchable; hold-or-improve + dims-100). Five agents live: E-recovery, I (t95), J (strv103a), amx sitting, oplot-left.
## §5.320 — Merkava source-anatomy convergence (OWNER session entry) (2026-08-17)
- Compared all six rostered owner marks (`merkava1b`, `merkava2b`, `merkava2d`, `merkava3c`, `merkava3d`, `merkava4b`) directly against the supplied nested source archives while keeping every playable vehicle first-party procedural. Mk.3B remains owner-retired and was used only as a family reference; deleted Mk.4M/Windbreaker was not restored.
- Preserved the authored family's strong low wedge/cast turret character, then corrected the source-specific proportions: shallower turret height, longer cheek slopes, mark-specific plan insets, and one broad turret-owned gun cradle whose buried shoulders, oval throat, collar, crown/chin courses and thicker tube form a continuous gun line. The former deep applique boxes were reduced to shallow seated cassettes so the primary turret surface remains visible rather than becoming a second box turret.
- Added a supported source-readable stern field to each mark: paired backed radiator/service bays, unequal hinged access doors, protected marker lamps, recovery eyes, tow hardware and a roof-edge cable saddle. All rear details remain hull-owned; every turret shell, cradle, roof station, bustle rack, ERA course and antenna rotates coherently through yaw.
- Final evidence contains **14 actual-source/procedural pairs + 14 yaw-0 + 14 yaw-90** frames per mark (**252 frames total**). Exact strict clip is **0 front / 0 rear / 0 shoe / 0 sweep** for all six; duplicate-course audit is **6/6 PASS** with one suspension-driven tread/connector course each. Focused assets were regenerated for the six tanks (**54 files**) and pass `tank-assets-check` with metadata, geometry and muzzle bores current.
...
## §5.323 — OWNER ORDER (verbatim, 2026-08-17): "the upior ifv (id upior)'s hull is backwards, fix and move its turret accordingly so it stays in same place on hull but still faces right direction"
- The owner's garage observation outranks all prior receipts (the §5.279/§5.286 chain corrected the REF registration; the PROC build's own facing is now in question). Scoped lane spawned: establish the pixel fact FIRST, flip the hull at the STATION level (not a π group wrap — §B4/bucket law), re-seat the turret at the same hull station with the gun forward, re-prove §B5, re-gate ×2 (the flip may RECOVER curve points vs the nose-forward ref). afvFamily guards ×9 untouchable. Seven agents live.

## §5.324 — LANE E RECOVERY DELIVERED (all three items); germany trio sitting live (2026-08-17)
- **leo2a4m `→dd1de614`** (58/114107): wave hull+gun kept verbatim, wrapper-era turret spliced (donor A4 loft from b66d6d03^), the recovery reworked the r1-class hardware line into the 3-column p95 budget (plate/collar + low C6, capped drums, vertical SEMs, sunk FLW200, compacted PERI) — gate repaired 0(heightM-3.30-class)→**86.2 ×2 (dims 100)**; the −3.3 vs 89.5 = the ordered turret's honest cost. **leo2a6m `→e004f4d8`**: prior agent's five finish items verified, zero recovery edits, **91 PASS ×2 hold-or-improve**. **kf51b `→b5d8ab4e`**: integration finished (track-clip 311/129→0/0 via tub/sponson re-seats, turret-parent 0/0/0, duplicate-course PASS, garage read captured, FALSE-0 respected; b3d15714 absorbed-verified). Guards ×8 exact start+end. npm exit 0.
- Recovery finding: the dead agent's WIP was further along than §5.311 estimated (parse already fixed; 2a6m complete; kf51b substantially done) — the recovery's real work = leo2a4m's gate repair + kf51b's audits/garage + packets. Combined germany trio sitting LIVE (re-certs for the two ratified surfaces + kf51b order-delivery). Landing after its verdicts.
...
## §5.329 — OWNER ORDERS (verbatim, 2026-08-17): leclerc three-item graduate-change round
- THREE leclerc orders w/ surface-markup receipts folded into one lane (the FLAGSHIP graduate, live 733d24fd, §3 frozen 683be340 pre-§5.229-class): (1) "its lower glacis is really pushed back and doesnt line up with the upper glacis. fix" — markup: hull faces 804/805, the lower plate at z 2.95-3.02 vs the beak at z 3.58 = ~0.56m recessed step; re-loft to the continuous real-Leclerc nose; (2) "make its rare wheel much larger and move it up a little without deleting any tank parts" — markup: gearEndWheelHardware, current radius ~0.225 at y-center 0.84 — enlarge toward the §B6 raised-drive class + raise the axle, track wrap re-derives, DELETE NOTHING; (3) "the leclerc turret and hulls are also a little see throughable, just make them filled up roperly" — §B2 sweep + armor-grade closures, variants inherit through shared builders. ONE graduate-change landing (fix → gates-or-evidence → changed-view receipts → re-freeze note). Eleven agents live.

## §5.330 — OWNER ORDER (verbatim, 2026-08-17): "make the terminator 2 much better and have 2 shooting holes for both its barrels (create this in code) with a super fast reload" (+screenshot)
- Screenshot: bmpt_terminator2's twin 30mms carry only ONE fallback bore (the tankFactory pass installs exactly one + suppresses authored bores — twin-gun tanks can't show two mouths). Folded into the afvFamily lane as item 3: (1) OPT-IN twin-bore core knob in tankFactory (spec.gun.muzzles offsets → one assembly per tip; ABSENT-PARAM BYTE-IDENTICAL w/ sentinel hash proof — the §5.279 pattern); (2) wire the clone's both tips (§B3.1 ×2); (3) quality round to the §5.269-critic bar (the removed bmpt's grammar receipts = the map); (4) gun.reloadS to autocannon-class "super fast" (spec, npm-gated). Baseline 1c7d8fbc. Eleven agents live (the afvFamily lane now carries four items).
...
## §5.333 — LANE J LANDED: strv103a NEW id (owner §5.317) (2026-08-17)
- **strv103a `281b67ac`** (44/63595): gate **88.6 ×2 / dims 100** (heightM 2.16 +0.74%); PROVENANCE CLEAN — CC-BY-4.0 "Strv 103B" by BFJFFK (Sketchfab, embedded tags, zero rip fingerprints; ATTRIBUTION updated from TBD). Full §5.287 wiring (spec variant tier 8, profiles+SWEDEN_PROFILES, tier/labels/markings, fleetOrder 81→103A→103B→122, rosterPolicy ×2, decorations, HULL_ONLY_SHADOW_IDS, icons ×9). §B5 fixed-mount rig-probe: rig_turret EMPTY + bore trio under rig_hull per §5.313; yaw-90 0/44 moved. Guards strv103 4c8f1330 UNMOVED (verified through lane-I casemate churn) + centurion3/leo2a5. Documented caps: mast-clamp instrument frame (print's fused whips trip the ×1.30 loader clamp — §E mast-excision candidate filed, strv81 class) + the ordered A-read nose divergence (dozer-less beak vs the print's fence, worst views 88.6-88.8 front quarters). Solid-olive A distinct from the splinter B at a glance.
- MAPS HELD (fidelity/evaluator/critic/vertex carry lane I's in-flight t95 rows — they land with lane I or a consolidated sweep; strv103a's map rows ride there; reproducibility note banked). Garage identity sitting spawned. Cross-lane hygiene noted: the lane restored its accidental fidelity-report rewrite from HEAD w/ receipts.
## §5.334 — Merkava gun-housing family rebuild + Mk.4B running-gear rebuild (OWNER session entry) (2026-08-17)
- Preserved the accepted low cast/wedge turret silhouettes from §5.320 and replaced the bad mechanical gun roots on all six rostered marks (`merkava1b`, `merkava2b`, `merkava2d`, `merkava3c`, `merkava3d`, `merkava4b`). Each mark now has one short tapered oval canvas/steel boot buried into the turret cheek, with overlapping socket, throat and cinch courses. The complete housing is `rig_gun`-owned, so the boot, collar and barrel elevate and yaw as one; the legacy rectangular/prismatic mantlet paths are bypassed for these marks.
- Rebuilt Mk.4B running gear as one low dense suspension-driven course: six compact pressed-steel road wheels, smaller source-proportioned end gears, tighter shoe pitch and darker coherent wheel/track materials. The new end-wheel color is opt-in on the canonical spinning meshes; no static cover wheel or second tread layer was introduced. Skirts and armor remain intact.
- Final evidence contains **14 source/procedural pairs + 14 yaw-0 + 14 yaw-90** frames for every mark (**252 frames**). Exact strict track clip is **0 front / 0 rear / 0 shoe / 0 sweep** for all six and duplicate-course audit is **6/6 PASS**. Turret-parent audit reports no gun-housing, roof or Mk.4B offender; its two legacy flags are the broad hull-owned `hullGlass`/`hullDark` buckets on Mk.1B/Mk.2D. Selective icon, silhouette, armor, hit-zone and module assets were regenerated. Full `npm test` and the public production build pass after rebasing onto current main.

## §5.335 — LANE E LANDED on triple-PASS verdicts (§5.324 sitting) (2026-08-17)
- **leo2a4m `dd1de614` ORDER-DELIVERED re-bind** (hull+gun verbatim-HEAD at diff level, wrapper turret verbatim vs buildLeo2A4, ring closed, §B5 full-package unity, all six hardware-line items read; gate 86.2/dims 100 reproduced BYTE-IDENTICAL by the critic's own run — the honest ordered-turret cost). **leo2a6m `e004f4d8` RE-CERT PASS 9.3-class** (all five finish items at the coded stations incl. the ISAF placard bridging the rail gap; 91/100/100 reproduced byte-identical, hold-or-improve). **kf51b `b5d8ab4e` INTEGRATION-DELIVERED** (facet shader deleted in-diff, two-band skirted course w/ §5.262 tones, census kit, b3d15714 absorbed-verified, X KF51B garage card; FALSE-0 ledger-absence verified). Family strip = 7 distinct German vehicles. Guards ×4 byte-held ×5 sweeps.
- LANDED: leopard.js + germany.js + userdrops3.js + packets ×3 + gate JSONs ×2 + rows (leo2a4m 86.2 honest / leo2a6m 91). Owner merkava second wave (22ef740c gun housings + mk4b tracks) merged — extends the §5.321 no-gate stale-row class. The §5.299/§5.303 germany orders are COMPLETE.

## §5.336 — OWNER ORDER (verbatim, 2026-08-17): "make the type 10s larger make their tracks much better using our better track system and make their hulls and turrets much mcuh beter"
- Type10-pair lane spawned on the §5.308 state (type10 a1cecea6 reverted-old + type10b 77870ef0 pinned): owner-decreed enlargement (scale judged vs type90 side-by-side, dims re-derived + documented as §5.304-class divergence), fleet §B6 smart track/wheel course adoption (amx/kf51b exemplars), leclerc-bar hull/turret quality with the B-variant delta preserved. NOTE: the order supersedes the §5.299 type10b byte-pin by owner authority (documented). type90 d4a9410 + japan guards untouchable; modern3 ariete regions fenced. Twelve agents live.

## §5.337 — package.json conflict window CLOSED; test-chain union (2026-08-17)
- The owner's aceddcc8 (props/hitboxes + propPlacement.selftest in the test chain) collided with a lane's uncommitted type99Armor.selftest test-chain registration on the same giant line — the 3-way re-apply left markers for ~2 minutes (every lane's npm broken in the window). Resolved by union: the owner's committed line + the lane's type99Armor entry injected before tankAssets; JSON validated; both selftests present. The type99Armor line stays worktree-side until its owning lane lands (attribution: unknown lane — likely the china-armor class; verify at the next afvFamily/china landing). Pushed through 84a86901.

## §5.338 — oplot-left LANDED (owner §5.319) (2026-08-17)
- **ua_t84_oplot_m `d7d068be→6e76802a`** (66/96743): the left turret finished to print parity — 3-cassette Duplet cheek wall at the print's max-width plane (the screenshot's gap) + junction panel + 3 terrace lids + left smoke bank at the print shoulder (top capped ≤2.28, PNK-6 keeps the only >2.285 window) + wall kit density + bustle flank bin + gunner-sight zone grown to its print station. All authored s=−1 only — the §5.288-ratified right side byte-stays. Row: whole 35.3→**37.1 IMPROVED**, stations 68.5 held, **dims 100 HELD** ×2. Guards ×5 byte-held; §B5 left kit rides the turret; pre-existing §5.272 bow-hole class zero-delta (swap-run receipt); npm green twice incl. post-§5.337-union. The 9.3 ratification re-binds on the §5.254 receipts (scoped-order class, bmp3-gap precedent).
...
## §5.341 — type59 fix + IFV-sweep fixes LANDED (2026-08-17)
- **type59 `a62bf43c→ea3494b4`**: THE BULB root-caused = the Type 59-II searchlight assembly (not the mantlet — old drum at gun-local −0.05 emerged tangentially from the dome cheek as a crescent, pale gunMount §C carrier, mantlet collar buried inside the casting). Fixed: ruBoot canvas-wedge collar mantlet + searchlight re-seated dark/proud/gun-linked RIGHT; MGs added (DShK real ring-mount read at 2.57 < the 2.59 datum — zero new p95 columns; coax port gun-bucketed elev-proven; bow MG port inside the track band). Rows hold-or-improve ×2 (whole 19.0, stations 43.2, dims/floaters 100 HELD); chassis guard ac414eaa byte-untouched.
- **IFV sweep (§5.326) LANDED**: 16 ids probed, findings table + priority handoff on disk. FIXED ×3: type89 bow sprocket-bay windows→0 (`f7c1410`), fv510 fender-shelf corridors 4157→82px (`55fac760`), spz_puma bow-quarter corridors 2561→29px (`93e23240`, gate ×2 hold-or-improve isolation-proven — the dims-92.2 drift + 7758px -T read PRE-EXIST, banked as foreign findings). DEFECT handoff for the afvFamily lane: upior corridors (pre-flip data — apply post-flip), marder MILAN pocket, m3a3 ring slit, ua_m2a3 rear-shelf window + the shared donor-bow grammar; float re-seats for the clones. Ten untouched IFV ids byte-proven.
- **FOREIGN STREAM ATTRIBUTED (§5.337 mystery closed)**: the type99Armor.selftest package.json line + the untracked selftest + type99a/china.js WIP (live f184fdc4-class vs clean 7f613788) = an active type99-armor work stream (owner/parallel session); its selftest FAILS on its own WIP (ring seat 1.4436 vs 1.4) — live-tree npm red is THEIRS; clean-worktree gates unaffected (HEAD package.json carries no union line).

## §5.342 — t90sm FLUSH PANELS + FLOWING CAGE DELIVERED (owner §5.331, GRADUATE-CHANGE BY OWNER AUTHORITY) (2026-08-17)
- **Item 1 (three LEFT-flank vertical panels, faces 592/593 · 604/605 · 600/601 — the §5.74/§5.79 abrams panel-pitch class)**: all three marked constructs attributed to authored geometry in `buildT90SMLegacy` (profiles/t90.js — the §5.75 split moved the t90sm region out of russia.js) and re-seated ONTO the turret's own side slope (prism wall rake 40.0°), both flanks, print asymmetry preserved. Pitch receipts (° from vertical, before→after): forward cheek wedge (592/593) **0.0→33.9**; main cheek cassette outer face (600/601) **9.7→36.9** (top ring raised 0.44→0.50 to carry the retired carrier-box mass); its exposed standing inboard wall (604/605) **7.5→33.6 and BURIED** (top ring 1.10–1.16 vs shell 1.19–1.21 — the air gap behind the panel is gone, back sits flush). Family coherence: outer shingle course 11.0→45.8 L / 9.5→42.5 R (tucks under the main face, 3 cm shingle lip), flank transitions 7.8→33.8 / 6.7→39.5, left roof-edge cassette 0→24.7 w/ lid re-seated. RETIRED: the T4S brim apron (its faked flare is now the genuinely pitched shingle stack) and the r12 ±1.51-col casting-wall carrier boxes (vertical-stance mask carriers — they'd poke through the flush planes as fins; front-mask delta re-gates at print restore). EVERY bottom ring/lip byte-unchanged (plan/width cols + the 1.860/1.820 extents + 1.890 anchor court); whole-tank AABB byte-identical before/after ([−1.89,−0.0348,−3.435]..[1.89,2.245,6.273]) = dims hold exact.
- **Item 2 (cage zig-zag, turretDetail 1840/1841 = the old join-3 L (1.172,−0.98)→(1.292,−1.06) dz −0.08 REARWARD at rail y 0.47)**: the four per-cell rail runs + doubling-back joins (dz −0.18/−0.08 while stepping outboard, +0.05 overshoot pads, join-1 landing 27 mm past the cell rail end) re-authored as **ONE continuous monotone shared-vertex polyline per side per rail height** (9 vertices: rear-grille corner (0.67,−2.51) → 40.5° corner chamfer → aft run → 29.4°/32.3°/32.9° chamfer diagonals riding the cell faces → front end (1.4245,−0.40); z strictly forward, x strictly outboard — the leclerc/kf51-class flowing basket line). Even post rhythm (~0.3–0.4 m: bends + long-run midpoints, replacing the irregular 0.13–0.36 per-cell stiles) + a corner stile pair tying the sweep into the rear grille frame. Print asym preserved (left aft to −2.08, right −1.96); cell backings + buried carrier feet unchanged (§B2 closed — b2-rear-low proves no sky-through). Numeral seats moved z −0.32→−0.05 (§5.266 clip law: the auto-reseat pinned them 10 cm deeper and the cage front post eclipsed the rear digit; "527" now reads whole on the pitched cheek, both flanks, all four owner cams).
- **Gate/evidence state (§5.265 law honored)**: t90sm print is .bak-only (`recovered/t90sm.glb.bak`, no live GLB; vertex JSON registration live but un-renderable) — **NOT GATED** (a run would write the false-0 row). Evidence = §5.254 before/after pairs at the owner's four markup cameras ([1.56,6.35,12.55], [0.14,5.97,12.83], [−8.18,6.25,−6.41], [−4.30,7.86,−8.10], target [0,1.11,1.40], fov 36) + flank/cage closeups + §B5 yaw-90 (panels, cage, cells and numerals all rotate as one; hull kit stays) + §B2 rear-low → `shots/t90sm-flush/{before,after}/` (before captured at the pre-edit tree, byte-distinct). Ledger row (90.2/90.3/90.9/90.0/100/100) untouched; re-gate ×2 hold-or-improve OWED at print restore (fleet re-freeze sweep). §3-registry note: the t90sm row's `7efc69c9` freeze hash is pre-§5.229-stale (current-tree pre-edit baseline was 86454aa8); this landing moves it **86454aa8→`17b8330`** (50 meshes / 91,281 verts) — row update belongs to the queued fleet-wide re-freeze sweep.
- **Guards (hashgeo ×3: baseline, post-panels+cage, final)**: t90m 3f240690 · t90a e4d5a565 · t90a_burlak 86aefb82 · t90ms 68539120 · t90 6908d1a8 · pt91m 7738f7a2 · t90a_vladimir 45387624 · t72b3m 6d747b34 · **t64bv1 eabf99cc** · **t62mv1 ac414eaa** (order-pinned, shared-file) · t54 cedb8be8 · t44 72af6298 — ALL byte-held every batch. Diff surface: 8 hunks, all inside `buildT90SMLegacy` (t62 chassis region + russia.js untouched). Track lane untouched (zero hull/gear/track lines in the diff; track-geometry.selftest + identical AABB prove it). npm: every chain link green with these bytes (tankAssets 115, vehicleMarkings seats/yaw, track-geometry, world/ui suites) EXCEPT the §5.337-attributed foreign type99Armor selftest, re-proven foreign by counterfactual (fails identically with HEAD-bytes t90.js). Delivered UNCOMMITTED-UNSTAGED per order (PROGRAM-STATE edit unstaged over the foreign staged index — §staged-index law, no `git add`).

## §5.342 — LECLERC three-item round LANDED via hunk-surgery (owner §5.329) (2026-08-17)
- **leclerc `733d24fd→e6523de8`** (48/98207): (1) lower glacis re-lofted ONE continuous plane (0.27,2.95)→(1.21,3.46) sharing the beak edge — the 0.44m recessed wall gone, shackles re-seated on-plane; (2) rear sprocket r0.14→**0.24** @ y0.84→**0.92**, wrap re-derived §B6, far-edge byte-identical, NOTHING deleted (XLR cell floor cleared); (3) §B2 fills w/ real geometry (bow 288→22px, stern slivers→0, every survivor pixel-classified honest). Variants flow: leclerc_xlr `1f64e4c`, amx56 `71e39be`. Whole-AABB byte-equal x/z/top = dims hold. Oracle .bak-only → NOT GATED (§5.265); evidence = both owner cameras + §B2 sweeps + AABB receipts. track-clip 0/0 strict ×3.
- **LANDED BY INDEX-BLOB HUNK SURGERY**: misc.js's 18 worktree hunks split 9 leclerc (1063-1912) / 9 amx-lane (3515+, incl. their §5.339 deletion region); the committed blob = HEAD + leclerc hunks ONLY (worktree-applied, parse-checked, hash-verified: leclerc e6523de8 + amx pair UNCHANGED 33b5048/88801828 in the index tree). The amx lane's WIP stays worktree-side untouched — their landing proceeds normally. Live-tree misc.js keeps both lanes' hunks.

## §5.343 — T90SM flush-panels + cage-flow LANDED (owner §5.331) (2026-08-17)
- **t90sm `86454aa8→17b8330`** (50/91281; the build lives in profiles/t90.js per the §5.75 split — russia.js untouched, the order's file guess corrected): all three marked verticals pitched onto the 40° side rake (0.0/9.7/7.5° → 33.9/36.9/33.6°, the standing wall BURIED w/ its air gap closed), family coherence across shingles/transitions/roof cassettes, T4S brim + r12 carrier boxes retired (would read as fins through the flush planes); the cage re-authored as ONE shared-vertex monotone polyline per side (chamfer 40.5°, zero overshoot, post rhythm even) + the '527' numerals unclipped (§5.266 law). Whole-AABB byte-identical = dims hold; §B5 pixels-proven; guards ×12 byte-held incl. t62mv1/t64bv1. Graduate-change by owner authority; .bak-only oracle → evidence-not-gate (§5.265); re-gate ×2 owed at print restore (banked).

## §5.344 — amx §5.328 round LANDED: cupolas/MGs/decor + B2 Brénus ERA + dead-code deletion (2026-08-17)
- **amx30 `33b5048→33b0dbdc`** (83.8 EXACT hold ×2) + **amx30b2 `88801828→e34a5d1c`** (82.8 +0.2 ×2; dims 99.2 held exactly — 72 GIAT G2 bricks moved width 0.0, §5.263 proof). TOP-7 cupola = the roof landmark (ten-episcope belt, sight hood, domed split lid; silhouetteHeightM honestly re-derived 2.82 ×2 through the mounted CLAMS per the t62mv1 convention); CLAMS reads as a gun (census pintleMG + banana feed + print-supported AA arc rail); French decor grammar + §5.332 notes folded (discrete smoke tubes — the 2-vs-3 era-tell reads; PH-8-B lens re-faced; crown seam dressed); **B2 Brénus: 42 glacis + 8 nose (hull-fixed) + 22 turret bricks (rotate as one, §B5 receipts) on the native strippable eraCluster mechanism**; B2 glacis spare-links dropped w/ documentation (Brénus owns the plane). §B4 0/0 ×2; census mg1+8d ×2; npm honest unpiped exit 0.
- **buildAMX30Legacy DELETED (364 lines, grep-0)** — the §5.339-routed chip closed inside this landing. Guard: type90 d4a9410 EXACT ×4; amx40's 1a74c63c→ed5d23b2 move attributed to LANDED commits (decorations/tankFactory churn — strv103a's decorations edit class), not this lane.
- Icons note: the parallel icon regen captured amx30/amx30b2 at pre-§5.328 bytes — stale; the final §5.246 regen covers them.

## §5.345 — strv103a sitting PASS; owner leo2a6m-shambles + kf51b-descent orders (2026-08-17)
- **strv103a garage sitting PASS — binds 281b67ac** (49 sheets: S-Tank DNA, A-reads proven vs the print incl. the ordered dozer-less divergence, handedness correct, §B5 diffPx-0 independently re-measured, §B2 clean, garage card + carousel VIII placement, icons 1:1). NON-BLOCKING: the fa073ec0 landing shipped icons w/o the tank-assets.json manifest row (tool-gate only; folds into the final §5.246 regen commit — the parallel icon-regen stream's manifest carries strv103a).
- **OWNER (verbatim): "are we properly updating our leo 2a6m? its still in shambles visually." + "the turret front is jsut incomplete and misshapen."** — MY OWN RENDERS CONFIRM (shots/leo2a6m-check/): the cage dominates (frame-with-a-tank-inside), the turret front reads as a blank slab w/ box clutter at the gun root, the turret sits buried. GESTALT LESSON banked: garage-weighted sittings can verify items individually while the whole fails the eye — gestalt verdicts join the sitting protocol. Routed to the leopard lane as its PRIORITY item (front re-loft = the core; ratified finish items re-seat on the corrected gestalt).
- **OWNER (verbatim): "completely update our kf51 b to look actually descended from our leopard family, keeping its general turret shaping but changing everything else in terms of hull decorations equipment and cagfes"** — routed to the same lane as item 3 (leopard-hull grammar rebase, German kit sets, ISAF-class balanced cages, turret shaping kept; kf51 resident untouchable).
- The leopard lane = three items (leo2a4m §5.328-class round, leo2a6m gestalt, kf51b descent). Landings this hour: leclerc §5.342 (hunk surgery), t90sm §5.343, amx §5.344, type59+sweep §5.341.

## §5.346 — LANE I LANDED: t95 COMPLETE REDESIGN vs the WoT print (owner §5.317) (2026-08-17)
- **t95 `88bf1858→ce210ed4`** (41/80499): census/provenance = **viewer-rip fingerprint confirmed** (T95.obj.cleaner.materialmerger.gles — extraction-suspect LOCAL-ONLY quarantine, ATTRIBUTION class held); the print is z/y-true (hull 7.766≈7.6, muzzle→11.11) but stands in FIGHTING TRIM 4.565 wide vs the 3.86 shipping datum (ref ×0.8456, two-datum note); doomturtle rows retired inline in all four maps; specs widthM 3.8→3.86 (ordered datum). **NEW opt-in loader flag `maskFloorOracle`** (t95 sole consumer; absent-param identity): near-black rip albedo + inward-wound faces read as mask HOLES — constant emissive floor + DoubleSide fixes the ref masks (measured receipts).
- Honest re-baseline 80.9/dims-87.9 → **FINAL 84.9 ×2 (dims 100** — heightM 0.87%/hull 0.13%/overall 0.07%/width 0.07%; +4.0; ≥90 not print-permitted under the 3.86 datum, §B7 caps documented across 7 receipted ladder iterations). §5.313 fixedMount CARRIED (probe: fallback trio under rig_hull, rig_turret=[rig_gun] only). track-clip 0/1 = the sanctioned quad-run waiver class. Guards ×4 casemate residents EXACT ×2. npm: type99Armor failure counterfactual-proven foreign.
- LANDED: casemate.js (buildT95 rewritten 324 lines) + specs.js + the FOUR MAPS (finally landing the held shared-map state incl. strv103a's §5.333 rows) + reference-glb-loader.js (maskFloorOracle) + packet + gate JSON + row (80.8→84.9). Identity sitting SPAWNED.
## §5.347 — Merkava armored gun-mask correction + Mk.4B structural rebuild (OWNER session entry) (2026-08-17)
- Corrected the §5.334 over-simplification that treated the Merkava gun root as only a short oval boot. Every rostered mark now carries the iconic broad faceted armored mask: buried cheek-return wings flow into a closed polygonal crown/chin cradle, a backed oval tunnel is recessed in its face, and the short tapered flexible boot contracts around the gun inside that tunnel. The complete load path is `rig_gun`-owned and remains connected through elevation and turret yaw.
- Rebuilt Mk.4B around that larger mask rather than adding another decoration pass. Its outer arrowhead modules now form five short overlapping swept courses with a true lower undercut, its service bins are seated against the bustle, its roof stations follow the measured wedge roof line, and its running gear uses six source-spaced road wheels with smaller end gears inside one suspension-driven smart-track course. Existing skirts and hull armor remain intact.
- Final visual evidence contains **14 source/procedural pairs + 14 yaw-0 + 14 yaw-90** frames for every mark (**252 frames**). Duplicate-course audit is **6/6 PASS**; exact strict track clipping is **0 front / 0 rear / 0 shoe / 0 sweep** for all six; Mk.4B turret ownership is **0 stranded / 0 abutting / 0 dangling**. The two legacy parent-audit flags remain broad hull-owned Mk.1B `hullGlass` and Mk.2D `hullDark` buckets, not changed turret parts. Selective assets were regenerated for the six tanks (**54 files**) and pass geometry, metadata and muzzle-bore validation.

## §5.348 — ariete-MBT DELIVERED (re-cert live); t95 landing notes; specs hunk-split lesson (2026-08-17)
- **ariete-MBT (§5.322) DELIVERED**: c1 `49c15299→4e04e814` (51/74855) + c2 `b0b3c184→2703f8d0` (53/80843) — coax MG 42/59 collared aperture + commander pintle at TURMS (nested under the certified columns after two measured rejected sweeps) + loader kept stowed; §5.283 debts folded (rack-end caps INSIDE the rail envelope after the −14.7-plan ref-extension revert); c1 7-section skirts + hardware ≤1.799 width-guard; §5.262 gear tones. c1 rows ×2 hold-or-improve (stations +0.6, dims 100 HOLDS); c2 never gated; §B5 delta ZERO; guards carro/donor EXACT. INSTRUMENT INCIDENT documented: mid-round live-gate pollution proven via the byte-held carro guard moving 91→89.9 — verdicts re-derived clean-worktree, live ledger repaired. Changed-view re-cert sitting SPAWNED.
- **t95 landing (§5.346) completed via a SECOND hunk surgery**: the worktree specs.js carried the foreign type99-armor stream's combatAnatomy import (untracked file — the clean-worktree gate caught ERR_MODULE_NOT_FOUND); the landed blob = HEAD + lane I's 1498-dims hunk ONLY. LESSON (extends §5.287): shared SPEC files get the same hunk-audit as profile files — a foreign import of an untracked file is invisible until a clean gate runs. t95 identity sitting SPAWNED.

## §5.349 — OWNER ORDER (verbatim, 2026-08-17): "the bradleys are still not filled internally (see througable) and their side skirts/side armors are not attached to the hulls properly or with attachments"
- Merges the §5.341 IFV-sweep handoff into the afvFamily lane as MANDATORY (m3a3 ring slit + bow window; ua_m2a3 rear-shelf window + bow + ISU pocket; m2a2 bow slit beyond the certified tolerance — the owner's eye outranks it; ONE shared donor-bow closure in the bradley base, type89-closure model) + EXTENDS the §5.325 skirt-attachment treatment to ALL THREE bradleys (real hanger/rail + hinge/bolt hardware per panel, flush seams, §B9 parity). m2a2 + ua_m2a3 move by order (baselines recorded); the lane's scope is now: upior flip, terminator2, bradley fills+skirts ×3.

## §5.350 — oplot right wall + ua-t80 pair rebase LANDED (owner §5.340/§5.341-order/§5.348-ext) (2026-08-17)
- **ua_t84_oplot_m `6e76802a→66fc1724`** (66/100085): right cheek wall finished per the print's OWN right (measurably asymmetric — right face 1cm shier, NO tall right smoke cluster; low shoulder ledge + stowage instead, asymmetry law honored); whole 37.1→**38.2**, stations held, dims 100 held. §5.338 left + right wing byte-untouched.
- **ua_t80bv `554591b8→1e175410`**: dome rebased to the resident buildT80Line BV ring profile (sz 0.88, certified gun axis held); T-90-read front (t90a eraRuCheeks clamshell grammar + varied brick course); full-run K-1 skirts 8/side + top ERA strips + rubber fore-sections; **dims 95→97.4 IMPROVED (heightM 2.20 exact)**; whole/turret/stations drops = the ordered russian-dome-vs-tall-print cost, released by order + documented. **ua_t80u_kursk `1332bd55→fe3c4dc8`**: v1 ring profile rebase; dark steel clamshells + mixed varied course + re-seated V-tips; 6/side skirts; **dims 100 held**, whole 81.7 (−0.9 released). Two self-caught regressions fixed w/ receipts (idler-lane toe course 34/16→0/0; stern panel §B2 pockets→0). §B4/§B5/§B2 clean ×3; §5.263 widths unmoved; russian residents + guards byte-held ×7; npm green live.
- The §5.319/§5.340 oplot orders + the §5.341-order/§5.348-ERA-extension are COMPLETE.

## §5.351 — ariete-MBT round LANDED on double RE-CERT PASS (owner §5.322) (2026-08-17)
- **ariete_c1 `4e04e814`** + **ariete_c2 `2703f8d0`** re-bound (125-frame sitting; the live-tree hash mismatch DIAGNOSED as the §5.348 foreign-churn pollution signature and the canonical frame reproduced in a detached worktree — all four hashes EXACT; amplified pixel-diffs prove changes confined to the ordered surfaces w/ dark silhouette boundaries). MGs (coax aperture gun-bucketed in the pitch pair, commander pintle nested, loader stowed; census mg2+4d/mg2+5d), turret density at the exemplar bar, c1's exactly-7 skirt panels w/ hinge hardware vs c2's 13 (distinctness proven side-by-side), §5.262 gear reads crisp. Rows: c1 stations 85.8/dims 100 staged; c2 never gated. The §5.322 order is COMPLETE.

## §5.352 — t95 redesign RATIFIED 9.2 (sitting PASS at ce210ed4) — the §5.317 t95 order COMPLETE (2026-08-17)
- All redesign claims verified in critic pixels (54 sheets): quad-track census-proven, one-piece casemate, T5E1 bore, **§5.313 fixedMount probe state HOLDS w/ yaw-90 diff-0 ×2 cameras (the §5.268 orphan-torus chip class closed)**, towers/M2/travel-lock/sponson-stowage/racks, print parity at the 3.86 datum, §B2 zero see-through, family distinct. The −0.37 visibleBox scare cleared as the §5.269 instanced-AABB artifact class. Non-blocking minors banked for a future polish round (vent pots, muffler, grille relief, crutch mass, wheel tone, sight port, X-cable gauge). Gate context independently confirmed.
- §5.317 status: t95 COMPLETE (redesigned §5.346 + ratified 9.2); strv103a COMPLETE (§5.333 + PASS §5.345).

## §5.353 — Merkava angular gun-mask + Mk.4B track-course correction (OWNER session entry) (2026-08-17)
- Replaced the box-like Merkava mask stack on all six rostered marks with one connected four-ring octagonal loft. Each mask now has a broad buried rear seat, two hard shoulder breaks, a compact beveled mouth and eight visibly distinct armor planes; the deep cheek roots extend into the turret instead of ending as stand-off blocks. The recessed oval boot and complete mask remain `rig_gun`-owned and rotate/elevate as one supported assembly.
- Re-profiled the Mk.4B canonical smart track rather than adding a proxy layer: six road wheels now use a denser 0.82 m cadence, the sprocket/idler are smaller and lower, the return run is lower, the loaded ground run is longer, and the front/rear ramps are shallow continuous wraps. Shoe relief was thinned while preserving the skirts, armor and single suspension-driven course.
- Final proof is **252 frames** (14 paired + 14 yaw-0 + 14 yaw-90 for each of six marks). Duplicate-course audit is **6/6 PASS**; exact strict track clipping is **0 front / 0 rear / 0 shoe / 0 sweep** for all six; Mk.4B turret ownership remains **0 stranded / 0 abutting / 0 dangling**. All **54** selective icon/hit-zone/module assets are current; `npm test`, focused asset validation and the public production build pass.

## §5.353 — OWNER ORDER (verbatim, 2026-08-17): "time to start splirtting the afv family so this all goes a lot faster bbecause many of its steps should be seaparte agents"
- The afvFamily lane is being SPLIT: the current agent stops at a clean boundary + emits a PARTITION REPORT (per-item completion, region/hunk map, hashes, gate state, handoff notes); parallel region-fenced agents then take the remainders (upior flip / terminator2 / bradley set), with landings via the hunk-surgery pattern where regions interleave. The §5.353 split law: multi-item single-file lanes partition on owner order — region fences + hunk-split landings replace the one-lane-per-file rule when speed demands.

## §5.354 — SHARED-CHECKOUT REBASE INCIDENT + ABSORPTION (2026-08-17)
- The owner's session ran an interactive rebase of main onto b104b1e4 IN THIS CHECKOUT; their 465-file "calibrate fleet combat anatomy" pick sat STAGED at the edit stop; my §5.353 commit (unaware) landed on the detached rebase-HEAD and swallowed the pick. RECOVERED exactly: soft-reset (my commit preserved in reflog d85797a2), `git read-tree 7c213d2d` restored their staged pick INDEX-ONLY (worktree untouched), git ops FROZEN. The owner's continue then completed the rebase at **3635217c** — absorbing the ENTIRE worktree: my §5.353 append + icon resolutions, the afvFamily partition's completed work, AND their equipment/anatomy stream, all in their commit.
- **NEW LAW: check `.git/rebase-merge` BEFORE EVERY COMMIT in a shared checkout** (a staged 465-file pick is indistinguishable from your own staging otherwise).
- **HEAD re-binds (owner authority + partition-certified)**: upior `e32ab744` (the §5.323 FLIP + §B2 fills LIVE — the §5.286 ratification re-binds on the flip receipts), bmpt_terminator2 `8f9f4bcd` (twin bores ×2 §B3.1-proven + quality + reloadS 0.30 LIVE — §5.330 complete), m2a2 `89d68758`/m3a3 `9c545ac0`/ua_m2a3 `4b3b33fc` (§5.349 fills + attached skirts LIVE). Fleet-wide +1-mesh equipment moves (leclerc 8626645e, bmp3 e74a77a4, marder afa0b546 +12, etc.) = the owner's addEquipment system — hashes re-bind by owner authority, §5.278 class.
- §5.355 REMAINING from the partition: m2a2 honest gate row ×2 (blocked on a gate-page hang — closeout agent spawned), bradley packets ×2, ua_m2a3 enrollment DECIDED: measurement-only ref, NOT enrolled (clone convention); marder MILAN pocket + bmp3_rok/upior_ifv floats (§5.349 residue — second agent spawned); combined afv garage sitting for the flip/terminator2/bradley surfaces.

## §5.355 — type10 pair ENLARGED ×1.10 + fleet gear, landed-by-absorption (owner §5.336) (2026-08-17)
- **type10 `→b2f9a0ee`** (64/80854): ×1.10 judged live vs the type90 clone (scale-probe receipts; ×1.12 added nothing); gate **67.2 ×2 / dims 100** (the drop from 68.7 = the owner-decreed print divergence, §5.304 class); §B6 smart gear (raised sprocket/idler at print wraps, torsion arms, 0.112 links, real shoe relief, §5.262 tones, 49% wheel exposure), §B2 38→0, §B3.1 full gun run + §B3.2 density; three §B4 fixes receipted (+the ~3cm voxel-margin law); anatomy-reconcile law banked (combatAnatomy remaps gun pivots — the first-cut 10.26 muzzle receipt). **type10b `→ca20604`** (79/98612): FALSE-0 documented (new packet); B delta preserved via opt-in `s` params (A/B byte-proof on type90a/stb1); **the §5.299 byte-pin retired by owner authority**. npm + production build exit 0; 18 assets regenerated.
- Landed-by-absorption: the owner's sweep commits (a36d8743 assets + 3635217c calibrations) took the lane's files — content-verified complete at HEAD (markers, row, packets, icons). One external modern3 revert recovered losslessly mid-round from scratchpad. Guards at the fresh post-equipment baselines held exact. Garage sittings spawned (type10 pair + the combined afv flip/terminator/bradley surfaces).

## §5.356 — CRITICAL FLEET REGRESSION (owner, verbatim): floating turrets — pl01/upior_ifv/twardy/jaguar/"all afvs/ifvs" + challengers (2026-08-17)
- The owner reports turrets floating fleet-wide that "were right before". PRIME SUSPECT: the 3635217c combatAnatomy calibration's pivot remap writing into the VISUAL rig seat (the §5.355 lane documented the remap behavior). TOP-PRIORITY fix lane spawned: confirm in renders, root-cause the application site (visual rig vs sim/hitbox frame), fix minimally WITHOUT touching profiles or regenerating calibration data, fleet-wide seat scan (~115 ids), anatomy selftests stay green. All other landings queue behind this.
## §5.357 — Merkava square-frustum gun-mask correction (OWNER screenshot correction) (OWNER session entry; renumbered at merge) (2026-08-17)
- Supersedes §5.353's too-wide octagonal shoulder treatment. All six masks now use four dominant square-cone faces with only 12% clipped corner chamfers, a linear root-to-mouth taper and no width-holding shoulder shelves. Root width/height ratios were reduced from the Mk.4B worst case of 2:1 to approximately 1.1:1; the other marks use the same near-square grammar at their own scale.
- Every housing now starts with a constant-section socket buried **0.52–0.68 m** behind its mark-specific turret emergence plane. The socket and visible frustum are one closed `rig_gun` mesh, so the mask visibly grows out of the cheek and remains attached through elevation and yaw rather than stopping at the front armor.
- Final evidence is **252 frames** (14 paired + 14 yaw-0 + 14 yaw-90 for each mark). Assets, bores, duplicate-course and strict clip stages pass; Mk.4B ownership remains **0 stranded / 0 abutting / 0 dangling**. Full `npm test` and production build pass. The consolidated absolute geometry gate still rejects the six legacy rows on their pre-existing low source scores, hull-contiguity holes and pre-fittings decoration census; its generated failed-gate ledgers were intentionally not landed.

## §5.358 — leopard lane SPLIT (owner: "why are 2a4m and 2a6 changes taking so long?") (2026-08-17)
- The three-item leopard lane ran ~40 min with zero geometry cut (alive, stuck in study phase carrying leo2a4m + the leo2a6m gestalt re-loft + kf51b descent serially). §5.353 split doctrine applied: STOP + partition report ordered; three parallel agents take the items on its report. LESSON: multi-item lanes on heavy re-loft work split IMMEDIATELY — the §5.353 law now defaults to one-item-per-agent for loft-class work.

## §5.359 — leopard partition CORRECTION: all three items were BUILT (clean-room); completion agent live (2026-08-17)
- The §5.358 "zero edits" read was WRONG: the lane had built ALL THREE items in a pinned clean worktree (wt-5335 @ a7218931) because the live tree's foreign WIP poisons measurements (bisect receipts: main pre-edit read a4m dims 39.7 / a6m dims 25 — POLLUTED rows currently sitting in main's gate JSONs, to be replaced at the completion re-gate). Built: **leo2a6m `→d6025600`** (gestalt: cheek-wall front closure + slotCheekD param, cage→bustle-accent, EMES seated, §B2 corridor holes closed, the mg0 census debt fixed w/ a loader C6 — gate 91 HELD ×2 through the cage cut), **kf51b `→68e87948`** (leoHullV3 rebase, two-band skirts, German kit, leopard rear, ISAF accents — audit-clean, FALSE-0), **leo2a4m** (two-band ±1.885 skirts w/ §5.266 ERA grids, glacis field, stern slat cage at −0.1 honest cost — RATIFIED KEEP by the caging order + §5.335 precedent, hull quality; verified 86.1 ×2 at wip-final2, one small batch pending verify). Completion agent live: verify → hunk-aware merge onto current main (leopard.js moved again under the lane) → fresh-frame re-gates → packets → uncommitted delivery. CLEAN-ROOM LAW banked: heavy lanes may pin a worktree when live pollution is proven — but must REPORT the location in their first batch (the §5.358 miscommunication cost a false stall diagnosis).

## §5.360 — AFV set sitting: ALL FIVE PASS (2026-08-17)
- **upior PASS @ e32ab744** (the §5.323 flip resolved in pixels: prow +z / twin-door stern −z, turret rear-of-mid gun-forward, muzzle overhang −0.435 inside the nose, sponson fills solid — the only enclosed reads = kit openness, adjudicated); **bmpt_terminator2 PASS @ 8f9f4bcd** (BOTH bores end-on w/ depth parallax = geometry not paint, paired at yaw90; quality round decisively out-reads the before; muzzles/reloadS code receipts confirmed); **bradleys ×3 ALL PASS** @ 89d68758/9c545ac0/4b3b33fc (flush skirts w/ per-joint hangers + hinge seams + aprons, zero seam daylight ×13-frame batteries, bow windows closed, ring slit capped, shelf/ISU filled, §B9 wheels, family-true strips). Mid-sitting live pollution (the §5.356 fix lane's combatAnatomy WIP) handled via the canonical detached-frame double-sandwich — all verdicts bind to certified bytes. The §5.323/§5.330/§5.349 owner orders are fully RATIFIED.

## §5.361 — FLEET FLOAT REGRESSION FIXED + LANDED (§5.356) (2026-08-17)
- **Root cause (not stale data)**: `finalizeCombatAnatomy` passed `armor.turretPivot`/`gunPivot` through the plate-bounds→measured-envelope remap — but those arrays are RIG ANCHORS (tankFactory seats the visual rig_turret/rig_gun at exactly them), so on builds whose authored plates diverge from the visual envelope the pivot scaled up and the whole turret lifted (pl01 floated 0.56m; the gen --check passed 115/115, killing the stale theory). "Most tanks fine" = gate-certified plates keep the remap sub-2cm.
- **Fix**: combatAnatomy.js only (21+/16−) — pivots never ride the map; the casemate branch decomposes about the AUTHORED pivot; the rig-anchor law documented in-module. Zero profile edits, zero calibration regeneration; post-fix --check passes with a BYTE-IDENTICAL calibration file. Nine ids re-seated in pixels (pl01 2.699→2.07, twardy→1.38, jaguar→1.40, k1a1→1.50, challengers→1.55, marder→1.895 +x-drift-0, upior_ifv→1.66, m2a2→1.895; pl01's gun z restored 0.91→1.55); controls leclerc/t64bv1 byte-equal minima. npm EXIT=0 incl. both anatomy selftests. The owner's visual acceptance satisfied (before/after sets in shots/float-fix/).

## §5.362 — type10 pair sitting BOTH PASS; pivot re-auth follow-up; OWNER RECOIL ORDER (2026-08-17)
- **type10 + type10b PASS** (true-scale pairs decisive vs type90; §B6 gear zero-clip ×18 sheets; §B2 strict flood = zero shell holes; B-delta at scale; the ×1.10 order delivered). Mid-sitting §5.356-WIP pollution handled via the bit-exact worktree double-sandwich — verdicts bind to b2f9a0ee/ca20604.
- **CROSS-LANE FINDING (from the sitting)**: the landed §5.361 float fix un-remaps the type10 pair's rig (+0.242m forward, overall 10.444→10.686 vs the decreed 10.439) — their §5.336 rig was authored THROUGH the calibrated-pivot pipeline. Micro-fix spawned: pivot re-auth (z ≈ −0.009) at the new rig-anchor law, dims re-verified.
- **OWNER ORDER (verbatim)**: "make all cannons have proper recoil btw. and ifv autocannons should have recoil too, and the terminators should ahve alternating recoil depending on which gun is firing" — recoil-system lane spawned: fleet cannon recoil (the rig_recoil chain), IFV autocannon recoil (rate-scaled), and ALTERNATING twin-gun recoil for the terminator keyed off spec.gun.muzzles (the §5.330 knob pairs with it — alternate the recoil impulse + the muzzle-flash origin per shot).
## §5.363 — OWNER ORDER (verbatim, 2026-08-17): "add a bmp terminator 2 where it has an even crazier beefier two autocannon turret with even more equipment and decorations and even some era on a t90 hull"
- NEW id lane spawned: `bmpt_t90` ("BMPT T-90", Russia, tier 10, FALSE-0) — t90-line hull grammar (K-5 glacis, §5.350 skirt/ERA program, §5.262 gear) + a beefed twin-30mm station (muzzles ±0.20 via the §5.330 knob, fast reload, QUAD Ataka racks both sides, AG-17 pair, full sensor suite, station ERA). afvFamily.js append-only (region-fenced vs the live residue lane); §5.287 wiring checklist. Eight agents live: recoil-system, bmpt_t90, type10-pivot micro-fix, leopard-completion, bradley-closeout, marder-residue (+two sittings closed this hour).

## §5.364 — OWNER ORDERS (verbatim, 2026-08-17): type10 see-through/black-line/tracks/fills + type90 guns/mantlet/arc
- **"type 10s are see through and have a big black line on each side of hull. fix all of these, fix the hulls, make the tracks beefier and have the same decorations as other tracks, and fill up the insides"** → folded into the live type10 lane (items 2-5: black-line attribution first — likely the near-black suspension bays the §5.362 sitting adjudicated as designed openness, now owner-superseded; see-through fills §5.303-class; beefier §B6 tracks at the t90/leclerc bar; hull quality). **"fix the type 90s guns. they should properly be attached... proper mantlet and arc up and down porperly"** → new type90-pair lane (attachment/cradle, the Kyū-maru wide mantlet gun-bucketed riding elevation, gunPivot trunnion-line arc −7/+15 w/ §B5 pitch batteries; the owner's d4a9410 taller-turret decree respected; §5.361 rig-anchor law context). Eight agents live.

## §5.365 — BRADLEY CLOSEOUT LANDED (§5.355 complete) (2026-08-17)
- **The m2a2 "hang" DIAGNOSED, no tool change needed**: any page-side failure leaves __FIDELITY_READY unset and the gate polls it silently to a 150s timeout — the pre-rebase mixed tree was the trigger; gates in ~4s clean. LATENT GAP documented (poll __FIDELITY_ERROR too, fail fast). The equipment-mask suspicion EXONERATED — but it was masking the real §5.356 pivot lift (+0.162m on m2a2's turret, independently converging with the fix lane's nine-id list; the committed 90.9 row was stale — pre-stream truth 83.3).
- **m2a2 honest row ×2 at the current frame: 49.1** (hull 63 / turret 55.3 / stations 54.9 / **dims 100** after the flagged one-value fix — silhouetteHeightM 2.98→2.83, tuned-against-the-lifted-render corrected to the seated p95; applied + re-gated ×2 this landing). Whole-limited 49.1 = the honest dressed baseline; the §5.360 sitting ratified the attachments visually.
- Packets ×3 landed (m2a2 §5.349+§5.355 full section w/ the hang diagnosis; ua_m2a3 NEW packet w/ the NOT-ENROLLED decision; m3a3 landing addendum; sweep receipts corrected 4579→3785). Track-clip ×3 byte-identical donor debt. Guard movers pinned: 3635217c equipment splits (+1 mesh classes) + §5.361 pivot re-seats (counts identical) + the residue lane's live WIP — all attributed.
## §5.366 — Merkava square-frustum surface-detail pass (OWNER follow-up) (OWNER session entry; renumbered at merge) (2026-08-17)
- Broke up all six formerly flat gun-mask faces without changing the §5.357 near-square silhouette: tangent longitudinal weld/recess lines, two transverse armor breaks, a bolted inset roof inspection cover, asymmetric side inspection frames, staggered washer/bolt courses and low root lifting bosses now sit only millimetres above the parent planes.
- Each mark carries a distinct dark inlaid service stamp (`1B`, `2B`, `2D`, `3C`, `3D`, `4B`) built directly into the right mask face. Every new relief remains `rig_gun`-owned, so engraving, fasteners and covers elevate and yaw with the same connected buried housing; no turret decals or hull-stranded decoration were introduced.
- Fresh proof: **252 frames** (14 paired + 14 yaw-0 + 14 yaw-90 per mark), **6/6** one-course duplicate-track PASS, and exact strict clip **0/0/0/0** across all six. All **54** selective UI/anatomy assets were regenerated; anatomy alignment, nine-view asset validation, muzzle bores, full `npm test` and production build pass. The absolute source-comparison wrapper remains red only on the legacy §5.357 gate/contiguity/fittings debts; failed generated ledgers remain excluded.

## §5.366 — residue lane + LEOPARD TRIO LANDED (2026-08-17)
- **Residue (§5.349 complete)**: marder1a3 `→d25cfa80` (MILAN pedestal web + the m2a2-grammar bow closure — after-sweep pixel-identical to the certified guard profile; row 81.9→**82.5** +0.6 ×2), bmp3_rok `→496f3528` (the "cross-antenna" resolved into three chained-air defects — whip pots/pintle chain/pano gap — all seated; islands→0), upior_ifv `→e2cf8368` (mast-arm bridge + light platforms). Landed via hunk-split (7 residue hunks; the bmpt_t90 lane's 1166/1177 hunks HELD OUT in-flight). Isolation proven paired-at-one-tree ×8 guards.
- **LEOPARD TRIO (§5.345/§5.359 complete)**: leo2a4m `→af74fbf2` (86.1/100/100 ×2×2 — two-band ±1.885 ERA-grid skirts, glacis field, stern cage KEPT at the ratified −0.1, hull quality, final3 rails+shelf verified); leo2a6m `→d6025600` (**91/100/100 PASS ×2×2** — the gestalt: cheek-wall front closure, cage→bustle accent, EMES seated, mg0 debt fixed; the POLLUTED 39.7/25 rows replaced); kf51b `→79a0cc0` (leopard-descent + the completion's axe-aft fix clearing a real 3.2cm abut; NEVER-GATED verified ×2 frames). Merge proven exact (main's only delta = 3635217c's seven addEquipment conversions, zero overlap); guards ×9 pristine-vs-merged identical; npm ×2. Changed-view sittings next (leo2a6m gestalt + kf51b descent + a4m garage).

## §5.367 — Merkava flat rear turret datum + supported bustle roots (OWNER follow-up) (2026-08-17) — OWNER session entry; renumbered at merge (both sessions claimed §5.366)
- Added a closed transverse armor bulkhead at the authored basket root on `merkava1b`, `merkava2b`, `merkava2d`, `merkava3c`, `merkava3d`, and `merkava4b`. The plate is buried 13.5 cm into the existing tapered shell/bustle and overlaps the first rack course by 2.5 cm, giving every bustle and cage a flat physical turret face instead of a point or empty-air termination.
- Width and height derive from each mark's existing basket and turret envelope; the outer silhouette, hull, skirts and running gear remain unchanged. A recessed service seam, five bolts, and upper/lower rack shoes break up the face while making the attachment load path visible. All new geometry is turret-owned and rotates with the complete bustle/cage assembly.
- Fresh proof: **252 distinct frames** (14 paired + 14 yaw-0 + 14 yaw-90 per mark), **6/6** one-course duplicate-track PASS, exact strict clip **0/0/0/0**, current anatomy and **54** selective assets, `npm test` PASS and production build PASS. The consolidated source gate remains red only on the same pre-existing legacy source-score, hull-contiguity and pre-fittings census debts; its failed generated ledgers are excluded.

## §5.368 — bmpt_t90 LANDED (§5.363 order complete) (2026-08-17)
- **New id `bmpt_t90`** ("BMPT T-90", Russia, tier 10, photo-class/never-gated by design) `→9fa066b8` (76 meshes/123,244 verts, reproduced byte-identical across two mid-lane HEAD moves + re-bound post-§5.366). Certified t90a donor hull untouched (shared T90_PROFILES export) + K-5 glacis, §5.262 gear, 8/side skirt-ERA at the §B9 0.98 line, twin AG-17 bow pods; station = terminator2 grammar sized up — twin 30mm at x ±0.20 (gun.muzzles knob, reloadS 0.30), QUAD Ataka ×8 (§5.360 grammar doubled), K-5 face/cheek/wall ERA, full sensor suite, doubled smoke. Finding banked: t90a donor dresses turretTrack/turretCupola/turretEquipment past the shared clear helper — cleared profile-locally.
- Gates: npm ×2 (116 tanks), clip strict 0/0+0/0, §B2 0, §B3.1 bore ×2 (tagged 2/rims 2/discs 2, contrast 99.2), §B5 0/0/0 + yaw90 one-piece rotation, §B9 wheels. Guards ×17 identical (bmpt_terminator2 held 8f9f4bcd; full t90 family + afv residents). Shots shots/bmpt-t90/ (21), packet docs/references/tanks/bmpt_t90.md.
- **Calibrations hunk-split at landing**: kept the bmpt splice + landed-id rows (leo2a4m/leo2a6m/kf51b + marder1a3/bmp3_rok/upior_ifv/upior — §5.366 regen debt paid); DROPPED type90/type90a/type10/type10b rows (in-flight lane WIP baked by the lane's live-tree anatomy regen — their lanes land their own rows; live-tree hazard note).

## §5.369 — type10 pair LANDED (§5.364 order complete) (2026-08-17)
- **Item 1 — pivot re-auth (§5.362 finding under the §5.361 rig-anchor law)**: turretPivot `[0, 1.8027777777777776, 0.2713333333333332]` both ids; gunPivot type10 `[0.011, 0.4511015831134565, 1.1771211453744495]` / type10b `[0, 0.4511…, 1.1771…]`. Sited in `src/vehicles/japan.js`'s post-clone block, NOT the modern3 armor row — userdrops5's type90 clones armor via `fitArmorToDims` which SCALES pivots, so a donor-row edit would have moved the byte-held type90 frame. Item-1-alone builds returned to CERTIFIED BYTES (hashgeo b2f9a0ee/ca20604 exact; rig_gun world [0.011, 2.1231, 1.4125] restored; the +0.242 regression dead).
- **Items 2-5 (owner orders)**: black line ATTRIBUTED to an 11.5cm × 5.94m pure-black `hullShadow` AO fake (66/72 rays y 1.393–1.486) — retired for a 3.3cm gunmetal rail + hullDark seam, flank shadow-rows after **0**. See-through closed: before 60/103 rays passed clean through at y=1.20 (bay-liner→sponson-floor corridor) — filled with real geometry (bay/bow/rear walls, hydro struts, sponson-front closures, driver bulkhead, toolboxes); **after zero interior rows**. Tracks beefed to fleet anatomy (band 0.055→0.072, relief 0.55→0.85, pitch 0.112→0.138) with clip strict **0/0/0/0** both ids. AABB byte-equal before→after (10.4512/3.5640/6.6632; the §5.304 decree 10.439 held).
- Rows ×2 deterministic: **type10 66.9/66.9** (dims 100, floaters 100, hull 79.1 **+2.0**; whole −0.3/stations −1.6 = honest cost of the owner's seal orders vs the §5.304-decreed FALSE print — §5.355 precedent). type10b FALSE-0 unchanged. Hashes **6e25b62e** (66/86,338) / **5e6f7700** (81/104,096), live == clean-room across two landings. Guards: type90 `518e88f0`/type90a `71208238` byte-held clean-room A/B. npm + production build exit 0. Calibrations hunk-split at landing (kept type10/type10b; DROPPED the type90-guns lane's type90/type90a rows — that lane lands its own).

## §5.370 — Mk.4B Mk.1B-derived smart course + turret consolidation (OWNER correction) (2026-08-17) — OWNER session entry; renumbered at merge (both sessions claimed §5.369)
- The Merkava Mk.4B now literally derives its complete running gear from the Mk.1B's canonical smart-course object. One uniform envelope transform preserves the donor's six-station cadence, rising idler, dished suspension-driven wheel faces, link pitch, tread relief, connectors, tones and one-course ownership; only the length and width are scaled for the Mk.4B hull. This replaces the separate Mk.4B approximation rather than layering another track beneath it.
- The Mk.4B skirts and exterior hull armor are preserved. A hidden outboard floor datum alone was raised above the taller inherited return run, producing exact strict clearance without deleting or moving side armor. The final course passes duplicate-track **2/2** and exact strict clip **0 front / 0 rear / 0 shoe / 0 sweep** for both the donor Mk.1B and derived Mk.4B.
- The Mk.4B turret was consolidated around its bespoke five-plane armor shell: duplicate generic flank panels, redundant roof boxes, repeated smoke/MG sets and the five-whip thicket were removed; one buried lower locking rail, supported service fittings, a compact roof load, two seated radio whips and the authored commander/cradle weapon stations remain. Final proof is **84 distinct frames** (14 paired + 14 yaw-0 + 14 yaw-90 for both marks); focused anatomy/assets, bore checks, `npm test` and production build pass. The consolidated source wrapper remains red only on the known removed-source-GLB, legacy contiguity and pre-fittings census debts; its failed ledger is excluded.

## §5.371 — FLEET RECOIL SYSTEM LANDED (§5.362 order complete) (2026-08-17)
- Owner order: *"make all cannons have proper recoil btw. and ifv autocannons should have recoil too, and the terminators should ahve alternating recoil depending on which gun is firing"* — all three clauses delivered.
- **The rig** (`tankFactory.js` +166): caliber-true throw `REC_AMP = clamp(0.13 × cal/120, 0.06, 0.24)` replacing the old flat 0.55 m; separate rapid-belt profile (`RAPID_*`, 0.255 s cycle, 2–4 cm) for autocannons; per-barrel `rig_muzzle_tip_N` anchors; `gunMuzzleWorld(out, i)` + `recoilKick(age, scale, i)` returning the FIRED index; asymmetric yaw/roll kick; a geometric tube census (`recoilHasTube`, ≥0.5 m) that suppresses the slide on hull-printed casemates and reroutes the budget to boosted chassis rock.
- **Alternation** wired at all four fire paths — local sim (`state.js` cursor: shot N → `muzzles[N % len]`, shared by shell origin/flash origin/recoil kick), player shot views (`main.js`; also fixes a hardcoded `caliberMm: 120` → `p.spec.gun.caliberMm`), studio (`studio.js` — kick now precedes the flash sample so the flash sits on the FIRING tip), and networked battles (`browserBattleBridge.js` — server stays authoritative for ballistics; the visual owns the alternation cursor).
- **bmpt_t90** (landed §5.368) already alternated at runtime — the `spec.gun.muzzles` knob is generic — but was absent from every test/receipt surface: added to the selftest (now loops both terminators against their own authored bore tables + mirror symmetry + belt-refire clearance) and both receipt harnesses. NEW **roster guard** fails the build if a future `gun.muzzles` id ships without coverage; its first draft was VACUOUS (`ALL_TANK_IDS` is only 26 MBT ids → zero twin-plant matches) — rewritten against `TANK_SPECS` (146) with a `seen.length >= 2` anti-vacuity assert, then proven live by dropping bmpt_t90 and observing the intended failure.
- **Guards: 12/12 byte-identical** in an isolated clean-HEAD worktree (leclerc 8626645e, m1a2 d4a5bbad, t90m 2b7d896d, tiger1 ff33d1f8, bmp3 2aeae00, m2a2_bradley 278143f0, bmpt_terminator2 8f9f4bcd, bmpt_t90 9fa066b8, t95 ce210ed4, strv103a 3f6c913c, isu152 dbaa0fbe, marder1a3 d25cfa80) — recoil is a RUNTIME rig, so zero geometry may move. **An all-baselines table is the exact shape a silent no-op makes**, so the lane FALSIFIED it: a 20 mm bore nudge moved leclerc/m1a2/bmpt_t90 while t95 correctly held (its cannon prints into hull buckets). Sensitivity proven, guard real. npm exit 0 (recoilRig.selftest at line 408); ifv-recoil-probe GREEN.
- **Census**: 146 ids exercised, 0 build errors — **138 recoil** (20–168 mm, median 130), **all 14 IFVs** recoil.
- **ORCHESTRATOR ADJUDICATION — the 8 zero-tube-travel ids** (`t95, strv103, strv103a, jagdtiger, jpz_e100, sturmtiger, isu152, isu122s`, all casemate TDs): ACCEPTED as chassis-rock-only (1.075–1.152 rad/s vs the tube fleet's lower value). Their cannons are printed into certified HULL buckets, so `rig_recoil` holds only a hidden stub collar — sliding it would walk a loose collar along a static tube, a visible defect. Rigid-mount chassis recoil is the physically correct read for a casemate. FLAGGED TO OWNER as the one place "all cannons" is answered with chassis rather than tube recoil.
- Receipts `shots/recoil-system/`: both terminators alternating (`crop_bmpt_{t90,terminator2}_frozen_{left,right}_flash.png` — flash core on opposite tubes; live log i=0,1,0,1; muzzle origins 0.40 m / 0.32 m apart), cannon+IFV travel A/B **sim-paused** (leclerc 0.1535 m, m1a2 0.1406, bmp3 0.0307, m2a2 0.0256). Lesson banked: the first IFV pair was CONTAMINATED by turret traverse between captures (~90 px shift on a 30 mm throw) — pause the sim (`settings.isOpen()`) for A/B travel receipts.

## §5.372 — icon regen for the §5.366/§5.369 landings (2026-08-17)
- 65 files (72 rendered, 65 changed) for the eight ids whose geometry moved: type10/type10b, leo2a4m/leo2a6m/kf51b, marder1a3/bmp3_rok/upior_ifv. §5.246 recipe — rendered in a CLEAN-HEAD worktree at fee49178 so the art matches the merged tree, not the live tree (two lanes' WIP resident). Landed 624b379f.
- Manifest guard applied: `genIcons --ids=` PRESERVES unlisted entries (116 in, 116 out, 0 dropped) — verified by entry-set diff before copying, since a partial manifest would have wiped 108 tanks' rows. Changed manifest rows == exactly the eight ids.

## §5.373 — type90 pair guns LANDED (§5.364 order complete) (2026-08-17)
- Owner order: *"and fix the type 90s guns. they should properly be attached to the tank and have a proper mantlet and arc up and down porperly"*.
- Rows ×2 BIT-IDENTICAL (orchestrator re-ran after the lane died on a session limit): **type90 20.3 → 21.9** (+1.6; stations 32.6→38.6, whole 28.5→31.2, turret 20.3→21.9; hull 88.5 and dims 45.9 unmoved — the honest ceiling here is the print-vs-build divergence, not the gun work). Hashes **type90 518e88f0 → e10fb640**, **type90a 71208238 → 61cd559a**.
- Guards clean-HEAD A/B at 3a37ec93: type10 `6e25b62e` / type10b `5e6f7700` HELD (critical — the lane edits `profiles/japan.js`, which hosts §5.369's just-certified pivot block), plus leclerc `8626645e`, amx30 `4db67e1c`, amx30b2 `4b1a2a60`, type74 `818321a5`, type89 `e160e89b`. Calibration rows touched: exactly type90/type90a. npm exit 0 (clean-worktree, &&-gated).
- Packets `docs/references/tanks/type90.md` (appended) + `type90a.md` (NEW). Lane finding banked there: type90/type90a icons are STALE EVEN AT CLEAN HEAD (`a35ac3a7` landed proportions with no icon regen, and §5.372's regen covered only the eight §5.366/§5.369 ids) — icon regen queued below.
- Debt: no independent visual sitting ran for this pair (the lane died at the packet step). The elevation-arc clause is proven by the lane's own probes only — an independent sitting is owed.

## §5.374 — t14 armata PROPORTION ROUND r2 LANDED (owner order 2026-08-17) (2026-08-17)
- Owner order: *"update our t14 armata to look like this a lot more and match proportions better doing our visual comparison and geometric checks"* — the `t-14_armara_uralvagon_factory.glb` oracle was ALREADY registered in the four maps (the `df1de92c` onboarding wave); today's order was a match-quality round, not an onboarding. (The owner re-dropped the same GLB; staged to the gitignored candidates path, md5 4f4b3752.)
- **`499540b9` → `ca418af4`** with vert count UNCHANGED at 62,537 — pure re-proportioning, no added mass. Rows ×2 BIT-IDENTICAL: **geoMin 72.3 → 76.1** (turret 72.3→76.1, whole 75→77.5, stations 84→84.6, hull 79.4→78.1, dims/floaters 100).
- The measurement that carried it: in the extract's own axisMap frame (`gate_z = −2.057775×glb_z + 0.6785`, corroborated by gunBox hi 5.643 == our 5.64 muzzle) the ref turret spans world z −2.861..+1.358 vs ours −2.855..+1.62 — **the rear already registered; only the FRONT ran 0.26 long**. Every station forward of the shoulder line (local 1.04) pulled back by k=0.778 about that line; apex lands local 1.958 = world 1.358 exactly on the print, turret center and gun/muzzle datum both held.
- TWO EXPERIMENTS BANKED AS REVERTED (do not re-try): (1) aft ring re-seat to −0.68 measured 73.5→**21.9**, reverted — `tmp-moderns-worldtrace`'s camera-frame z is NOT build-world z and its apparent −0.70 offset was a FRAME ARTEFACT; the ring stays at −0.60. (2) chin re-loft to the oracle's 2.474/2.532 yMax reads measured whole 77.5→77.2 / stations 84.6→83.8 — the ref's 2.474 is a NARROW shroud-edge feature, not a 0.84-wide plate; the flat chin stands.
- Guards clean-HEAD A/B: all co-residents byte-identical (see §5.373 table). Calibration rows for t14: none needed. Packet appended to `docs/references/tanks/t14.md` by the orchestrator (the lane died before writing it).
- Debt: gate is honest at 76.1 and does NOT pass; turret/whole remain the ceiling. No visual sitting ran — receipts are owed.

## §5.375 — LANE-DEATH RECOVERY PROTOCOL (banked law) (2026-08-17)
- Five lanes died this session on account-level API exhaustion (credits, then a session limit), not on work errors. Three shapes learned: (1) **transcripts do not survive** — `SendMessage` resume returned "No transcript found", so recovery = respawn with a brief that points at the on-disk WIP, or finish it in the main loop; (2) **the WIP is intact and trustworthy** — every dead lane's files were byte-identical to the pre-crash scratchpad snapshot; snapshot IMMEDIATELY on death; (3) **a lane's dying words understate its progress** — both final lanes reported being mid-step ("Now the packets…", "Now the final gate ×2") yet had already written complete packets; ALWAYS audit the artifacts before assuming work is missing.
- Cheapest recovery for a lane that dies within one step of done: finish it in the MAIN LOOP (re-run the gate, write the missing packet, verify guards) rather than respawning an agent that must re-read everything.
- Corrected fencing lesson: I had fenced `src/game/studio.js` + `src/net/browserBattleBridge.js` as "owner WIP" from a stale handoff note; the recoil lane's audit proved every hunk in both was §5.362 work. **Audit diffs by CONTENT before trusting an inherited hazard list** — a wrong fence would have shipped a half-wired feature.

## §5.376 — M3A3 M2A2-foundation turret + BRAT protection round (OWNER order, 2026-08-17) — OWNER session entry; renumbered at merge (collided with my §5.373)
- Replaced the narrow A3 wedge / oversized-cabinet read with a broad M2A2-derived two-man ring and welded shell, deeper bustle, closed cheek-to-M242 saddle and full roof foundation. The compact left TOW pod now has an explicit wall-root/trunnion/cradle load path; the right stowage wing is independently seated.
- Added the A3 equipment program requested by the owner: re-seated CIV and ISU, two structural crew stations, shielded M2 + M240 roof weapons on distinct arcs, roof service bins, radio/smoke suite, spare links, cans and a filled four-rail bustle rack with braces. Every fitting rotates with the turret and remains outside armor receipts.
- Added destructible ERA to both turret cheeks/walls, three backed upper-glacis courses and two full side courses on the retained spaced-armor backing. Preserved the single suspension-driven smart track, wheels, skirts and hangers; mirrored front-fender bridges close the donor plan slots without entering the sweep.
- Proof: yaw-0 14-view battery + yaw-90 ownership views, `mg2+11d`, plan holes **0**, duplicate track **PASS**, exact strict clip **0/0+0/0**, regenerated combat anatomy. The forced absolute geometry wrapper remains unavailable for its documented reason: the registered M3A3 comparison GLB was removed and resolves to Vite HTML, while the packet already classifies the row as a lawful zero.

## §5.377 — INDEPENDENT SITTING: type90 pair PASS, t14 below bar (2026-08-17)
- First independent eyes on the three §5.373/§5.374 ids (both builder lanes died before any receipts). Critic built its OWN harness (`tools/tmp-b8sit-t90t14.html` + `.mjs`), 355 PNGs under `shots/type90-t14-sitting/`; none of the dead lanes' probes were reused. Nothing edited/staged.
- **type90 9.1 PASS / type90a 9.2 PASS** — all three owner clauses verified in pixels:
  - (a) ATTACHED: rig chain clean (`rig_gun` scale 1,1,1 under `rig_turret` scale 1,1,1 — the shear-inducing parent scale now isolated in a `type90_turretShell` group that EXCLUDES the gun). Solid-continuity census — 7 sample lines through the mount, 0.02 m steps, orientation-signed ray crossings so overlapping co-mesh solids cannot fake a void — **0 air samples in ~2,500** across all 9 elevations. Mantlet rear measured z 1.21–1.22 vs turret face plane 1.42 = **0.20 m buried**, a real overlap not a butt joint; magenta-ground daylight census 0 background px in the junction crop at every angle.
  - (b) MANTLET: one wide plate 1.20 × 0.48 × 0.32 m straddling the trunnion with rounded side edges, proud face plate, bolted retainers, tube-exit collar, bellows ring, coax hood — correct for the Kyū-maru's broad flat face, not a stub. **DEFECT BANKED**: stand-off only 0.06–0.11 m and it carries the same camo as the turret face, so at 1× it does not read as a distinct casting (the dark slot frame does the work). The oracle's block projects further with a fatter recoil sleeve.
  - (c) ARC (the clause most at risk — previously self-read only): **strongest result**. Driven on the production path (`rig_gun.rotation.x = -pitch`) across −10…+20: rendered angle == commanded to **±0.001°** at all 9 stops, and trunnion→`rig_muzzle` distance **invariant at 4.6594 m** at every angle — pure rotation, no shear, no scale in the pitch path. The earlier lane's reported −8/+14 → −6.65/+11.68 BENDING is gone. No tear/gap/intersection at any angle; whole plant rides the ring (yaw 45/90/180 + yaw90-elev20).
  - Pre-existing (NOT this round): the flank number decal is largely occluded by the hinomaru roundel; decals attach to the UNSCALED turret after `postAssemble`, world seats unchanged.
- **t14 8.6 — BELOW the 9.0 bar. The proportion claim is CONFIRMED; the residual is turret SHAPE.** Hull-registered (the print sits 0.666 m off in z — a bodily offset, not a proportion error), turret shell gun-excluded: front z 0.692 vs oracle 0.679 = **13 mm**; rear −3.521 vs −3.538 = 17 mm; base yMin **1.346 vs 1.346 EXACTLY**; half-width 1.445 vs 1.428 = 17 mm. The k=0.778 trim landed the apex on the print's line with the rear untouched, exactly as §5.374 claimed. Silhouette IoU vs the oracle: **ours 71.2–95.5%** against a filled-bounding-box baseline of 37.7–81.6% — better in every view, ~double in side and turret-front, covering 93–97.5% of the oracle everywhere; **plan view near-exact at 95.5%**. No seam/pinch/broken shape from the trim (chevron converges symmetrically; facets continuous through the shoulder knuckle to the chin).
  - **SIX DEFECTS with view evidence (the r3 work order)**: (1) turret roof 72 mm high, crown reads as a full-width straight bar where the print steps down; (2) print carries shoulder mass at BOTH front corners we lack; (3) commander's/EO stack smaller than the print's; (4) mid-turret flank fills a band the print leaves open (the r2 drum stretch reads slightly full); (5) gun tube sits a few cm low along its whole length; (6) SHAPE not proportion — the print's shroud is a sleek faceted wedge tapering to a knife over the gun, ours is blockier with a squarer front and flat unarticulated facets.
  - Adjudication: the owner's *"look like this a lot more"* is substantially met and receipted; *"match"* is not yet. Consistent with the honest 76.1 gate (turret/whole are the ceiling).

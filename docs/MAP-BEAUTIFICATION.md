...
| Coastal/Saltwind meadow and crop composition | The grass-height candidate remains unpublished. The separate unequal crop-rhythm candidate, combined with the corrected lighting at `83e74e4de`, was captured successfully but **rejected visually**: its establishing view still reads as parallel pale pegs. A root-color interpolation experiment at `4ac879eed` retains the same top colors, geometry and buffers; focused tests/typecheck/private build pass, but its matched native review and performance/residency acceptance are pending. These art candidates are not shipped |
...
| Road continuity | The22-map continuity candidate remains unpublished. The single-composition follow-up at `65a50643f` still fails three outer-bank slopes and both Reservoir footprint limits in the retained dense eight-case run. An authored-exit correction is in development: Reservoir's southeast service route currently extends355m diagonally southwest. Its intentional layout/dressing changes must be reported separately from a matched-layout grading comparison; all new approach segments and tie-ins require coverage, not just the final boundary tails. Query cost, regenerated collision/storage and mode placement remain release holds |
| Nighttime entry and lights | Final-light-first correction, downward headlight aim and restrained window/streetlamp intensity pushed; matched native closeups and lifecycle checks pass ([entry](NIGHT-ENTRY-CHECKPOINT.md), [aim](NIGHT-LIGHT-AIM-CHECKPOINT.md), [intensity](NIGHT-WORLD-INTENSITY-CHECKPOINT.md)). A source audit found a discarded authored-day PMREM bake before selected-night preparation on first/different-map entry. Its covered-entry fix is local; a fresh current-main A/B pair will count actual bake returns and target installation, not infer them from callbacks or call them GPU-completion/loading-speed proof. Service windows glow but are not all spatial lights; flat interiors and unshadowed point-light occlusion remain |

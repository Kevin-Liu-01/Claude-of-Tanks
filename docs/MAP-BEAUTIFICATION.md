...
| Torn tree crowns | Coherent-corner candidate is local and image-reviewed. The first fixed-work A/B/B/A did not clear the strict cost gate; it is held. An index-preserving candidate retains identical expanded triangle attributes while reducing its near-card buffer bytes by about41%. The mismatched Medium/High pair remains invalid. One corrected native High/M5 pair now has identical camera/work/quality: summed CPU submission +0.079%, completed-wall sum −0.930%, but their medians increase about3.5–3.8%. This is comparable diagnostic evidence, not a performance pass or release approval; matched index-candidate visual review remains pending |
...
| Ironworks masonry | Foundry-only brick pigment is image-reviewed with matched native close/wide/detail views; Copper Mesa and other material buckets are preserved. The maintained cost gate passes with equal draw/resource counts and render p95; the +0.1ms median increase remains recorded. [Evidence and limits](FOUNDRY-MASONRY-CHECKPOINT.md). Roof glare, flat trim and settlement composition remain separate |

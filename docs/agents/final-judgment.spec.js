...
const VIEWS = [
  ...
  { key: 'techtree', what: 'tech tree screen' },
...
const LENSES = [
  ...
  { key: 'hud_ux', prompt: 'LENS: HUD/UI/UX. Read player_view.png, sniper_view.png, garage.png, techtree.png. Then a temp puppeteer script: open settings, rebind a key, confirm localStorage persistence; open the tech tree; exercise the camo picker and map picker. Judge against real WoT UI and check for dead/fake affordances (a past audit found dead consumable slots and a no-op minimap zoom — verify their current state).' },
  ...
  { key: 'completeness', prompt: 'LENS: completeness vs the commission. The user asked for: a World-of-Tanks-level game in Three.js at AAA quality; deep research reflected in systems; modern tanks (20+) alongside WWII; hit/armor/shell/movement systems; HD models with real sourced assets where possible; editable controls; firing at enemies; multiple maps; tech trees and flags; Switzer typography; kill cams and shot info with armor diagrams; a camo system; strong performance. Check EVERY item: present / partial / missing, with evidence. Also audit docs/ATTRIBUTION.md against the files actually in public/ (every asset attributed? quarantine section accurate?) and list what a real WoT player would still immediately miss.' },
]

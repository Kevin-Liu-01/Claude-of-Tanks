...
| 8 | 2026-08-08 | tank_switch worst 286-366 ms (Lap); switchprof: every switch 3-7 tasks of 50-250 ms; SEPv3 selection spike 632 ms with prog+91 in ONE task | switchprof (6 timed switches under profiler): (1) selection-time program compile storm — getProgramParameter 1828 ms + (program) 821 ms across 6 switches, worst = never-before-selected specs compiling their whole subtree in one slice (SEPv3 +91 programs/632 ms); (2) camo bakes getImageData 1804 ms per-selection; (3) pedestal floor-fit raycasts 880 ms (intersectTriangle/robustFloorY). Pedestal prefetch only reaches +-1 neighbors and a 1.4 s browse cadence outruns it | QUEUED (next wake): chunk the selection-path compile — reuse the compileHiddenVariantsChunked/linkerBreathingSlices pattern at the garage onSelect compile site so a fresh spec's +91 programs link across frames instead of one 632 ms slice; bake chunking is the follow-up if still red | pending | pending |

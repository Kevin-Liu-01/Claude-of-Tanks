...
|---|---|---|---|---|---|
| Leopard 2 A6 (`leo2a6`) | buh | https://sketchfab.com/buh-late (user-supplied download) | CC-BY 4.0 | `public/models/tanks/leo2a6_buh.glb` | REPLACES the procedural Leopard 2A6 model (modern1.js gameplay stats unchanged). Fully articulated: authored turret/gun nodes, individual wheels. Credit line renders on its garage card via `spec.community`. |
...
register in EVERY build (the garage stats card renders the credit; these
...
|---|---|---|---|---|
| Baked combat SFX (29 files, COMBAT-SFX r2, 2026-08-01: layered cannon fire per caliber class — sub punch + crack/report + rumble tail × small/medium/large/huge; penetration clang+thud+debris ×2; receiving-end interior whump; ricochet whine ×3 variants; non-pen thunk ×2; ammo-rack tank explosion core ×2 + debris rain + turret-pop deep accent; burn-out cook-off; HE burst ×2; shell-into-dirt; ERA pop) | Generated by this project via `tools/make-sfx.mjs`: **100% procedural DSP synthesized in node** (seeded/deterministic — swept sines, filtered/shaped noise, modal metal partials, baked slapback echoes; no microphone, no recordings, no model) → local ffmpeg mastering (character EQ, tanh soft-clip saturation for weight, compression, peak normalize, `alimiter`) → mono 48 kHz Opus 96k. Self-gated: clean decode, duration, true peak ≤ −1 dBTP, per-mix LUFS windows, **bass-energy floors** (energy <120 Hz: fire ≥25%, explosions ≥35% — measured 70–94%; ricochet capped ≤4% so deflections stay bright). A/B captures vs the retired live-synth combat one-shots: `shots/sfx-r2/ab/` (README has the numbers) | Local synthesis — this repo (re-runnable: `node tools/make-sfx.mjs`; zero downloads, no accounts/keys/cloud) | Original procedural output of this repo — CC0-by-construction posture, no third-party material of any kind | `public/audio/sfx/*.ogg` (29 files, ~511 KiB total, ≤900 KiB budget; lazy-loaded at the first user gesture like the voice set — boot untouched) |
...
official `piper-tts` PyPI wheels instead. COMBAT-SFX r2 (2026-08-01) kept the
same posture when the combat one-shots moved from live WebAudio synthesis to
the baked layered set above: everything is offline node DSP + local ffmpeg —
still zero downloads, zero third-party audio.
...
| C1 Ariete (user drop, model swap retired 2026-08-11) | DustyMojito | https://sketchfab.com/DustyMojito (user-supplied download) | Sketchfab Standard (free download; use-in-project OK, no raw redistribution — not CC) | `public/models/tanks/community/ariete-dustymojito.glb` (local oracle only) | The playable now uses the original procedural `buildArieteSource2026`; `MODEL_SOURCE.ariete` remains disabled. The owner's new full-resolution `c1_ariete_italian_mbt.glb` (SHA-256 `738505b3099016c938daa85f8eb82806cd6af19a2aa3e15b26810bc6c163607e`) was used locally for visual measurement only and was never copied into the repository. |

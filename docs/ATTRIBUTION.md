...
|---|---|---|---|---|
| Crew radio voice lines (15 call ids, 30 files incl. 2–3 read variants: spotting, kill confirms, hits/bounces both ways, ammo rack, fire/fire-out, engine/track/gun damage, reload "Up!"/"Loaded!", movement, "Firing!"/"On the way!", repairs) | Generated by this project via `tools/make-voices.mjs` (VOICE r1, 2026-08-01): **Piper neural TTS run 100% locally** (piper-tts 1.6.0, GPL-3.0 build-time tool only — nothing of it ships), four voice models = four crew personas (table below) → ffmpeg intercom chain (silence trim, speechnorm, 300–3400 Hz bandpass, 4:1 compression, light bit-crush grit, seeded pink-noise static bed, squelch clicks) → 2-pass loudness normalize to −19 LUFS → mono 24 kHz Opus. Old macOS-`say` set retired (robotic) — preserved at `shots/voices-r1/old-full/`, A/B pairs at `shots/voices-r1/ab/` | Local synthesis — this repo (re-runnable: `node tools/make-voices.mjs`; engine + models auto-bootstrap into `~/.cache/cot-piper` via anonymous downloads from GitHub releases / `huggingface.co/rhasspy/piper-voices`) | Recorded speech is synthetic — no third-party recordings sampled. Voice-model dataset licenses verified per MODEL_CARD (table below): all public-domain / CC0 / CC BY-SA 4.0 — **deliberately no NonCommercial models**, so this payload stays out of the NC quarantine | `public/audio/voice/*.ogg` (30 files, ~88 KiB total) |

Voice models (from `huggingface.co/rhasspy/piper-voices`, anonymous download;
license = the training dataset's license as stated in each MODEL_CARD):

| Crew role | Piper model | Training dataset | Dataset license |
|---|---|---|---|
| Commander (UK male) | `en_GB-northern_english_male-medium` | OpenSLR 83 — Crowdsourced UK/Ireland English dialect corpus (Google) | CC BY-SA 4.0 (attribution: openslr.org/83) |
| Gunner (deep US male) | `en_US-joe-medium` | OHF-Voice voice-datasets (Joe) | CC0 |
| Driver (US male) | `en_US-john-medium` | LibriVox recordings | Public domain |
| Loader (US female) | `en_US-kristin-medium` | LibriVox recordings | Public domain |
...
for the VOICE r1 neural crew round (2026-08-01): local/offline engines only,
anonymous downloads only, no accounts/keys/cloud TTS — the macOS "aarch64"
piper binary release turned out to be mislabeled x86_64 (no Rosetta on this
Mac), so the engine runs from the official `piper-tts` PyPI wheels instead.

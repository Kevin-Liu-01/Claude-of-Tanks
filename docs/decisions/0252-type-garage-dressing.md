...
Status: amended by [0295](0295-open-map-garage-staging.md)
...
Abrams, T-90M, and K2 builders behind the existing garage-lull gate.
...
is `pre-6c7b07533-original`. The same four exact maintenance scenes are mounted
in every staging area. The old additive three-tank/three-turret layout and its
Leclerc builder are removed. None of this work occurs while battle owns the
renderer. Verdant alone uses the original enclosed ceiling and full truss run;
the other selections use open map-bound staging roots described by ADR 0295.
...
- Vehicle exhibits must come from the first-party fleet factory; the four
  maintenance scenes are shared rather than rebuilt for each environment.

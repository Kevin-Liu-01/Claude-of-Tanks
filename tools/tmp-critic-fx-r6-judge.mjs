// TEMP critic probe (effects_combat r5 judging) — mid-motion frame sequences:
// battle-start flyby, live rough-ground drive (suspension/dust/sway), chase-cam
// player fire event, close side-cam fire sequence (flash/recoil/smoke over
// time), tracer+impact wide, real shell-kill destruction sequence, live
// de-track while driving. Output: shots/critic_fx_r6_judge/.
...
try {
  ...
  await page.evaluate(() => {
    ...
    D.game.player.state.yaw += 0.9; // steer off the road onto rough meadow
    window.__TEL = [];
  ...
  // ---- B2. chase-cam player fire (what the player actually sees) --------
  await page.evaluate(() => {
    const D = window.__DEBUG;
    D.rig.release();
    D.aimAtNearest();
  ...
  await sleep(70); await shot('chase_fire_a');
  await sleep(80); await shot('chase_fire_b');
  await page.evaluate(() => { window.__DEBUG.flags.forceFire = false; });
  await sleep(150); await shot('chase_fire_c');
  await sleep(400); await shot('chase_fire_d');
  ...
  const fired = await page.evaluate(() => {
    ...
    D.aimAtNearest();
    D.game.phase = 'battle';
    D.fastForward(4); // gun settles + reload
  ...
  t = 0;
  for (const age of [0.1, 0.25, 0.55, 0.9, 1.5, 2.5, 4.5, 7.0, 12.0, 20.0]) {
    await page.evaluate((o) => {
      const D = window.__DEBUG;
      D.game.phase = 'battle';
      if (o.dt > 0) D.fastForward(o.dt);
  ...
  await sleep(900); await shot('detrack_live2');
} catch (e) {

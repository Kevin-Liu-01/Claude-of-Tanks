...
const arm = source.indexOf('await page.evaluate(installPerfSampler, { sampleMs: seconds * 1000, waitForControl: true })');
...
assert.match(source, /if \(!earlyWindow\) \{\s*await page\.evaluate\(installPerfSampler, \{ sampleMs: seconds \* 1000, waitForControl: false \}\)/,
...
assert.ok(cameraStart > source.indexOf('await page.evaluate(installPerfSampler, { sampleMs: seconds * 1000, waitForControl: false })'),
...
assert.match(source, /if \(rosterProvenance && !report\) report = \{[\s\S]*?failure: err\.message/,
...
assert.ok(failureCapture > diagnosticGc && failureCapture < source.indexOf('if (rosterProvenance && !report) report ='),

...
const loaderOutputPath = resolve('src/vehicles/combatAnatomyLoaders.generated.js');
...
const groupSources = Object.fromEntries(groupNames.map((group) => [
  join(groupOutputDir, `${group}.generated.js`),
  fileHeader
    ...
    + `export const COMBAT_ANATOMY_CALIBRATIONS = Object.freeze(${JSON.stringify(rowsByGroup[group])});\n`,
]));
const source = fileHeader
  + groupNames.map((group) => `import { COMBAT_ANATOMY_CALIBRATIONS as ${groupBinding(group)} } from './combatAnatomyGroups/${group}.generated.js';`).join('\n')
...
const loaderSource = `// Generated by tools/gen-combat-anatomy.mjs. Do not hand-edit.\n`
  ...
  + groupNames.map((group) => `  ${JSON.stringify(group)}: () => import('./combatAnatomyGroups/${group}.generated.js'),`).join('\n')
  + `\n});\n`;
const outputs = {
  [outPath]: source,
...
const staleGroupPaths = existsSync(groupOutputDir)
  ? readdirSync(groupOutputDir)
    .filter((name) => name.endsWith('.generated.js'))

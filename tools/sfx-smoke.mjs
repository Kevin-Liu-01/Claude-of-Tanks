#!/usr/bin/env node
// sfx-smoke.mjs — end-to-end verification for the COMBAT-SFX redesign (r2).
//
...
//       - writes new_*.wav A/B copies into shots/sfx-r2/ab/ and, when
...
//       - writes old_*.wav + old-metrics.json into shots/sfx-r2/ab/
//
...
const outDir = resolve('shots/sfx-r2');
...
try {
  ...
  if (!CAPTURE_OLD && existsSync(join(abDir, 'old-metrics.json'))) {
    ...
    writeFileSync(join(abDir, 'README.md'),
      `# COMBAT-SFX r2 — A/B listening copies\n\n` +
